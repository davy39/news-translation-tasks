...
According to the [Python Documentation](https://docs.python.org/3/tutorial/datastructures.html#dictionaries):

* Keys have to be unique within one dictionary.

> It is best to think of a dictionary as a set of _key: value_ pairs, with the requirement that the keys are **unique** (within one dictionary).

* Keys have to be immutable. 

> Unlike sequences, which are indexed by a range of numbers, dictionaries are indexed by _keys_, which can be any **immutable** type; strings and numbers can always be keys.
...
We are checking if the **_key_** 15 is in the dictionary, not the value. This is why the expression evaluates to `False`.

💡 **Tip:** You can use the `in` operator to check if a value is in a dictionary with <dict>[.values()](https://docs.python.org/3/library/stdtypes.html#dict.values).
...
The length of a dictionary is the number of key-value pairs it contains. You can check the length of a dictionary with the [len()](https://docs.python.org/3/library/functions.html#len) function that we commonly use, just like we check the length of lists, tuples, and strings:
...
To do this, we need to use the built-in method [.items()](https://docs.python.org/3/library/stdtypes.html#dict.items), which allows us to iterate over the key-value pairs as tuples of this format `(key, value)`. 
...
With [.items()](https://docs.python.org/3/library/stdtypes.html#dict.items) and for loops, you can use the power of a tuple assignment to directly assign the keys and values to individual variables that you can use within the loop:
...
You can iterate over the values of a dictionary using the [.values()](https://docs.python.org/3/library/stdtypes.html#dict.values) method.
...
💡 **Tips:** To learn more about dictionary methods, I recommend reading [this article in the Python Documentation](https://docs.python.org/3/library/stdtypes.html#mapping-types-dict).
...
**I really hope you liked my article and found it helpful.** Now you can work with dictionaries in your Python projects. [Check out my online courses](https://www.udemy.com/user/estefania-cn/). Follow me on [Twitter](https://twitter.com/EstefaniaCassN). ⭐️
