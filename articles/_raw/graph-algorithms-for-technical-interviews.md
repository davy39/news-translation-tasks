...
Watch the full course below or on [the freeCodeCamp.org YouTube channel](https://youtu.be/tWVWeAqZ0WU) (2-hour watch).
...
Following every approach, we're also going to implement the code of course, I'm going to be writing all of my code in JavaScript, you'll be able to follow along in any language that you like.
...
So if you're in JavaScript, they'll be an object, if you're in Python, they'll be a dictionary.
...
So you can actually build this algorithm will kind of talk about things at a high level, consider this the pseudocode, then, of course, we'll express it in some JavaScript code later on.
...
For me in JavaScript, that's as simple as just using a JavaScript array, right? I'll make it empty at the start.
...
So if you're familiar in JavaScript, if you just use a for of loop, they'll iterate an order through an array.
...
With that source note, again, the queue here is just going to be an array in JavaScript.

So I'll say const q equals an array that begins with just the source node.

Awesome.

And I'm going to use this queue by just committing to two specific methods on my arrays in JavaScript.
...
Let's go over Ah, JavaScript solution for this has path problem.
...
So I can create a queue, kind of in a pinch, I always just use an array in JavaScript, I'm gonna initialize that queue with my source on it.
...
Now let's go over a JavaScript solution for this undirected path problem.
...
And I know I want my adjacency list to be in the form of a plain old JavaScript object.
...
And I'll make it a new JavaScript set.

So if you're unfamiliar with sets, and JavaScript, they're really just a collection of items.
...
What I want to do is all right check if my source node is already in the visited set to do that in JavaScript I can check visited out has.
...
Hey, programmers, Alvin here, right, now let's go over a JavaScript solution for this connected components count problem.
...
So I can say for let node of my graph really say in my graph here, because for this problem we're given looks like JavaScript objects.
...
So I can create my constant visited, make it a JavaScript set, because JavaScript sets off for me O of one lookup, and o of one addition, I can pass this visited set my reference until all of these calls over here.
...
So this is a very common mistake in JavaScript want to bring in, I think this example, I could test it manually.
...
And other times they're actually numbers, notice that they're missing quotes, that has to do with, you know, kind of just JavaScript objects, technically, keys of a JavaScript object are going to be always converted as strings, although the data within these arrays over here is going to be number.
...
So that's a really important thing to watch out for in JavaScript.

And JavaScript is pretty unique in that regard.
...
Hey, programmers, Alvin here, right now let's go over a JavaScript solution for this largest component problem.
...
Now let's go over a JavaScript solution for the shortest path problem.
...
For me, that means a JavaScript object, I'll call it graph, by the end of this function, this will help I'm going to return the graph and some common code, I'm going to iterate through every pair, right, basically every edge, I'll say, for left edge of edges.
...
So if you're unfamiliar with the set constructor in JavaScript, if you want to initialize it with some values, you actually have to pass in an array containing those values.
...
Hey, programmers, Alvin here, right now, let's go over a JavaScript walkthrough for this island count problem.
...
So in JavaScript, just a new set, you're probably wondering, you know, what am I going to make the members of this set? Well, I need to designate positions, right? Think about it as if positions are like the nodes in this grid in this graph, right.
...
I think this is actually worth going through specifically more of like a language thing in JavaScript, compared to other programming languages.
...
Hey, programmers, Alvin here, right now let's go over a JavaScript walkthrough for this minimum Island problem.
...
So I'll create const visited gonna make it my neighs JavaScript set.

And a few reasons for that.

Well, for one JavaScript set, gives me O of one lookup, but also o of one insertion.
...
So I think a great default value here is to use positive infinity so I can set some will say min size variable To be positive infinity, JavaScript, if I make it positive infinity, I know when I encounter any like valid Island size, it's going to be less than infinity.
