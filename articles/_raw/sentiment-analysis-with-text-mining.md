...
For this project, we'll be working with the [Twitter US Airline Sentiment data set on Kaggle](https://www.kaggle.com/crowdflower/twitter-airline-sentiment). It contains the tweet’s text and one variable with three possible sentiment values. Let's start by importing the packages and configuring some settings.
...
_Imbalanced distribution of the target class labels_
...
Below you can find the distribution of the number of words in a tweet per target class. For brevity, we will limit us to only this variable. The charts for all TextCounts variables are in the [notebook on Github](https://github.com/bertcarremans/TwitterUSAirlineSentiment).
...
In our function `grid_vect`we additionally generate the `classification_report` on the test data. This provides some interesting metrics per target class. This might be more appropriate here. These metrics are the precision, recall and F1 score**.**
...
With the elements of the [confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix) we can calculate Precision and Recall.
...
Here we will compare the performance of a `MultinomialNB`and `LogisticRegression`.
...
One issue with CountVectorizer is that there might be words that occur frequently. These words might not have discriminatory information. Thus they can be removed. [TF-IDF (term frequency — inverse document frequency)](https://en.wikipedia.org/wiki/Tf%E2%80%93idf)can be used to down-weight these frequent words.
...
The Word2Vec algorithm is part of the [gensim](https://radimrehurek.com/gensim/models/word2vec.html) package.
...
We only consider LogisticRegression as we have negative values in the Word2Vec vectors. MultinomialNB assumes that the variables have a [multinomial distribution](https://en.wikipedia.org/wiki/Multinomial_distribution). So they cannot contain negative values.
...
For the fun, we will use the best model and apply it to some new tweets that contain _@VirginAmerica_. I selected 3 negative and 3 positive tweets by hand.
