...
* **Complex string with run-time data:** Friends, Android has you covered. There are parameterized strings supported by the platform with a syntax similar to the one used in Java’s `String.format()`. More than that, plural-strings (using different strings based on the quantity of something) are also supported. Check out [this StackOverflow post](https://stackoverflow.com/questions/2397613/are-parameters-in-strings-xml-possible) for parameterized strings and the [official documentation](https://developer.android.com/guide/topics/resources/string-resource.html) for plural-strings.
...
* You can use as few of the features in data-binding as just using it to eliminate `findViewById` calls to much more advanced features (like in [this post](https://medium.com/google-developers/android-data-binding-recyclerview-db7c40d9f0e4), George Mount of Google tries to write a single adapter for all the recycler-views in an app).
...
You can take a look at [this post](https://medium.com/code-better/hiding-api-keys-from-your-android-repository-b23f5598b906) to figure out how to hide your API keys from your repository while still including them in the build process and making them available in your code.
...
* **Cheat-sheet** — [This is an official printable cheat-sheet](https://resources.jetbrains.com/storage/products/intellij-idea/docs/IntelliJIDEA_ReferenceCard.pdf) for the keyboard shortcuts by Jetbrains (the company behind IntelliJ). Versions for both Windows and Mac are available.
* **Official Guide** — [This is the official guide](https://www.jetbrains.com/help/idea/mastering-intellij-idea-keyboard-shortcuts.html) provided by Jetbrains to mastering keyboard shortcuts on IntelliJ platform.
* Also check out [these](https://www.youtube.com/watch?v=hdrAlhRI5vM) [two](https://www.youtube.com/watch?v=eOV2owswDkE) videos

#### That’s all folks

These are the five things which I think anyone working in Android development should focus on first. If you have any other suggestions regarding these or any other topics under the sky, do contact me on Twitter at [@varun_barad](https://twitter.com/varun_barad).
