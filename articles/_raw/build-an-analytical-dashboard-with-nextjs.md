...
* [The Subject Area](#heading-the-subject-area)
    
* [The Final Dashboard](#heading-the-final-dashboard)
    
* [How to Configure Highcharts and Flexmonster for Our Next.js App](#heading-how-to-configure-highcharts-and-flexmonster-for-our-next-js-app)
    
* [How to Set Up the Dashboard](#heading-how-to-set-up-the-dashboard)
    
* [How to Set Up the Chart Configuration](#heading-how-to-set-up-the-chart-configuration)
    
* [Filling Up the Dashboard with Other Charts](#heading-filling-up-the-dashboard-with-other-charts)
    
* [Connect between Flexmonster and Highcharts](#heading-connect-between-flexmonster-and-highcharts)
    
* [Full demo link](#heading-full-demo-link)
...
1. Install Next.js (installation guide [here](https://nextjs.org/docs/app/getting-started/installation))
...
* [PivotTable.js](https://github.com/nicolaskruchten/pivottable) — basic open-source pivot functionality
    
* [Orb](https://www.npmjs.com/package/orb) — clean pivot UI with neat design
    
* [WebDataRocks](https://www.webdatarocks.com/demos/react/pivot-table-demo/) — modern, free pivot table solution
    

**Charts:**

1. [Nivo](https://nivo.rocks/) — awesome free charts with D3 power
    
2. [Chart.js](https://www.chartjs.org/) — simple yet powerful canvas charts
    
3. [Recharts](https://recharts.org/en-US/) — React components for D3 charts
...
In this tutorial, we’ll create an interactive dashboard using Next.js, featuring a pivot table and several charts on the final page.

![The final dashboard](https://cdn.hashnode.com/res/hashnode/image/upload/v1736375903721/ea1faca7-0bf3-4d32-83fc-5963035b832e.png align="center")

### **Tools**

I was looking for the tools that offer customizable and functional UI, wide range of chart types and pivot table features. So, I focused on [Flexmonster](https://www.flexmonster.com/) for creating tables and [Highcharts](https://www.highcharts.com/) for the charts. These tools offer user-friendly interfaces, extensive customization options, handle large datasets, and integrate with each other.
...
### 3\. Import Flexmonster styles to `global.css`:
...
Now you’ll create the wrapper for your future pivot table that integrates Flexmonster and Highcharts. First, let’s create a `PivotWrapper.tsx` file.
...
Now, import the wrapper in the `analytical-dashboard/page.tsx` file. You can change the route name `analytical-dashboard` to your liking:
...
Let’s define a `WithHighcharts()` function to render our dashboard and work with its essential parts. First, we’ll initialize the [ref object](https://react.dev/reference/react/useRef) for the pivot table to access it and its events. The charts will be created in the `createChart()` function, which runs when pivot data loads (in the `reportComplete` event). Finally, we return the `layout` function.

Add the code below to the `analytical-dashboard/page.tsx` file:
...
If you are interested in a detailed integration of Flexmonster with [Next.js](https://www.flexmonster.com/doc/integration-with-next-js/) and [Highcharts](https://www.flexmonster.com/doc/integration-with-highcharts/), you can check the complete documentation that I’ve linked here.
...
In the previous step, we already created a default grid. If you run it now, it will look like this:

![The default grid of the Flexmonster pivot table](https://cdn.hashnode.com/res/hashnode/image/upload/v1736375968187/a71c06db-1f6d-4d0a-ad0e-4b4457d5f575.png align="center")
...
![GIF: connecting dataset fields to the grid](https://cdn.hashnode.com/res/hashnode/image/upload/v1736376015936/122a95ed-98aa-4128-9c18-d31e95be4437.gif align="center")
...
![Location and Age grid with raw data](https://cdn.hashnode.com/res/hashnode/image/upload/v1736376040585/181e9f1d-38ed-4731-8700-bbee02a7bbd5.png align="center")

Alright, let’s tidy up this table a bit. We’ll keep just one decimal place, which you can do in the **Format** tab like this:

![Format tab](https://cdn.hashnode.com/res/hashnode/image/upload/v1736378089026/8e853863-86ee-40a8-be66-6fa9c339f164.png align="center")

It’s quite simple: select the required field (in this case, there will only be one) in the **Choose Value** field and set **Decimal Places** to 1.

We’ll also remove the **Grand Totals** since they are unnecessary for our purposes. You can do this in the **Settings** tab:

![Layout options](https://cdn.hashnode.com/res/hashnode/image/upload/v1736378067956/a66dd0f9-0589-473e-b0af-c9c6bfe0d0b1.png align="center")
...
![GIF: filtering data](https://cdn.hashnode.com/res/hashnode/image/upload/v1736376126270/4bc3f72c-5e68-4e39-b1da-19f179ad2e4f.gif align="center")
...
Here’s my idea: we want to identify areas where the distribution of respondents by age is uneven. We have four age ranges: youngsters (18-29), millennials (30-44), Gen X (45-59), and seniors (60+). In a balanced distribution, each age group should account for 100/4 = 25% (with a margin of ±2÷3%).
...
* &lt;17% or &gt;32%: strongly underrepresented or overrepresented.
...
![GIF: Conditional formatting](https://cdn.hashnode.com/res/hashnode/image/upload/v1736376163028/c7c28901-30b4-49f7-b146-2ed5e6dbfc9f.gif align="center")
...
Let’s move on to the `createChart()` function. At the moment, it’s empty. To define the chart, paste [`pivotRef.current!.flexmonster.highcharts?.getData()`](https://www.flexmonster.com/doc/integration-with-highcharts/?r=stfc2) into it. It gets three parameters inside: `chart options`, `callbackHandler`, and `updateHandler`.
...
**Note!** You should change `data` **before** it passes to the `Highcharts.chart()` function as a parameter.
...
Now, let’s create the remaining charts following the same process – adjusting the chart `type`, `slice` options, and `callbackHandler` functions for each diagram. You can find the full chart configuration of each chart at [the end of the guide](#heading-full-demo-link) on my GitHub.

### **1\. Frequency of using airlines distributed by locations**

Here we can see with a [stacked column chart](https://www.highcharts.com/docs/chart-and-series-types/column-chart) how often people from different regions fly.
...
Still, pollsters managed to gather some insights about rule violations. The [bar chart](https://www.highcharts.com/docs/chart-and-series-types/bar-chart) above reveals a trend: frequent flyers tend to neglect the rules, while those who fly less often are more likely to follow them.
...
The [line chart](https://www.highcharts.com/docs/chart-and-series-types/line-chart) below shows that age influences your ease of communication with strangers, with middle-aged people often feeling more comfortable initiating conversations.
...
The [area chart](https://www.highcharts.com/docs/chart-and-series-types/area-chart) covers all the questions about what passengers consider rude, allowing us to observe how different age groups respond to various situations. 
...
I attached links to the Highcharts documentation for each chart, but additional charts are available. You can inspect them [here](https://www.highcharts.com/docs/chart-and-series-types/chart-typeshttps://www.highcharts.com/docs/chart-and-series-types/chart-types).

## How to Sync Data Between Flexmonster and Highcharts

Let’s recall the earlier section on [configuring chart options](#heading-2-define-the-chart-options). Here, adding the `reportFilters` field to the slice object enables data synchronization across all dashboard elements, from pivot table to charts.
...
You can check out the full Next.js demo app and the dashboard yourself [here](https://github.com/StefanErrorerko/AnalyticalDashboardFlyingEtiquette).
