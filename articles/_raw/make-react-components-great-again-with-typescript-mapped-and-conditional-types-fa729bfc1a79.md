...
The `T[K]` operator gets the type for the provided key. `Person['name']`will result in `string` and `Person[_keyof_` Person] will result in `string | number | ((msg: string) => string)`.
...
_Inferred type view in VsCode_
...
Now let us convert this simple example to TypeScript so that it's type safe. Take a look at **_index.tsx_** in the below code sandbox:
