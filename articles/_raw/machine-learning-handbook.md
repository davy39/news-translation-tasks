---
title: Machine Learning Fundamentals Handbook – Key Concepts, Algorithms, and Python
  Code Examples
subtitle: ''
...
1. [**Chapter 1: What is Machine Learning?**](#heading-chapter-1-what-is-machine-learning)
    
2. [**Chapter 2: Most popular Machine Learning algorithms**](#heading-chapter-2-most-popular-machine-learning-algorithms)
...
[**3\. Chapter 3: Feature Selection**](#heading-chapter-3-feature-selection-in-machine-learning)
...
[**4\. Chapter 4: Resampling Technique**](#heading-chapter-4-resampling-techniques-in-machine-learning)
...
[**6\. Closing**](#heading-about-the-author-thats-me)
...
If you have no prior statistical knowledge and wish to learn or refresh your understanding of essential statistical concepts, I'd recommend this article: [**Fundamental Statistical Concepts for Data Science**](https://link-to-article.com/)

For a comprehensive guide on kickstarting a career in Data Science and AI, and insights on securing a Data Science job, you can delve into my previous handbook: [**Launching Your Data Science & AI Career**](https://link-to-handbook.com/)
...
Note that we will not delve into [unsupervised learning techniques](https://www.freecodecamp.org/news/supervised-vs-unsupervised-learning/) here, or enter into granular details of each algorithm.
...
The main idea behind linear regression is to find the best-fitting straight line, *the regression line,* through a set of paired ( X, Y ) data. One example of the Linear Regression application is modeling the impact of flipper length on penguins’ body mass\_,\_ which is visualized below:
...
Note that the above description for Linear Regression is from my article named [Complete Guide to Linear Regression](https://pub.towardsai.net/complete-guide-to-linear-regression-86c5eddb7eda).
...
Another very popular Machine Learning technique is Logistic Regression which, though named regression, is actually a supervised classification technique\_.\_
...
π\_k represents the overall **prior probability** that a randomly chosen observation comes from the k\_th\_ class. f\_k(x) , which is equal to Pr(X = x|Y = k), represents the **posterior probability**, and is the **density function of X** for an observation that comes from the k\_th\_ class (density function of the predictors).

This is the probability of X=x given the observation is from certain class. Stated differently, it is the probability that the observation belongs to the k\_th\_ class, given the predictor value for that observation.
...
where μ\_k and σ\_k² are the mean and variance parameters for the k\_th\_ class. Assuming that σ\_¹² = · · · = σ\_K² (there is a shared variance term across all K classes, which we denote by σ2).
...
Another classification method that relies on Bayes Rule\*\*,\*\* like LDA, is Naïve Bayes Classification approach. For more about Bayes Theorem, Bayes Rule and a corresponding example, you can read [these](https://www.freecodecamp.org/news/bayes-rule-explained/) [articles](https://towardsdatascience.com/fundamentals-of-statistics-for-data-scientists-and-data-analysts-69d93a05aae7).
...
where Y stands for the class of observation, k is the k\_th\_ class and x1, …, xn stands for feature 1 till feature n, respectively. f\_k(x) = Pr(X = x|Y = k), represents the posterior probability, which like in case of LDA is the density function of **X** for an observation that comes from the k\_th\_ class (density function of the predictors).
...
Like Logistic Regression, in the case of the Naïve Bayes classification approach we use Maximum Likelihood Estimation (MLE) as estimation technique. There is a great article providing detailed, coincise summary for this approach with corresponding example which you can find [here](https://towardsdatascience.com/naive-bayes-classifier-81d512f50a7c).
...
Naïve Bayes Classifier has proven to be faster and has a higher bias and lower variance. Logistic regression has a low bias and higher variance. Depending on your individual case, and the [bias-variance trade-off](https://towardsdatascience.com/bias-variance-trade-off-overfitting-regularization-in-machine-learning-d79c6d8f20b4), you can pick the corresponding approach.
...
When building a decision tree, especially when dealing with large number of features, the tree can become too big with too many leaves. This will effect the **interpretability** of the model, and might potentially result in an [**overfitting**](https://www.freecodecamp.org/news/what-is-overfitting-machine-learning/) problem. Therefore, picking a good stopping criteria is essential for the interpretability and for the performance of the model.
...
Entropy is another node purity measure, and like the Gini index, the entropy will take on a small value if the m\_th\_ node is pure. In fact, the Gini index and the entropy are quite similar numerical and can be expressed as follows:‌
...
* Choose the best model among these p-k models, and define it M\_(k+1) by using performance metrics such as [RSS](https://en.wikipedia.org/wiki/Residual_sum_of_squares)/[R-squared](https://www.investopedia.com/terms/r/r-squared.asp).
    

**Step 3:** Select the single model with the best performance among these M\_0,….M\_p models (one with smallest [Cross Validation Error](https://datascienceplus.com/cross-validation-estimating-prediction-error/), [C\_p](https://medium.com/analytics-vidhya/model-selection-cp-aic-bic-and-adjusted-r2-6a0af25945b6), [AIC](https://medium.com/analytics-vidhya/model-selection-cp-aic-bic-and-adjusted-r2-6a0af25945b6) [(Akaike Information Criterion)](https://en.wikipedia.org/wiki/Akaike_information_criterion), [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion) (Bayesian Information Criteria)or [adjusted R-squared](https://en.wikipedia.org/wiki/Coefficient_of_determinationv) is your best model M\*).
...
Each time, the model is fitted again to identify the least statistically significant variable until the stopping rule is reached. (For example, all p- values need to be smaller then 5%.) Then we compare all these models with different number of predictors (best M\_ks) and select the single model with the best performance among these M\_0,….M\_p models (one with smallest [Cross Validation Error](https://datascienceplus.com/cross-validation-estimating-prediction-error/), [C\_p](https://medium.com/analytics-vidhya/model-selection-cp-aic-bic-and-adjusted-r2-6a0af25945b6), [AIC](https://medium.com/analytics-vidhya/model-selection-cp-aic-bic-and-adjusted-r2-6a0af25945b6) [(Akaike Information Criterion)](https://en.wikipedia.org/wiki/Akaike_information_criterion), [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion) (Bayesian Information Criteria)or [adjusted R-squared](https://en.wikipedia.org/wiki/Coefficient_of_determinationv) is your best model M\*).
...
* Choose the best model among these k models, and define it M\_(k-1) by using performance metrics such as [RSS](https://en.wikipedia.org/wiki/Residual_sum_of_squares)/[R-squared](https://www.investopedia.com/terms/r/r-squared.asp).
...
Let's explore examples of multiple linear regression, involving p\_p\_ independent variables or predictors utilized to model the dependent variable y\_y\_.
...
This is a simple approach to randomly split the data into training and validation sets. This approach usually uses [Sklearn’s train\_test\_split()](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html) function.
...
The choice of k in K-fold is a matter of [Bias-Variance Trade-Off](https://towardsdatascience.com/bias-variance-trade-off-overfitting-regularization-in-machine-learning-d79c6d8f20b4) and the efficiency of the model. Usually, K-Fold CV and LOOCV provide similar results and their performance can be evaluated using simulated data.
...
Read more about it [here](https://github.com/TatevKaren/mathematics-statistics-for-data-science/tree/main/Bootstrapping).‌ ‌ ‌
...
* ∇\_J\_(*θ*): This is the gradient of the cost function *J* with respect to the parameter θ It indicates the direction and magnitude of the steepest increase of *J*. By subtracting this from the current parameter value (multiplied by the learning rate), we adjust *θ* in the direction of the steepest decrease of *J*.
