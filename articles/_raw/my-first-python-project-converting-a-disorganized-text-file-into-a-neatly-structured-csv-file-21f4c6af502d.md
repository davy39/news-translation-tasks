---
title: 'My first Python project: how I converted a disorganized text file into a neat
  CSV file'
subtitle: ''
author: freeCodeCamp
co_authors: []
...
_Here, ‘Glower’ is preceded by a newline whereas others are not_

![Image](https://cdn-media-1.freecodecamp.org/images/FN02Y6fnT3JCd7o6-ld2BmvCrjMPjKoicKZ9)
_Here, the example of ‘Shirk’ is preceded by a newline_
...
_‘Topic’ is the key and the rest are the key’s value. The format gets slightly trickier to maintain as each topic contains multiple words_
...
_The above code finds matches as per the regex and adds it to a list of strings._
...
_Funny, how a single line — “errors = ‘replace’ ” did the job_
...
_Sometimes, all you need is 1 line of code!_
...
_First ‘for’ loop replace ‘\n’ with ‘’ and the second one deletes ‘’, thus giving a clean list._
...
_Slicing collects two indexes and extracts the string between them_
...
_The format for storing all data as one single value for a particular key_
...
_A separate list of words for each key and similarly for meanings and examples_
...
_Doing the right things isn’t always easy and what looks easy isn’t always right!_
...
_This fragment of code worked flawlessly! :)_
...
_List5 is the required final output_
...
_This output is based on the beta file containing both prior and current fragments of code._
...
_Here, the example sentences for ‘Incense’ and ‘Ire’ takes up two lines instead of one._
...
_Modification — inserted another ‘elif’ condition and rewrote code for the ‘else’ part_
...
_The Enumerate Function adds a counter to an iterable enabling easy debugging by checking the line number where the error occurred._
...
_This too worked well for a while but then snap!_
...
_This returns ‘True’ if it matches lines containing a number and a word at the beginning of the line instead of the previous one which would match the same anywhere in the line._
...
_Replaced this with the below code_

![Image](https://cdn-media-1.freecodecamp.org/images/dDKUZ56tPnPXy7K2eSCDX-YkGsOJW0prraFX)
_This is much more efficient than the previous one as it makes use of the already defined ‘hasNumbersDot’ function. Also, it matched the full line (full key)_
...
_Prior to this modification, the last example sentence could not be extracted from each key_
