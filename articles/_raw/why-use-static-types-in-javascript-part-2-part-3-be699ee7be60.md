...
In summary, the signature for our app state is now `AppState<E,` D> — `w`here E is of the `shape Htt`pErro`r` and `D is Payment`Method`. And Ap`pState has four (and only these four) possible st`ates: NotF`et`ched, Fe`tc`hing, F`ailur`e and S`uccess.
