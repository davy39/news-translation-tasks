...
> Get this post in PDF/ePub/MOBI format at [vuehandbook.com](https://vuehandbook.com)
...
Everything changed when Google unveiled [Google Maps](https://www.google.com/maps) and [GMail](https://www.google.com/gmail), two applications that ran inside the browser. [Ajax](https://developer.mozilla.org/en-US/docs/Web/Guide/AJAX/Getting_Started) made asynchronous network requests possible. Over time, developers started building on top of the Web platform, while engineers worked on the platform itself — browsers, the Web standards, the browser APIs, and the JavaScript language.

Libraries like [jQuery](https://jquery.com/) and [Mootools](https://mootools.net/) were the first big projects that built upon JavaScript and were hugely popular for a while. They basically provided a nicer API to interact with the browser and provided workarounds for bugs and inconsistencies among the various browsers.

Frameworks like [Backbone](http://backbonejs.org/), [Ember](https://www.emberjs.com/), [Knockout](http://knockoutjs.com/), and [AngularJS](https://angularjs.org/) were the first wave of modern JavaScript frameworks.

The second wave, which is the current one, has [React](https://reactjs.org/), [Angular](https://angular.io/), and [Vue](https://vuejs.org/) as its main actors.
...
Frameworks abstract the interaction with the browser and the DOM. Instead of manipulating elements by referencing them in the DOM, we [declaratively](https://en.wikipedia.org/wiki/Declarative_programming) define and interact with them, at a higher level.

Using a framework is like using the [C programming language](https://en.wikipedia.org/wiki/C_(programming_language)) instead of using the [Assembly language](https://en.wikipedia.org/wiki/Assembly_language) to write system programs. It’s like using a computer to write a document instead of using a typewriter. It’s like having a self-driving car instead of driving the car yourself.
...
Its [npm](https://www.npmjs.com/) download count is growing every day, and now it’s at ~350,000 downloads per week.
...
You don’t need to know about [webpack](https://webpack.js.org/), [Babel](https://babeljs.io/), npm or anything to get started with Vue. But when you’re ready, Vue makes it simple for you to rely on them.
...
The new Angular (Angular 2.0) also solved many of the AngularJS issues, but in very different ways. It also requires a buy-in to [TypeScript](https://www.typescriptlang.org/) which not all developers enjoy using (or want to learn).
...
One big difference with React is [JSX](https://reactjs.org/docs/introducing-jsx.html). While you can technically use JSX in Vue, it’s not a popular approach and instead the [templating system](https://vuejs.org/v2/guide/syntax.html) is used. Any HTML file is a valid Vue template. JSX is very different than HTML, and has a learning curve for people on the team that might only need to work with the HTML part of the app, like designers.

Vue templates are very similar to [Mustache](http://mustache.github.io/) and [Handlebars](https://handlebarsjs.com/) (although they differ in terms of flexibility). As such, they are more familiar to developers that already used frameworks like Angular and Ember.

The official state management library, [Vuex](https://vuex.vuejs.org/), follows the Flux architecture and is somewhat similar to [Redux](https://redux.js.org/) in its concepts. Again, this is part of the positive things about Vue, which saw this good pattern in React and borrowed it for its ecosystem. And while you can use Redux with Vue, Vuex is specifically tailored for Vue and its inner workings.
...
A simpler way, without having to install anything, is to go to [CodeSandbox](https://codesandbox.io/s/vue). The link opens the Vue CLI default application.

CodeSandbox is a cool code editor that allows you build apps in the cloud. You can use any npm package, and can easily integrate with [Zeit Now](https://zeit.co/now) for an easy deployment and with [GitHub](https://github.com/) to manage versioning.
...
The cool thing is that it’s an interactive process. You need to pick a preset. By default, there is one preset that provides Babel and [ESLint](https://eslint.org/) integration:
...
Next up: testing. Vue CLI lets me choose between the two most popular unit testing solutions: [Mocha + Chai](https://mochajs.org/) and [Jest](https://jestjs.io/).
...
Vue CLI then asks me if I prefer using [Yarn](https://yarnpkg.com/lang/en/) or NPM:
...
Extracted from the above, I made a sample [preset](https://github.com/flaviocopes/vue-cli-preset/blob/master/preset.json) which contains this configuration:
...
When you’re first experimenting with Vue, if you open the Browser Developer Tools, you will find this message: “Download the Vue Devtools extension for a better development experience: [https://github.com/vuejs/vue-devtools](https://github.com/vuejs/vue-devtools)"

![Image](https://cdn-media-1.freecodecamp.org/images/J-LJE-u3DphYF8pOpMnkCX9KoNz3fGp4OPea)
_Hint to install the Vue devtools_
...
Go to this page on the Google Chrome [Store](https://chrome.google.com/webstore/detail/vuejs-devtools/nhdogjmejiglipccpnnnanhbledajbpd) and click `**Add to Chrome**`.
...
You can find the Firefox dev tools extension in the Mozilla Add-ons [store](https://addons.mozilla.org/en-US/firefox/addon/vue-js-devtools/).
...
_Select component in the page_
...
[Visual Studio Code](https://code.visualstudio.com/) is one of the most used code editors in the world right now. Editors have, like many software products, a cycle. Once [TextMate](https://macromates.com/) was the favorite among developers, then it was [Sublime Text](https://www.sublimetext.com/), now it’s VS Code.
...
It’s called Vetur, it’s hugely popular (more than 3 million downloads), and you can find it [on the Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=octref.vetur).
...
[IntelliSense](https://code.visualstudio.com/docs/editor/intellisense) is also enabled by Vetur, for each different language, with autocomplete:
...
[Emmet](https://emmet.io/), the popular HTML/CSS abbreviations engine, is supported by default. You can type one of the Emmet abbreviations, and by pressing `tab` VS Code will automatically expand it to the HTML equivalent:
...
Most of the languages supported use [Prettier](https://prettier.io/) for automatic formatting, a tool that’s becoming an industry standard. It uses your Prettier configuration to determine your preferences.
...
[See on JSFiddle](https://jsfiddle.net/flaviocopes/nvgedhq4/)
...
[See on JSFiddle](https://jsfiddle.net/flaviocopes/3kebv908/)
...
This is using the [CommonJS](http://requirejs.org/docs/commonjs.html) syntax and it works as well. But I recommend using the ES Modules syntax, as that is a JavaScript standard.
...
Read [here](https://vuejs.org/v2/guide/custom-directive.html) if you’re interested in learning more.
...
For more on propagation, bubbling and capturing, see my [JavaScript events guide](https://flaviocopes.com/javascript-events).
...
Vuex is not the only state management option you can use in Vue (you can use [Redux](https://medium.com/@quincylarson/17a99705b8e1) too), but its main advantage is that it’s official, and its integration with Vue.js is what makes it shine.

With React you have the trouble of having to choose one of the many libraries available, as the ecosystem is huge and has no actual standard. Lately Redux was the most popular choice, with [MobX](https://mobx.js.org/getting-started.html) following up in terms of popularity. With Vue I’d go as far as to say that you won’t need to look around for anything other than Vuex, especially when starting out.
...
The examples I use will be put CodeSandbox, which is a great service that has a Vue CLI [sample](https://codesandbox.io/s/vue) ready to go. I recommend using it to play around.
...
The full, working source code is available [here](https://codesandbox.io/s/zq7k7nkzkm).
...
[UNPKG](https://unpkg.com/#/) is a very handy tool that makes every npm package available in the browser with a simple link.
...
> Get this post PDF/ePub/Kindle ebook at [vuehandbook.com](https://vuehandbook.com)
