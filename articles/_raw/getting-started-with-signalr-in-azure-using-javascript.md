...
One way to step away from polling is to integrate [SignalR](https://dotnet.microsoft.com/apps/aspnet/signalr), a persistent connection tool that uses websockets and related technologies to allow servers to _push_ updates to clients. 
...
# What does it do?
[Open-sourced](https://github.com/aspnet/AspNetCore/tree/master/src/SignalR) SignalR creates a persistent connection between a client and server. It uses websockets first, then longpolling and other technologies when websockets are unavailable. 
...
Because we're working with Azure, we're going to be creating function apps to interface with SignalR. I wrote a getting-started [blog post](https://wilkie.tech/tech/azure-function-apps-101/) about Azure function apps a little while ago. 
...
const { url: connectionUrl, accessToken } = await axios
  ...
  .catch(console.error)
```
I swear to you, the only place on the entire internet I found to let me know how to request a connection using the `userId` was an answer on [this Stack Overflow question](https://stackoverflow.com/questions/29509396/signalr-client-how-to-set-user-when-start-connection). 
...
_This post originally appeared on [wilkie.tech](https://wilkie.tech/tech/getting-started-with-signal-r-in-azure/)._
