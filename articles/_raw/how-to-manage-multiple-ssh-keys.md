...
If you have _some_ private key stored in file `~/.ssh/id_rsa`, SSH client will use that private key for communication encryption. If that key is passworded (as it should be), you will be prompted for a password, like so:
...
Can we somehow save passwords so we don’t have to type them in every time? If only there was something like a _keychain_ for saving password-protected SSH keys ?.

Rest assured, there are answers to all those questions.

## **Enter, SSH `config`**

As it turns out, [SSH configuration file](https://linux.die.net/man/5/ssh_config) is a thing, a thing which can help us out. It is a per-user configuration file for SSH communication. Create a new file: `~/.ssh/config` and open it for editing.
...
* [The ultimate guide to SSH keys](https://www.freecodecamp.org/news/the-ultimate-guide-to-ssh-setting-up-ssh-keys/)
* [A top-down introduction to SSH](https://www.freecodecamp.org/news/a-top-down-introduction-to-ssh-965f4fadd32e/)
