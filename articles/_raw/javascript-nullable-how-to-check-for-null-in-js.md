---
title: JavaScript Nullable – How to Check for Null in JS
subtitle: ''
...
_null is nothing; undefined is not something_
...
`leviticus` is _intentionally_ without an object value (**null**). Whereas `dune` is declared, yet it's _unintentionally_ missing a value (**undefined**).
...
Another curiosity is that when you loosely check for equality using [double equals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Equality_comparisons_and_sameness) `==`, `null` and `undefined` will return `true`.
...
But when you strictly check for equality using [triple equals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Equality_comparisons_and_sameness) `===`, null and undefined will return `false`.
...
* `null == undefined` evaluates as true because they _are_ loosely equal.
* `null === undefined` evaluates as false because they are not, _in fact_, equal.
...
Come say hey to me over on Twitter: [https://twitter.com/EamonnCottrell](https://twitter.com/EamonnCottrell)
