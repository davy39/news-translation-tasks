...
One of the major factors of Node's success is npm - its popular package manager, which allows JavaScript developers to share useful packages like [lodash](https://www.npmjs.com/package/lodash) and [moment](https://www.npmjs.com/package/moment) quickly and easily. 
...
You can specify a global flag `-g` if you want to install a package in the global context which you can use anywhere across your machine (this is common for command-line tooling packages like [live-server](https://github.com/tapio/live-server)). 
...
With the humongous number of packages that have been published and can easily be installed, npm packages are susceptible to bad authors with malicious intentions like [these](https://medium.com/@jsoverson/how-two-malicious-npm-packages-targeted-sabotaged-one-other-fed7199099c8). 

Realising that there was an issue in the ecosystem, the npm.js organisation came up with the [idea](https://blog.npmjs.org/post/173719309445/npm-audit-identify-and-fix-insecure) of `npm audit`. They maintain a list of security loopholes that developers can audit their dependencies against using the `npm audit` command. 
...
Sending a package to our [npmjs.com](https://npmjs.com) fulfillment centre is super easy as we only need to run `npm publish`. The tricky part, which is **not** specific to npm package authors, is determining the version of the package.
