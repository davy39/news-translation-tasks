---
title: System Design Interview Question Handbook – Concepts You Should Know
subtitle: ''
author: Zubin Pratap
co_authors: []
series: null
...
1. [Section 1: Networks & Protocols (IP, DNS, HTTP, TCP etc)](#heading-section-1-networks-and-protocols)
    
2. [Section 2: Storage, Latency & Throughput](#heading-section-2-storage-latency-amp-throughput)
    
3. [Section 3: Availability](#heading-section-3-system-availability)
    
4. [Section 4: Caching](#heading-section-4-caching)
    
5. [Section 5: Proxies](#heading-section-5-proxies)
    
6. [Section 6: Load Balancing](#heading-section-6-load-balancing)
    
7. [Section 7: Consistent Hashing](#heading-section-7-consistent-hashing)
    
8. [Section 8: Databases](#heading-section-8-databases)
    
9. [Section 9: Leader Election](#heading-section-9-leader-election)
    
10. [Section 10: Polling, Streaming, Sockets](#heading-section-10-polling-streaming-sockets)
    
11. [Section 11: Endpoint Protection](#heading-section-11-endpoint-protection)
    
12. [Section 12: Messages & Pub-Sub](#heading-section-12-messaging-amp-pub-sub)
    
13. [Section 13: Smaller Essentials](#heading-section-13-smaller-essentials)
...
### HTTP - [Hyper Text Transfer Protocol](https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol)
...
Caching! This is a very fundamental and easy-to-understand technique to speed up performance in a system. Thus caching helps to reduce ["latency"](#storage-latency-throughput) in a system.
...
So load balancers are like traffic managers who direct traffic. And they do this to maintain [availability](#system-availability) and [throughput](#storage-latency-throughput).

When understanding where a load balancer is inserted in the system's architecture, you can see that load balancers can be thought of as [reverse proxies](#proxies). But a load balancer can be inserted in other places too - between other exchanges - for example, between your server and your database.
...
**Durability** is the promise that once the data is stored in the database, it will remain so. It will be "[persistent](#heading-section-2-storage-latency-amp-throughput)" - stored on disk and not in "memory".
...
Replication means to duplicate (make copies of, replicate) your database. You may remember that when we discussed [availability](#system-availability).
...
Let's move back to servers again for a slightly more advanced topic. We already understand the principle of [Availability](#heading-section-3-system-availability), and how redundancy is one way to increase availability. We have also walked through some practical considerations when handling the [routing of requests](#heading-section-6-load-balancing) to clusters of redundant servers.
