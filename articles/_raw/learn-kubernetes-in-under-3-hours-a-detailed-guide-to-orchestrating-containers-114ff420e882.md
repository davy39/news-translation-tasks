...
_Fig. 1. Sentiment Analysis Web App_
...
_Fig. 2. Data flow in the Sentiment Analysis WebApp_
...
Install and start the Nginx WebServer ([how to](https://www.nginx.com/resources/wiki/start/topics/tutorials/install/)). Then move the contents of the sa-frontend/build folder to [_your_nginx_installation_dir_]/**html.**

This way the generated index.html file will be accessible in [_your_nginx_installation_dir_]/html/index.html. T**his is the default file that Nginx serves**.

By default the Nginx WebServer listens on port 80. You can specify a different port by updating server.listen property in the file [_your_nginx_installation_dir_]/conf/nginx.conf.
...
_Fig. 3. React app served from Nginx_
...
_Fig. 4. Microservice Spring WebApp missing_
...
_Fig. 6. Microservice architecture completed_
...
_Fig. 7. Nginx web server with static files on a VM_
...
_Fig. 8. Nginx web server serving static files in a Container_
...
_Fig. 9. Port Mapping of Host to Container_
...
Because the port was run in the host (your computer) in port 80 it should be accessible on the localhost:80. If you do not have native docker support, you can open the application in <docker-machine ip>:80. To find your docker-machine ip ex`ecute docker-mach`ine ip
...
_Fig. 10. Microservices running in Containers_
...
_Fig. 12. Request to the API Server_
...
_Fig. 13. Microservices running in a Kubernetes Managed Cluster_
...
_Fig. 15. Load balancing between pods_
...
_Fig. 16. Kubernetes Service maintaining IP addresses_
...
_Fig. 17. Pods with labels and their manifests_
...
_Fig. 18. Load balancing with the LoadBalancer Service_
...
_Fig. 21. RollingUpdate replacing pods_
...
Most of the commands are self-explanatory, besides one detail that you had to work out yourself. Why the first revision has a **CHANGE-CAUSE** of <none> meanwhile the second revision **has a CHANGE-**CAUS_E of “kubectl.exe apply –filename=sa-frontend-deployment-gree**n**.yaml –recor_d=true“.
...
_Fig. 22. Current Application State_
...
_Fig. 23. Updated Application State_
