...
1. [Flutter is Cross-Platform](#heading-flutter-is-cross-platform-1)
2. [How Flutter is Cross-Platform](#heading-flutter-is-cross-platform-1)
3. [Flutter Has a Powerful UI Engine](#heading-flutter-has-a-powerful-ui-engine) 
4. [How Flutter Renders UIs](#heading-how-flutter-renders-uis)
5. [Flutter Has Great State Management](#heading-flutter-has-great-state-management) 
6. [Flutter Provides a Great Developer Experience](#heading-flutter-provides-a-great-developer-experience) 
7. [Flutter Has a Wonderful Developer Community](#heading-flutter-has-a-wonderful-developer-community)
8. [Summary](#please-use-flutter)
...
_Same Flutter app working on desktop, mobile, and web._

The Flutter framework gives you APIs for painting and events (through widgets). It also provides APIs for platform-specific services (through method channels). So Flutter _exposes_ _everything_ to you to build the app in any way you want it. That's how cross-platform works.

![Image](https://www.freecodecamp.org/news/content/images/2022/06/Screenshot--224--2.png)
_Adapted from https://youtu.be/l-YO9CmaSUM_
...
Behind the scenes, Flutter keeps **3 separate trees** for the UI. We normally code the outermost widget tree. In turn, the Flutter engine creates an _Element_ and a _RenderObject_ tree based on the widget tree.
...
When UI is re-rendered, Flutter always destroys and rebuilds any changed widget (because widgets are immutable). However, it only replaces a linked Element or RenderObject _if need be._
...
_The 3 UI trees, adapted from https://youtu.be/996ZgFRENMs_

![Image](https://www.freecodecamp.org/news/content/images/2022/07/widget_tree_2.png)
_The 3 UI trees with the Center widget as an example. Adapted from https://youtu.be/996ZgFRENMs_
...
Flutter leverages the JIT for development. Flutter ships a [Dart VM](https://mrale.ph/dartvm) to the platform in which the app's code is hosted. That way, when you launch the `flutter run` command, pressing `r` in that terminal ships in the changes in dart files, without recompiling the whole app. This instantaneous, yet _cross-platform_ change-reflecting feature of Flutter is called **hot-reloading**.

Flutter also has **hot-restarting**. Hot restarting is achieved by pressing `R` (uppercase instead of lowercase this time). _Hot restarting_ restarts the app entirely.
...
_Editing Flutter as an Android app in Android Studio_

![Image](https://www.freecodecamp.org/news/content/images/2022/06/demo3.gif)
_Editing Flutter as a Windows app in VS Code_
...
There is also [dartpad.dev](https://dartpad.dev), [flutlab.io](https://flutlab.io), and [flutterflow.io](https://flutterflow.io) that let you create Flutter apps online, in the browser. You will find these useful as they do not require as many computing resources as running Flutter for Android or iOS. 
...
Around Flutter there are so many developers that have extended the framework. These developers have published more than 24,000 packages on [pub.dev](https://pub.dev). Each package has at least one widget you can import to your Flutter app. 
