...
In the last post, we tackled the problem of [Machine Learning classification through the lens of dimensionality reduction](https://medium.freecodecamp.org/an-illustrative-introduction-to-fishers-linear-discriminant-9484efee15ac?source=friends_link&sk=842b291dc6304d477b278c7aa622ec23). We saw how Fisher’s Linear Discriminant can project data points from higher to smaller dimensions. The projection follows two principles.
...
_Before we begin, make sure you follow along with these_ [_Colab notebooks_](https://github.com/sthalles/logistic-regression).
...
For binary classification, the posterior probabilities are given by the sigmoid function σ applied over a linear combination of the inputs _ϕ._
...
_For binary classification, the posterior probabilities are represented by the logistic Sigmoid function._
...
_Trainable parameters for binary logistic regression._
...
To find these parameters, we usually optimize the cross-entropy error function.

The cross-entropy measures how **far** the model’s predictions are from the labels. It **increases** when the predicted values deviate from the true values and **decrease** otherwise.
...
_The gradient of the cross-entropy_

Note that to compute this derivative, we need the derivative of the sigmoid function w.r.t weights **w**. Luckily, one nice property of the sigmoid is that we can express its derivative in terms of itself.

![Image](https://www.freecodecamp.org/news/content/images/2021/02/1_dzZwMfiCdsj278OkpRFmUw.png)
_The derivative of the sigmoid function._
...
_Here ak are the activations._

Since logistic regression treats its predictions as probabilities, we need to change the way we represent our labels. Up to this point, the target/label vector is usually represented as a vector of integers. Each value representing a different class. If we want them to be equally valued probabilities, they need to be between 0 and 1. To do this, we can change their representation to one-hot-encodings.

![Image](https://www.freecodecamp.org/news/content/images/2021/02/0_DcT6ulzhWBZVFrKO.png)
_One-hot-encoding representation._

This time, for inputs with M features and K different classes, logistic regression learns MxK parameters. We can view it as a matrix.

![Image](https://www.freecodecamp.org/news/content/images/2021/02/1_I98p3Bbe7AwceWmmI2C3Gg.png)
_Trainable parameters for multiclass logistic regression._
...
The binary cross-entropy treats the targets as scalars. They take either 0 or 1. For multiclass classification, targets are represented as one-hot-encoded vectors.
...
_Closed-form solution for linear regression._
...
_Image based on [Deep Learning](https://www.deeplearningbook.org/contents/numerical.html" rel="noopener nofollow) book by Ian Goodfellow and Yoshua Bengio and Aaron Courville._

Especially in (c), depending on the magnitude of the step we take, we might get our loss function to rather increasing. To avoid this caveat, we take advantage of the information given by the second derivative.
...
Following sklearn’s based API, we can **fit** and **eval** the model:
...
First, **H** has to be a square matrix. Second, the columns of **H** need to be linearly independent. It means that for any column **_i_** of **H**, **_i_** cannot be represented as a linear combination of any other column **_j_**.

That is the same to say that the columns of **H** span the coordinate system. Or that the determinant of **H** is non-zero

Since for really large matrices these constraints are likely not to hold, we can use pseudo-inverse techniques here. That is what the function **_pinv(H)_** does on the code above.
...
_Decision boundaries for Logistic Regression using Newton’s method. Circles represent training data and crosses test instances._
...
* And can be used for supervised and unsupervised learning.
...
For more cool stuff on Machine Learning, take a look at:

* [An introduction to high-dimensional hyper-parameter tuning](https://www.freecodecamp.org/news/an-introduction-to-high-dimensional-hyper-parameter-tuning-df5c0106e5a4/)
* [An illustrative introduction to Fisher’s Linear Discriminant](https://www.freecodecamp.org/news/an-illustrative-introduction-to-fishers-linear-discriminant-9484efee15ac/)
