...
1. [What is an AI and Agentic Workflow?](#heading-what-is-an-ai-and-agentic-workflow)
...
[Langbase](http://langbase.com) is a Serverless Composable AI Developer platform with multi-agent orchestration and advanced long-term memory. It’s designed for seamless AI development and deployment. Langbase provides support to over 100+ LLMs through one API ensuring a unified developer experience, with easy model switching and optimization.
...
1. **Pipe Agents:** Pipe agents on Langbase are different from other agents. They are serverless AI agents with agentic tools that can work with any language or framework. Pipe agents are easily deployable, and with just one API they let you connect 100+ LLMs to any data to build any developer API workflow.
    
2. **Memory Agents:** Langbase memory agents (long-term memory solution) are designed to acquire, process, retain, and retrieve information seamlessly. They dynamically attach private data to any LLM, enabling context-aware responses in real time and reducing hallucinations. Memory, when connected to a pipe agent, becomes a memory agent.
    
3. **BaseAI.dev:** BaseAI is the open-source first web AI framework. With it you can start building local-first, agentic pipes, tools, and memory and deploy serverless with one command.
    
4. **AI Studio:** Langbase AI Studio provides a playground to collaborate on AI agents, memory and tools. With it you can build, collaborate, test and deploy pipe and memory (RAG) agents.
    
5. **LangUI:** LangUI is a free, open-source Tailwind library with ready-made components designed for AI and GPT projects.
    
6. **Langbase SDK:** Langbase offers a TypeScript AI SDK that simplifies development. It helps you easily integrate LLMs, create memory agents, and chain them together into pipelines—all with minimal code. It supports JavaScript, TypeScript, Node.js, Next.js, React, and more, enabling faster development with a great developer experience.
...
* **API-first platform**: Simple APIs (Pipe and memory agent APIs) for easy integration with clear documentation and community support.
    ...
    💡 Use the Pipe API to manage the pipe agents in your Langbase account. It has create, update, list and run endpoints. The Langbase Memory API lets you manage memories and documents in your Langbase account programmatically.
...
* **Customer Support Agent:** Build customer support agents/apps that can handle complex, context-aware conversations across support tickets, emails, and chats, providing accurate, efficient resolutions. Check out the customer support agents [here](https://langbase.com/docs/solutions/customer-support).
    
* **Coding Agent:** Create multi-agentic apps that assist developers by generating code snippets, debugging, and reviewing code in real time, improving productivity in software development workflows. Here’s an example coding agent [demo](https://code-alchemist.langbase.dev/).
    

### Getting Started with Langbase

* To get started with Langbase signup for free [here](https://langbase.com/signup).
...
[LlamaIndex](https://www.llamaindex.ai/) is an open-source framework built for RAG applications and agent-based systems. It provides essential tools to ingest, structure, and connect private or domain-specific data to LLMs, enabling more accurate and reliable text generation.
...
For more details, check out their [documentation](https://docs.llamaindex.ai/en/stable/#getting-started).

## AG2

[AG2](https://ag2.ai/) (formerly AutoGen) is an open-source framework for building AI agents and enabling multi-agent collaboration. AG2 provides a framework for building autonomous workflows and agent collaboration, simplifying the creation of specialized agents that can work together seamlessly.
...
For more details visit the [documentation](https://docs.ag2.ai/docs/Getting-Started).

## Braintrust

[Braintrust](https://www.braintrust.dev/) is an end-to-end platform for evaluating, improving, and deploying large language models (LLMs) with tools for prompt engineering, data management, and continuous evaluation. Designed to make building AI applications more robust and iterative, Braintrust helps you prototype rapidly with different prompts and models, evaluate performance with built-in tools, and monitor real-world interactions in real time.

### Key Features

* **Iterative experimentation**: Rapidly prototype and test prompts with different models in the integrated [playground](https://www.braintrust.dev/docs/guides/playground). You can experiment with real dataset inputs, compare responses across models (OpenAI, Anthropic, Mistral, Google, Meta, and more), and fine-tune performance in the playground.
...
* To get started [signup](https://www.braintrust.dev/signup) on Braintrust.
    
* Once you’re signed up, you’ll be asked to create an organization for free.
    
* To run your first eval, you can either use the [UI](https://www.braintrust.dev/docs/start/eval-ui) or the [starter code](https://www.braintrust.dev/docs/start/eval-sdk) available. Install the Braintrust SDK using this command:
    
    ```bash
    npm install braintrust autoevals
    ```
    
    For more details visit the [documentation](https://www.braintrust.dev/docs/start).
...
[FlowiseAI](https://flowiseai.com/) is an open source low-code tool for developers to build customized LLM orchestration flows & AI agents. With its intuitive drag-and-drop interface, Flowise makes LLM technology accessible to a wider audience, including those with little to no coding experience.
...
* To get started, install Flowise locally using NPM.
    ...
    💡 Prerequisite: ensure [NodeJS](https://nodejs.org/en/download) is installed on machine. Node `v18.15.0` or `v20` and above is supported.
    
* Start Flowise using this command and open [localhost:3000](http://localhost:3000):
...
For more details check out these getting started [steps](https://docs.flowiseai.com/getting-started).
