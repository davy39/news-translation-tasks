...
We start by creating a `sealed trait` that represents the block of our chain. The `Chain` can have two types: it can be an `EmptyChain` or a `ChainLink`. The former is our block zero (the _genesis block_), and it is implemented as a singleton (it is a `case object`), while the latter is a regular mined block.
...
As we said in the description of the actor, one of the possible reaction to a message is the creation of other actors. When this happens, the father becomes the _supervisor_ of its children. If a children fails, the supervisor can decide the action to take, may it be create a new actor, ignore the failure, or throw it up to its own supervisor. In this way the Actor System becomes a hierarchy tree, each node supervising its children. This is the way the actor model provides **fault tolerance**.
