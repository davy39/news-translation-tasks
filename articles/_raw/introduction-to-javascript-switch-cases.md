...
Then `8`. That’s a match, so the execution starts from `case 8`until the nearest `break`.
...
There are so many practical examples of switch cases, you can head over to [google.com](https://google.com/) and run a quick search for more switch cases examples.
