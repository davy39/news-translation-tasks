...
Here’s a picture from [wiki.osdev.org](https://wiki.osdev.org/Expanded_Main_Page) that shows each bit of the EFLAGS register:
...
With EAX set to 1, the CPUID will return a bitfield in EDX, which will contain the following values. Different brands may give different meaning to these (source [https://wiki.osdev.org/CPUID](https://wiki.osdev.org/CPUID))
...
More information about what information CPUID instruction can give you according to the value passed in the EAX register, can be found here: [https://gitlab.com/x86-cpuid.org/x86-cpuid-db](https://gitlab.com/x86-cpuid.org/x86-cpuid-db)
