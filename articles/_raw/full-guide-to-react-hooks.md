...
* [A bit of history about React and what hooks are for](#heading-a-bit-of-history-about-react-and-what-hooks-are-for)
    
* [Frequently used React hooks](#heading-frequently-used-react-hooks)
...
* [Some less common but still useful hooks](#heading-some-less-common-but-still-useful-hooks)
    
    * [UseCallback hook](#useCallback-hook)
        
    * [UseMemo hook](#useMemo-hook)
        
* [Custom React hooks](#heading-custom-react-hooks)
...
We can declare a ref using the useRef hook the following way: `const ref = useRef(initialValue)`. Then `reference.current` accesses the reference value, and `reference.current = newValue` updates the reference value. Pretty simple.
