...
1. [What are ARIA Attributes?](#heading-what-are-aria-attributes)
    
2. [The `alt` Attribute](#heading-the-alt-attribute)
    
3. [The `aria-label` Attribute](#heading-the-aria-label-attribute)
    
    * [Best practices for Using `aria-label`](#heading-best-practices-for-using-aria-label)
        
4. [The `aria-labelledby` Attribute](#heading-the-aria-labelledby-attribute)
    
    * [How is `aria-label` Different from `aria-labelledby`?](#heading-how-is-aria-label-different-from-aria-labelledby)
        
    * [Best Practices for Using `aria-labelledby`](#heading-best-practices-for-using-aria-labelledby)
        
5. [The `aria-describedby` Attribute](#heading-the-aria-describedby-attribute)
    
    * [Best Pracices for Using `aria-describedby`](#heading-best-practices-for-using-aria-describedby)
        
6. [The `role` Attribute](#heading-the-role-attribute)
    
    * [Common `role` Values](#heading-common-role-values)
        
    * [Best Practices for Using the `role` Attribute](#heading-best-practices-for-using-the-role-attribute)
        
7. [The `aria-controls` Attribute](#heading-the-aria-controls-attribute)
    
    * [Best Practices for Using `aria-controls`](#heading-best-practices-for-using-aria-controls)
        
8. [The `aria-selected` Attribute](#heading-the-aria-selected-attribute)
    
    * [Best Practices for Using `aria-selected`](#heading-best-practices-for-using-aria-selected)
        
9. [The `tabindex` Attribute](#heading-the-tabindex-attribute)
    
    * [Possible `tabindex` Values](#heading-possible-tabindex-values)
        
    * [Best Practices for Using `tabindex`](#heading-best-practices-for-using-tabindex)
        
10. [The `title` Attribute](#heading-the-title-attribute)
    
    * [Accessibility Concerns with `title`](#heading-accessibility-concerns-with-the-title-attribute)
        
    * [Best Practices for Using the `title` Attribute](#heading-best-practices-for-using-the-title-attribute)
        
11. [Using the `for` Attribute in `label`](#heading-using-the-for-attribute-in-label)
    
    * [Best Practices for Using the `for` Attribute](#heading-best-practices-for-using-the-for-attribute)
        
12. [The `scope` Attribute](#heading-the-scope-attribute)
    
    * [Possible `scope` Values](#heading-possible-scope-values)
        
    * [Best Practices for Using `scope`](#heading-best-practices-for-using-the-scope-attribute)
        
13. [The `aria-hidden` Attribute](#heading-the-aria-hidden-attribute)
    
    * [Best Practices for Using `aria-hidden`](#heading-best-practices-for-using-aria-hidden)
        
14. [The `inert` Attribute](#heading-the-inert-attribute)
    
    * [Best Practices for Using `inert`](#heading-best-practices-for-using-the-inert-attribute)
        
15. [The `aria-live` Attribute](#heading-the-aria-live-attribute)
    
    * [Possible Values for `aria-live`](#heading-possible-values-for-aria-live)
        
    * [Best Practices for Using `aria-live`](#heading-best-practices-for-using-aria-live)
        
16. [The `aria-roledescription` Attribute](#heading-the-aria-roledescription-attribute)
    
    * [Best Practices for Using `aria-roledescription`](#heading-best-practices-for-using-aria-roledescription)
        
17. [The `aria-atomic` Attribute](#heading-the-aria-atomic-attribute)
    
    * [Best Practices for Using `aria-atomic`](#heading-the-aria-atomic-attribute)
...
**Positive values:** Elements with positive values become focusable and are included in the tab order with their numbers determining the order in which they are focused. Elements with lower numbers are focused before elements with higher numbers.
...
**Note:** Using positive `tabindex` values can lead to a confusing and non-intuitive tab order. It is generally better to use `tabindex="0"` for elements that should be part of the natural tab order.

**Zero:** You use this to make an element focusable and include it in the natural tab order based on its position in the document. It's useful for making elements focusable that are not normally focusable (like `<div>` or `<span>`).
...
**Negative values:** You use this to remove an element from the tab order, meaning it cannot be focused using the `Tab` key. But it can still be focused programmatically (via JavaScript). It's useful for elements that should not be focusable by default but may need to be focusable under certain conditions.
...
* [ARIA examples by properties and states](https://www.w3.org/WAI/ARIA/apg/example-index/#examples_by_props_label)
