...
- to implement a `factory method` to [encapsulate the complexity](https://khalilstemmler.com/articles/typescript-value-object/) required in order to create an instance of the class
- to use an `abstract factory` in order [to create a specific type of instance of the class](https://khalilstemmler.com/wiki/abstract-factory/)
- when the property shouldn't ever change

### Scenarios where it _seems like_ it might make sense but actually leads to an [anemic domain model](https://khalilstemmler.com/wiki/anemic-domain-model/):

- to perform validation logic on atttributes for that class (use [Value Objects](https://khalilstemmler.com/articles/typescript-value-object/) instead)
...
If you enjoyed this article, you should [check out my blog](https://khalilstemmler.com). I write about **Advanced TypeScript & Node.js best practices for large-scale applications** and teach developers how to write flexible, maintainable software.
