---
title: Google Sheets Tutorial – How to Enable Multiple Selection Data Validation Using
  Apps Script
subtitle: ''
...
_Gif of Michaelengelo from Teenage Mutant Ninja Turtles_
...
_Screenshot of Google Sheet's data menu_
...
_Screenshot of right-clicking menu in Google Sheets_
...
_Screenshot of Google Sheets' data validation options menu_
...
You'll be able to create files using the plus `+` icon. For this project we need a `Code.gs` file and a `Page.html` file.

![Image](https://www.freecodecamp.org/news/content/images/2023/01/image-8.png)

## The Code.gs File

Start in the Code.gs file.
...
_Screenshot of Google Sheets' custom drop-down bar_
...
Here is the full code for the `Code.gs` file:
...
_Screenshot of our custom Google Sheet html sidebar_
...
Remember, `["purple"]` is an array of length 1. The `j` loop does not count the letters in the string _within_ the array – we are just counting the length _of_ the array.
...
_screenshot of inspected element: the input with an id_

![Image](https://www.freecodecamp.org/news/content/images/2023/01/image-12.png)
_screenshot of inspected element: the form and input elements_
...
The first button pulls the data validation from the cell. It runs the `showDialog()` function we created in `Code.gs` and builds the form of checkbox entries if there exist data validation values.
...
The second button fills the selected values into the current cell. **This is what we were after all along!** This runs the `fillCell` function in `Code.gs`.
