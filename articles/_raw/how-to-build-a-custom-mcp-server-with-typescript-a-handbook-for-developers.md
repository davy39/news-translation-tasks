...
* [What is the Model Context Protocol (MCP)?](#heading-what-is-the-model-context-protocol-mcp)
    ...
    * [What does "Protocol" mean?](#heading-what-does-protocol-mean)
        
    * [What is a "Model"?](#heading-what-is-a-model)
        
    * [What is "Context"?](#heading-what-is-context)
        
    * [Putting It All Together: What is Model Context Protocol?](#heading-putting-it-all-together-what-is-model-context-protocol)
        
* [Why MCP is Necessary](#heading-why-mcp-is-necessary)
    
    * [The MCP Connector in Action](#heading-the-mcp-connector-in-action)
        
    * [Universal Access Across Platforms](#heading-universal-access-across-platforms)
        
    * [Developers are Key](#heading-developers-are-key)
        
    * [Beyond Built-in Integrations](#heading-beyond-built-in-integrations)
        
    * [The Power of Reusability](#heading-the-power-of-reusability)
        
    * [The Burden without MCP](#heading-the-burden-without-mcp)
        
    * [A Practical GitHub Example](#heading-a-practical-github-example)
        
    * [Why MCP Matters for Developers](#heading-why-mcp-matters-for-developers)
        
* [RAG vs MCP](#heading-rag-vs-mcp)
    
    * [What is RAG](#heading-what-is-rag)
        
    * [RAG: The “Mise en Place” Prep](#heading-rag-the-mise-en-place-prep)
        
    * [MCP: The Rolling Assistant Cart](#heading-mcp-the-rolling-assistant-cart)
        
    * [Bringing It All Together](#heading-bringing-it-all-together)
...
* [How AI Apps Talk to MCP Servers — A Practical Example](#heading-how-ai-apps-talk-to-mcp-servers-a-practical-example)
    ...
    * [Scenario: Asking Claude About Your Schedule](#heading-scenario-asking-claude-about-your-schedule)
        
    * [Discovering the Right MCP Server](#heading-discovering-the-right-mcp-server)
        
    * [MCP Server Fetches and Returns the Data](#heading-mcp-server-fetches-and-returns-the-data)
        
    * [Model Converts Structured Data into Natural Language](#heading-model-converts-structured-data-into-natural-language)
        
    * [Under the Hood: Abstracting the Complexity](#heading-under-the-hood-abstracting-the-complexity)
        
    * [Mirroring Standard Web App Workflows](#heading-mirroring-standard-web-app-workflows)
...
* [The MCP Architecture — How It All Fits Together](#heading-the-mcp-architecture-how-it-all-fits-together)
    ...
    * [1\. MCP Host](#heading-1-mcp-host)
        
    * [2\. MCP Client](#heading-2-mcp-client)
        
    * [3\. MCP Server](#heading-3-mcp-server)
        
    * [4\. Data Sources – Local or Remote](#heading-4-data-sources-local-or-remote)
        
* [Opportunities for Web Developers](#heading-opportunities-for-web-developers)
    
    * [SDK Options: Pick Your Language](#heading-sdk-options-pick-your-language)
        
    * [From Backend Service to AI-Enabled Developer](#heading-from-backend-service-to-ai-enabled-developer)
...
![Model Context Protocol - Structured Communication](https://cdn.hashnode.com/res/hashnode/image/upload/v1750671107217/c0c8c59d-5c2c-451e-9649-0889cae36c05.png align="center")
...
![How MCP Server communicates with Local Data and generate Response](https://cdn.hashnode.com/res/hashnode/image/upload/v1750678334371/7244b0ee-f015-48f2-9433-2cf29a194b06.jpeg align="center")
...
![How MCP feeds context to Cursor Editor](https://cdn.hashnode.com/res/hashnode/image/upload/v1750671538182/0955d5b2-48ce-4e12-877b-0eeb7d2ddad2.png align="center")
...
Let's compare that with what used to happen before. For example, today, ChatGPT lets you do web searches – you can just ask it to find something online, and it'll fetch the result. But this feature is only there because [OpenAI](https://openai.com), the makers of ChatGPT, built it into the app.
...
* Read through the [GitHub API documentation](https://docs.github.com/en/rest)
...
And boom – it works! You don't need to write any custom integration logic. That's the magic of MCP. And here's the best part: GitHub already released their [official MCP server](https://github.com/github/github-mcp-server). You can use it right now.
...
Now that we’ve covered MCP, let’s look at another popular approach called [RAG](https://en.wikipedia.org/wiki/Retrieval-augmented_generation) and see how they differ. Many AI builders start by using RAG to ground their models in static knowledge, so it’s helpful to see how that approach compares to streaming live data with MCP.
...
Now let's check out the [official MCP documentation](https://modelcontextprotocol.io/introduction). It’ll help things start to feel much clearer. So what does the definition say?

> MCP is an open protocol that standardizes how applications provide context to LLMs. ([Source: MCP Documentation](https://modelcontextprotocol.io/introduction))

Yep – exactly what we've already talked about. And then comes a brilliant line from the docs:

> Think of MCP like a USB-C port for AI applications. ([Source: MCP Documentation](https://modelcontextprotocol.io/introduction))
...
![USB-C universal connector analogy](https://cdn.hashnode.com/res/hashnode/image/upload/v1750678229073/7df559c2-48bf-4a39-8415-31276609ca98.jpeg align="center")
...
![Claude vague answer without MCP](https://cdn.hashnode.com/res/hashnode/image/upload/v1750671970459/c1cecbfc-1012-4580-ba95-ccfeb0854129.png align="center")
...
![Claude positive answer with MCP Server](https://cdn.hashnode.com/res/hashnode/image/upload/v1750672087724/efa2637d-1a35-4f7a-b8f6-2d9183580c23.png align="center")
...
![MCP local transport process similar to REST API](https://cdn.hashnode.com/res/hashnode/image/upload/v1750678140969/9f22b8a9-9c56-45cc-95ae-5b946e379133.jpeg align="center")
...
Let's now take a look at the MCP architecture. Once you see the structure, everything we've discussed will make even more sense. Here's what the diagram shows (the diagram was collected from the [MCP documentation](https://modelcontextprotocol.io/introduction)):

[![MCP architecture diagram from the docs](https://cdn.hashnode.com/res/hashnode/image/upload/v1750672357388/9d07577d-447e-4a13-b205-1d0e72b5d18a.png align="center")](https://modelcontextprotocol.io/introduction)
...
![MCP components talk to each other](https://cdn.hashnode.com/res/hashnode/image/upload/v1750672511389/bd68a3f8-7bf3-4fcc-b631-c99a4ea05667.png align="center")
...
My project is a Node.js project. I've created a `server.js` file, and honestly, that's the only file I've used in this project. All the code is written inside that one file.
...
Run the install command (mentioned in the [docs](https://github.com/modelcontextprotocol/typescript-sdk)).
...
First we need to install the `googleapis` package with the below command in the terminal:
...
Now let’s write the function `getMyCalendarDataByDate` and call the `google.calendar` method according to [Google Calendar API](https://developers.google.com/workspace/calendar/api/quickstart/nodejs). This `google.calendar()` method receives an object as parameter and we need to mention `version` and `auth` here. `version` is simply the Calendar API version number and `auth` is the Google API Public Key for authentication.
...
Here, you can see that I’ve used the Google API public key as an environment variable. So, we’ll create a `.env` file in the root of the project directory and add the following inside that file:

```plaintext
GOOGLE_PUBLIC_API_KEY=WRITE_YOUR_GOOGLE_PUBLIC_API_KEY
```

Don’t forget to replace with your own Google Public API Key. You can grab your public key from [Google Cloud Console](https://cloud.google.com/cloud-console).
...
Here you can see, I have mentioned my Public Calendar ID using another environment variable called `CALENDAR_ID`. So go back to your .env file and set the new environment variable:
...
* Go to [Google Calendar](https://calendar.google.com/) on your computer.
...
![How to connect MCP Server in cursor](https://cdn.hashnode.com/res/hashnode/image/upload/v1750673140780/14ac470e-38e7-4cf4-bef8-823fc155c015.png align="center")
...
![How to connect MCP Server in Cursor](https://cdn.hashnode.com/res/hashnode/image/upload/v1750673187700/9a07465a-0e7b-4bb7-9523-286dcf38373a.png align="center")
...
You can find all the source code from this handbook in [this GitHub repository](https://github.com/logicbaselabs/mcp-tutorial). If it helped you in any way, consider giving it a star to show your support!

Also, if you found the handbook valuable, feel free to share it with others who might benefit from it. I’d really appreciate your thoughts – mention me on X [@sumit\_analyzen](https://x.com/sumit_analyzen), watch my [coding tutorials](https://youtube.com/@logicBaseLabs), or simply [connect with me on LinkedIn](https://www.linkedin.com/in/sumitanalyzen).
