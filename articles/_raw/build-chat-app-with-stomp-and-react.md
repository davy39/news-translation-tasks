...
To use STOMP over WebSocket, you can use [@stomp/stompjs](https://www.npmjs.com/package/@stomp/stompjs), but that has tricky callbacks and a complicated API that caters to more specialized use cases. Luckily, there’s also the lesser-known [@stompjs/rx-stomp](https://www.npmjs.com/package/@stomp/rx-stomp) which provides a nice interface via [RxJS](https://www.npmjs.com/package/rxjs) observables. Observables aren't exclusive to Angular, and they fit quite well with how React works. It's a neat interface when composing complex workflows and pipelines with many different message sources.

The tutorial follows a somewhat similar path as the initial version in [Angular](https://stomp-js.github.io/guide/rx-stomp/rx-stomp-with-angular.html), but the component structure and code style are tuned towards the functional style of React.
...
* [Starter STOMP Server with RabbitMQ](#heading-starter-stomp-server-with-rabbitmq)
    
* [Starter React Template](#heading-starter-react-template)
    
* [How to Install RxStomp](#heading-how-to-install-rxstomp)
    
* [How to Manage Connection and Disconnection with the STOMP Server](#heading-how-to-manage-connection-and-disconnection-with-the-stomp-server)
    
* [How to Monitor the Connection Status](#heading-how-to-monitor-the-connection-status)
    
* [How to Send Messages](#heading-how-to-send-messages)
    
* [How to Receive Messages](#heading-how-to-receive-messages)
...
If you’d like to use RabbitMQ too (not strictly required), here’s are [installation guides for different operating systems](https://www.rabbitmq.com/docs/download). To add the extension, you’ll need to run:

```bash
$ rabbitmq-plugins enable rabbitmq_web_stomp
```

If you’re able to use `Docker`, a Docker file similar to [this](https://github.com/harsh183/rabbitmq-intro/blob/master/code_examples/Dockerfile) will set everything needed for the tutorial:
...
For this tutorial, we'll use [Vite](https://vite.dev/guide/)'s `react-ts` template. The central part of our application will be in the `App` component, and we'll create child components for other specific STOMP functionality.
...
For a better dev experience, we logged all messages with timestamps to a local console and set low timer frequencies. Your configuration should be quite different for your production application, so check out the [RxStompConfig docs](https://stomp-js.github.io/api-docs/latest/classes/RxStompConfig.html) for all the options available.
...
RxStomp has a [RxStompState enum](https://stomp-js.github.io/api-docs/latest/miscellaneous/enumerations.html#RxStompState) that represents possible connection states with our broker. Our next goal is to display the connection status in our UI.
...
You can find the final code on Gitlab: [https://gitlab.com/harsh183/rxstomp-react-tutorial](https://gitlab.com/harsh183/rxstomp-react-tutorial). Feel free to use it as a starter template too and report any issues that may come up.
