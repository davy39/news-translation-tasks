...
Here we want to take out the messages only from the bot, hence the `filter(r => r.type === ‘bo`t’) operator. T`he retryWhen(err$ =&`gt; …) operator automatically re-connects to the WebSocket after it has been disconnected.
...
When the message gets through all the operators, we push it into our array of messages `(msg) => this.pushMsg(m`sg).
