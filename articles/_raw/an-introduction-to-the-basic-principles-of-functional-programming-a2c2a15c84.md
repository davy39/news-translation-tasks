...
This article uses Clojure as a programming language example to explain Functional Programming. If you are not comfortable with a LISP-type-of-language, I also published the same post in JavaScript. Take a look: [**Functional Programming Principles in Javascript**](https://medium.freecodecamp.org/functional-programming-principles-in-javascript-1b8fc6c3563f)
...
_“water drop” by [Unsplash](https://unsplash.com/@martinmohan?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title="">Mohan Murugesan</a> on <a href="https://unsplash.com?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title=")_
...
Now imagine some mathematicians argue that the `PI` value is actually `[42](https://en.wikipedia.org/wiki/Phrases_from_The_Hitchhiker%27s_Guide_to_the_Galaxy#Answer_to_the_Ultimate_Question_of_Life,_the_Universe,_and_Everything_(42))`and change the value of the global object.

Our impure function will now result in `10 * 10 * 42` = `4200`. For the same parameter (`radius = 10`), we have a different result. Let's fix it!

TA-DA ?! Now we’ll always pass theP`I` value as a parameter to the function. So now we are just accessing parameters passed to the function. No e`xternal object.`
...
_“Change neon light signage” by [Unsplash](https://unsplash.com/@rossf?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title="">Ross Findon</a> on <a href="https://unsplash.com?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title=")_
...
So here we have the `sum` function that receives a vector of numerical values. The `recur` jumps back into the `loop` until we get the vector empty ([our recursion `base case`](https://en.wikipedia.org/wiki/Recursion_(computer_science)#Recursive_functions_and_algorithms)). For each "iteration" we will add the value to the `total` accumulator.
...
_“person holding eyeglasses” by [Unsplash](https://unsplash.com/@joshcala?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title="">Josh Calabrese</a> on <a href="https://unsplash.com?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title=")_
...
_“first-class” by [Unsplash](https://unsplash.com/@andrewtneel?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title="">Andrew Neel</a> on <a href="https://unsplash.com?utm_source=medium&amp;utm_medium=referral" rel="noopener" target="_blank" title=")_
...
`#(> x` %) is just a anonymous function that receiv`e`s x and compares it with each element in the collectio`n`. % represents the parameter of the anonymous function — in this case the current element inside t`he fil`ter.
...
* we have the anonymous function `#(< 21 (:age` %)). Remember that t`h`e % represents the current element from the collection? Well, the element of the collection is a people map. If we `do (:age {:name "TK" :age 2`6}), it returns the age valu`e,` 26 in this case.
...
> The `_map_` method transforms a collection by applying a function to all of its elements and building a new collection from the returned values.
