---
title: How to Provision a Nexus Sonatype OSS on an Orange PI 5 with Ansible
subtitle: ''
...
Nexus 3 OSS is an [Open Source](https://github.com/sonatype/nexus-public) [artifact repository manager](https://www.sonatype.com/products/repository-oss) that can handle multiple formats like container images, Python PIP, Java jar, and many others.
...
Nexus 3 will run on an [Orange PI 5 computer with 8 GB or RAM](http://www.orangepi.org/html/hardWare/computerAndMicrocontrollers/details/Orange-Pi-5.html), but this provisioning can be done on any machine with the [minimum requirements](https://help.sonatype.com/repomanager3/product-information/sonatype-nexus-repository-system-requirements). Part of the setup will consist of setting a proxy for [PyPI.org](https://pypi.org/), for the machines listed on my [inventory](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/inventories/home/hosts.yaml) file.
...
* There are two roles: ‘nexus’ and ‘clients’. The nexus role is used to setup the artifact management software, while the client role sets up the [pip](https://docs.python.org/3/installing/index.html) settings on every machine.
...
* The file ‘[site.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/site.yaml)’ Orchestrates the role execution:
...
The file [main.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/main.yaml) describes the order and purpose of each installation task for the Nexus role:
...
1. [third\_party.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/third_party.yaml): In here we install the OpenJDK8 (Nexus 3 is written in Java) and logrotate to take care of the stale logs.
    
2. [pre\_install.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/pre_install.yaml): A lot happens here, like creating required directories for nexus, dedicated non-privileged user that will run the process.
    
3. [download.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/download.yaml): As the name says, we get a fresh version of the Nexus 3 OSS software and make sure it has the right checksum. We don’t want to install malware from the Internet.
    

Then come the tasks that fall into the “customized installation group”:

1. [install.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/install.yaml): Unpack the software, prepare the systemd unit to start it automatically, setup JVM settings for Nexus, and deploy the logrotate configuration.
    
2. [post\_install.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/post_install.yaml): Exciting stuff happens here – the software is installed, and we run it for the first time. We also change the default password [using the REST API](https://help.sonatype.com/repomanager3/integrations/rest-and-integration-api), so we can move to the customization stage.
    
3. [user.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/user.yaml): Here we prepare to provide our end users with proper access to the services offered by Nexus. We do this using a combination of the REST-API and Ansible client code:
...
The last file on the Nexus 3 role is ‘[repositories.yaml](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/nexus/tasks/repositories.yaml)’. In here we go through the following steps:
...
The clients role is much simpler and only requires deploying a [template for pip.conf](https://tutorials.kodegeek.com/Nexus3OnOrangePI/ansible/roles/clients/templates/pip.conf.j2) with enough information to force the search on our new repository:
...
I recommend spending some time and reading the [Nexus 3 book](https://help.sonatype.com/repomanager3) to get yourself familiar with the features this tool can offer.
...
Nexus 3 *has lots* of configurable settings. We covered only the surface here. While preparing this article I found '[ThoTeam Nexus3-oss repository](https://github.com/ansible-ThoTeam/nexus3-oss)' with a very complete and up-to-date playbook, but it was way more complex than anything I required for my home lab.

[Archiva](https://archiva.apache.org/) is another Open Source artifact manager, it is more limited in functionality but also simpler to setup.
