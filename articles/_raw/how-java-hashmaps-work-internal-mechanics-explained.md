...
The `HashMap` is fundamentally based on a hash table implementation, and its performance depends on two key parameters: initial capacity and load factor. The [original javadocs](https://docs.oracle.com/javase/8/docs/api/java/util/Hashtable.html) of the Hash table class define these two parameters as follows:
