---
title: Learn Key System Design Principles Behind High-Traffic Platforms Like Gaming
  and Job Discovery
subtitle: ''
...
1. [Introduction: What is System Design and Why Scale Matters](#heading-introduction-what-is-system-design-and-why-scale-matters)
    
2. [Approaches to System Design](#heading-approaches-to-system-design)
    
    * [1\. Bottom-Up Approach](#heading-1-bottom-up-approach)
        
    * [2\. Top-Down Approach](#heading-2-top-down-approach)
        
    * [3\. Hybrid Design](#heading-3-hybrid-design)
        
3. [Important Concepts in System Design](#heading-important-concepts-in-system-design)
    
    * [Full Stack Web Application Components](#heading-full-stack-web-application-components)
        
    * [How Computers Talk to Each Other (The Internet)](#heading-how-computers-talk-to-each-other-the-internet)
        
    * [The Problem of Growth](#heading-the-problem-of-growth)
        
    * [Speeding Up Your Website with Caching and CDNs](#heading-speeding-up-your-website-with-caching-and-cdns)
        
    * [Building Your Application: Monolith vs. Microservices](#heading-building-your-application-monolith-vs-microservices)
        
    * [The APIs](#heading-the-apis)
        
    * [Handling Real-Time Data](#heading-handling-real-time-data)
        
    * [Databases](#heading-databases)
        
    * [Understanding the CAP Theorem](#heading-understanding-the-cap-theorem)
        
    * [Rate Limiting and Monitoring](#heading-rate-limiting-and-monitoring)
        
4. [Case Studies: Scaling in the Real World](#heading-case-studies-scaling-in-the-real-world)
    
    * [Case Study 1: Scaling a Job Search Application](#heading-case-study-1-scaling-a-job-search-application)
        
    * [Case Study 2: Scaling an Online Gaming Application](#heading-case-study-2-scaling-an-online-gaming-application)
        
5. [Q&A](#heading-qampa)
    
6. [Final Notes](#heading-final-notes)
...
![full stack web overview](https://cdn.hashnode.com/res/hashnode/image/upload/v1755603745131/1114a65f-ea7c-4e3f-aca6-4821c8ca683a.png align="left")
...
When you type a website's URL into the browser – and this site could be a simple portfolio site or a full stack app – how does your computer know where to send the request? It uses the [**Domain Name System**](https://www.freecodecamp.org/news/what-is-dns/) **(DNS)**. The DNS is like a phonebook for the internet – it translates a human-readable website name, like "example.com," into a unique numeric IP address that computers can understand.

Once your computer has the IP address, it uses **communication protocols** to send and receive data. One of the most important protocols is [**TCP**](https://www.freecodecamp.org/news/tcp-vs-udp/). It breaks data into small, numbered packets. If a packet gets lost or arrives out of order, TCP ensures it's resent and reassembled correctly, making it a very reliable way to send data.

On top of TCP, we use higher-level protocols like [**HTTP**](https://www.freecodecamp.org/news/what-is-http/). This is an application-level protocol that's easier for developers to use. It's the language your browser speaks to the server.
...
To fix this, you can use a [**Content Delivery Network (CDN)**](https://www.freecodecamp.org/news/how-cdns-improve-performance-in-front-end-projects/). A CDN is a network of servers around the world that store copies of your website's static files – like images, videos, and text files. When a user requests one of these files, the CDN serves it from the closest server, making the website load much faster for them.
...
You can read more about the [difference between CDNs and caching here](https://www.freecodecamp.org/news/caching-vs-content-delivery-network/).

### Building Your Application: Monolith vs. Microservices

As your website grows, its code can become a tangled mess. You might start with a **monolith**, where all the features (like player stats and live scores, in our example) are built into a single, large program. [A monolith is easier to start with](https://www.freecodecamp.org/news/microservices-vs-monoliths-explained/), but it can be hard to manage and update.

A better approach for a large-scale application is to use a [**microservice architecture**](https://www.freecodecamp.org/news/the-microservices-book-build-and-manage-services-in-the-cloud/). This means breaking your application into smaller, independent services, each with a specific job. For example, one service could handle player stats and another could handle live scores. This makes your code more organised and easier to update, because a change in one service won't affect the others.

With microservices, you need an [**API Gateway**](https://www.freecodecamp.org/news/what-are-api-gateways/). This acts as a single entry point for all user requests, directing them to the correct microservice behind the scenes. It also handles security and other common tasks.
...
Another style is [**GraphQL**](https://www.freecodecamp.org/news/building-consuming-and-documenting-a-graphql-api/). Instead of getting all the data a REST API provides, GraphQL lets the client ask for only the specific data it needs, which can make things faster and more efficient.
...
For server-to-server communication, [**gRPC**](https://www.freecodecamp.org/news/what-is-grpc-protocol-buffers-stream-architecture/) is often used. It's known for being very fast because it uses a more efficient data format called Protocol Buffers instead of JSON.
...
That's where [**WebSockets**](https://www.freecodecamp.org/news/learn-websockets-socket-io/) come in. Unlike HTTP, which is a one-and-done request-and-response model, a WebSocket creates a continuous, two-way connection between the client and server. This allows the server to send updates to the user as soon as they happen, creating a real-time experience.

When microservices need to communicate without being directly connected, they can use [**message queues**](https://www.freecodecamp.org/news/how-message-queues-make-distributed-systems-more-reliable/). A service sends a message to the queue, and another service picks it up when it's ready. This helps to decouple the services, so they don't have to worry about the other service being available at that exact moment.
...
Many applications rely on [**relational databases (SQL)**](https://www.freecodecamp.org/news/learn-relational-database-basics-key-concepts-for-beginners/), which store data in structured tables with rows and columns and are great for handling structured information. But for applications requiring high flexibility or handling massive unstructured datasets, [**NoSQL databases**](https://www.freecodecamp.org/news/learn-nosql-in-3-hours/) (like MongoDB or Cassandra) are often chosen. These databases don't follow the strict rules of SQL and are better for handling massive amounts of data.

They follow [ACID properties](https://www.freecodecamp.org/news/acid-databases-explained/):
...
[To incorporate it](https://www.freecodecamp.org/news/implement-api-rate-limiting-in-strapi/), you can implement limits at the API gateway, reverse proxy (like NGINX), or within your service logic itself. Many cloud providers (AWS API Gateway, GCP Endpoints) also have built-in support.

#### What is Monitoring?

[Monitoring](https://www.freecodecamp.org/news/the-front-end-monitoring-handbook/) is the practice of collecting metrics, logs, and traces from your system to understand its health in real time. Typical signals include:
...
To work it into your system, you can use observability stacks like [Prometheus + Grafana](https://www.freecodecamp.org/news/kubernetes-cluster-observability-with-prometheus-and-grafana-on-aws/), or managed solutions like Datadog, New Relic, or CloudWatch. You can also set alerts for threshold breaches (for example, 5% error rate spike).
...
We’ll look at an example site called [Upstaff](https://upstaff.com/). It’s a platform that focuses on hiring AI engineers as its core service (although it services other job profiles as well). At its core, it handles structured information – things like user profiles, job postings, and applications.
...
![system design job portal flowchart](https://cdn.hashnode.com/res/hashnode/image/upload/v1754281195211/570680c1-2813-43b0-87a1-48a817ab6c9a.png align="left")
...
![system design game website flowchart](https://cdn.hashnode.com/res/hashnode/image/upload/v1754287625776/546f3949-efa1-4a57-9906-ceb3f8a62f63.png align="left")
...
In my last [article](https://www.freecodecamp.org/news/become-a-full-stack-developer-and-get-a-job), I shared all the resources that helped me learn system design.

System design is crucial for building reliable, high-performance applications. I explored the following resources:

* [**Free GitHub system design repositor**](https://github.com/donnemartin/system-design-primer)**y**
    
* [**Another free GitHub system design repositor**](https://github.com/ByteByteGoHq/system-design-101)**y**
    
* [Free System Design Handbook](https://www.systemdesignhandbook.com/system-design-interview-handbook/)
    
* [Free Hands-on System Design Learning Platform](https://grokkingthesystemdesign.com/intro-to-system-design/)
    
* [Free Intro to System Design Blog](https://www.educative.io/guide/system-design)
