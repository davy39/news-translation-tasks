...
This tutorial focuses on creating a set of REST APIs for a Todo application. Keep in mind that I did not touch on the database here – I will cover that [in another article](https://www.freecodecamp.org/news/how-to-use-mysql-in-deno-oak/).

At any point if you feel lost or want to check a reference, here is the entire source code of this tutorial: **[Chapter 1: Oak](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak).**

![Image](https://www.freecodecamp.org/news/content/images/2021/04/image-171.png)
_Photo by [Unsplash](https://unsplash.com/@bernardtheclerk?utm_source=ghost&amp;utm_medium=referral&amp;utm_campaign=api-credit">Bernard de Clerk</a> / <a href="https://unsplash.com/?utm_source=ghost&amp;utm_medium=referral&amp;utm_campaign=api-credit)_
...
But if you are using a different operating system, just head over to [**deno.land installation**](https://deno.land/#installation)**.** They have a lot of ways you can easily install it on your machine.
...
_running command "deno --version" to see which version of deno is installed_
...
Before you move on, here is the entire source code of this tutorial: **[Chapter 1: Oak](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak).**
...
If you want to learn more about different types of types (pun intended) check out this very easy and basic guide on [**Basic types by Typescript**](https://www.typescriptlang.org/docs/handbook/basic-types.html). Just give it a quick glance for 2-3 minutes and head back here.
...
`router.get` takes 2 parameters. The first is route which we have set to `/` and the second is function. The function itself takes an argument which is an object. What I am doing here is [destructuring](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment) the object and getting only `response`.
...
`any` helps you avoid type checking in TypeScript. You can read more about it [here](https://www.typescriptlang.org/docs/handbook/basic-types.html#any).
...
_Result of running localhost:8080 on your browser_
...
_Cool, now we have a colourful console._

If you get stuck anywhere you can simply go to the source code of this tutorial [**here**](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak).
...
If you got stuck anywhere while following this, simple head over to the source code of this tutorial **[here](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak).**
...
Hopefully this example gives you a bit more insight into interfaces. If you want more detailed information check out the docs on [interfaces here](https://www.typescriptlang.org/docs/handbook/interfaces.html).
...
If you want to learn more about `interfaces` in TypeScript check out this amazing detailed documentation on interfaces **[here](https://www.typescriptlang.org/docs/handbook/interfaces.html).**
...
_The Rock appreciates all the effort you are doing_
...
More on response statuses [**here**](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status)**.**
...
Once your server is running, you can access the `GET /todos` API. I am using `postman` which is a Google Chrome extension and can be downloaded [here](https://chrome.google.com/webstore/detail/postman/fhbjgbiflinjbdggehcddcbncdddomop//%40).

You can use whatever rest client you like. I like using `postman` because I think it is very easy.

In Postman, open up a new tab. Set the request to type `GET` and in the `URL` bar type `http://localhost:8080/todos`. Hit `Send` and this is what you see:

![Image](https://www.freecodecamp.org/news/content/images/2020/05/Screenshot-2020-05-29-at-02.01.11.png)
_GET /todos API response_

Cool! 1 API done, 4 more to go. ??

If you feel stuck anywhere just have sneak peak at the source code directly [**here**](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak)**.**
...
* `import { v4 } from `[https://deno.land/std/uuid/mod.ts](https://deno.land/std/uuid/mod.ts)`;` this will be used to create a new unique one for the todo being created
...
In my postman, I open up a new tab. Set the request to `POST` type and in the `URL` bar type `http://localhost:8080/todos`. Then hit `Send` and this is what you see:

![Image](https://www.freecodecamp.org/news/content/images/2020/05/Screenshot-2020-05-29-at-02.24.00.png)
_I send an empty request and get a 400 status error code along with an error message_
...
_Awesome, POST /todos with body content { todo: "eat a lamma" } is success &amp; we can see content appended to our current todo list_
...
We then `_todos.find_((_t_)` => _t.id_ === _params.id_); use [Array.find()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/find) to find the `todo` with the id provided in `params.id`. If it matches we get a `Todo` with shape `todo`, otherwise `undefined`.
...
In my postman, I open up a new tab. Set the request to `GET` type and in the `URL` bar type `http://localhost:8080/todos/:id`, then hit `Send`.
...
_404 status, no record found case_

![Image](https://www.freecodecamp.org/news/content/images/2020/05/Screenshot-2020-05-29-at-02.41.36.png)
_Provided it a known ID and it returned the todo associated with that ID along with status 200_

If you need to reference the original source code of this tutorial go [**here**](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak).
...
Open up a new tab in Postman. Set the request to `PUT` and in the `URL` bar type in `http://localhost:8080/todos/:id`, then hit `Send`:
...
_404 status returned and no todo found error message given_

![Image](https://www.freecodecamp.org/news/content/images/2020/05/Screenshot-2020-05-29-at-03.00.21.png)
_Provided a known ID, updated todo content in body. It returned the updated todo along with all the other todos_
...
Open up a new tab in Postman. This time set the request to `DELETE` and in the `URL` bar type `http://localhost:8080/todos/:id` and hit `Send`.
...
**Reminder:** If you get stuck anywhere you can use the [source code here](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak).
...
_Execution of order of logging middleware when GET /todos API is called._
...
_Sharing the image above again for revision._
...
_API being logged in our console_

This is it – we are done.

If you still feel stuck, take a look at the entire source code for this tutorial here: [github.com/adeelibr/deno-playground/tree/master/chapter_1:oak](https://github.com/adeelibr/deno-playground/tree/master/chapter_1:oak)

I hope that you found this article useful and that it was able to help you learn something today.

If you liked it, please do share it on social media. If you want to have a discussion about it, reach out to me on [Twitter](https://twitter.com/adeelibr).
