...
_by [Sachin Malhotra](https://medium.com/@sachinmalhotra) and [Divya Godayal](https://medium.com/@divyagodayal)_
...
In case you’ve forgotten the [problem](https://medium.freecodecamp.org/an-introduction-to-part-of-speech-tagging-and-the-hidden-markov-model-953d45338f24) we were trying to tackle in the previous article, let us revise it for you.
...
In case any of this seems like Greek to you, go read the [previous article](https://medium.com/@divyagodayal/part-of-speech-tagging-hmms-part-1-953d45338f24) to brush up on the Markov Chain Model, Hidden Markov Models, and Part of Speech Tagging.

![Image](https://cdn-media-1.freecodecamp.org/images/RIOlv0lcV03Ol30xkYD-rr0vbLMrMEdJreAe)
_The state diagram that Peter’s mom gave you before leaving._

In that [previous article](https://medium.com/@divyagodayal/part-of-speech-tagging-hmms-part-1-953d45338f24), we had briefly modeled the problem of Part of Speech tagging using the Hidden Markov Model.
...
This here is the conditional model to solve this generic problem given the training data. Another approach that is mostly adopted in machine learning and natural language processing is to use a **generative model_._**
...
2. second, the example x has been generated from the distribution p(x|y). The model p(x|y) can be interpreted as a **“channel”** which takes a label y as its input, and corrupts it to produce x as its output.

### Generative Part of Speech Tagging Model

Let us assume a finite set of words V and a finite sequence of tags K. Then the set S will be the set of all sequence, tags pairs `<x1, x2, x3 ... xn, y1, y2, y3, ...,` yn> such that `n >` 0 ∀x `∊ V a`nd ∀y ∊ K .
...
1. How exactly do we define the generative model probability `p(<x1, x2, x3 ... xn, y1, y2, y3, ..., y`n>)
...
_Observations and States over time for the baby sleeping problem_
...
_Observations and States over time for the POS tagging problem_
...
_Transition probability and Emission probability for a Trigram HMM_
...
1. **c(u, v, s)** represents the trigram count of states u, v and s. Meaning it represents the number of times the three states u, v and s occurred together in that order in the training corpus.
2. **c(u, v)** following along similar lines as that of the trigram count, this is the bigram count of states u and v given the training corpus.
3. **c(s → x)** is the number of times in the training set that the state s and observation x are paired with each other. And finally,
4. **c(s)** is the prior probability of an observation being labelled as the state s.
...
_Calculations for Awake appearing after Awake_
...
_Calculations for observing ‘Quiet’ when the state is ‘Awake’_
...
definition. It then uses the identity described before to calculate the highest probability for any sequence.
...
#### Calculations for the Part of Speech Tagging Problem
...
_All combinations of sequence paths_
...
1. `c(u, v, s)` is 0
2. `c(u, v)` is 0
3. We get an unknown word in the test sentence, and we don’t have any training tags associated with it.
...
P(thing|gave, the) > P(think|gave, the) ideally, but uniform distribution using Laplace smoothing will not consider this.
...
To read more on these different types of smoothing techniques in more detail, refer to [this](https://nlp.stanford.edu/~wcmac/papers/20050421-smoothing-tutorial.pdf) tutorial. Which smoothing technique to choose highly depends upon the type of application at hand, the type of data being considered, and also on the size of the data set.
...
[_HMM-POS-Tagger — An HMM based Part of Speech Tagger implementation using Laplace Smoothing and Trigram HMMs_github.com](https://github.com/DivyaGodayal/HMM-POS-Tagger)
...
[_HMM-Part-of-Speech-Tagger — An HMM based Part of Speech Tagger_github.com](https://github.com/edorado93/HMM-Part-of-Speech-Tagger)
...
A lot of snapshots of formulas and calculations in the two articles are derived from [here](http://1. http://www.cs.columbia.edu/~mcollins/courses/nlp2011/notes/hmms.pdf).
