---
title: The Nine Most Common Mistakes Developers Make in JavaScript (and How to Fix
  Them)
subtitle: ''
author: freeCodeCamp
co_authors: []
...
Like its name implies, the [assignment operator](https://www.w3resource.com/javascript/operators/assignment-operator.php#:~:text=Assignment%20Operators,value%20of%20its%20right%20operand.&text=That%20is%2C%20a%20%3D%20b%20assigns,shown%20in%20the%20following%20table.)(=) is used to assign values to variables. Developers often confuse it with the equality operator.
...
Callbacks are one way that JavaScript handles asynchronous operations. Promises and async/await, however, are preferable methods for handling asynchronous operations because multiple callbacks lead to [callback hell](http://callbackhell.com/).
...
`this​` is a commonly [misunderstood concept](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this) in JavaScript. To use `this`​ in JavaScript, you really need to understand how it works because it operates a bit differently compared to other languages.
...
Sometimes, while working with JavaScript, developers may want to take advantage of the `localStorage` for saving values. But a common mistake is trying to save [arrays and objects](https://www.tutorialspoint.com/javascript/javascript_arrays_object.htm) as-is in the `localStorage`. `localStorage` only accepts strings.
...
Setting [default values](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Default_parameters) in dynamic variables is a very good practice for preventing unexpected errors. Here's an example of a common mistake:​​
...
Yes, developers still make this mistake. Naming is hard, but developers really have no choice. Comments are good practice in programming, and so is naming [variables](https://en.wikipedia.org/wiki/Variable_(computer_science)).
...
It is common practice to check [boolean values](https://www.w3schools.com/js/js_booleans.asp) as seen in the above code. While this is okay, errors set in when testing some values.
