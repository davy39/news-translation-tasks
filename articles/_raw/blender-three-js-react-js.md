---
title: How to Implement a Blender Model in a React.js Application using Three.js
subtitle: ''
author: Matthes B.
co_authors: []
series: null
...
1. [💭 What are Three.js and Blender?](#heading-what-are-threejs-and-blender)
2. [🔧 How to Set Up React.js with Three.js](#heading-how-to-set-up-reactjs-with-threejs)
3. [**🔨** How to Create a Blender Model](#heading-how-to-create-a-blender-model)
4. [**✏️** Texture Baking for Procedural Materials](#heading-texture-baking-for-procedural-materials)
...
_**gltf Tools** extension in Visual Studio Code_
...
_Blender scene with two spheres and one plane in the **Layout** tab_
...
_Adding animations to all three objects in the **Animation** tab_
...
_Material of one sphere where we just adjust the `Base Color` within the `Principled BSDF` node_
...
_Using the `Glossy BSDF` node to add a material to the large sphere_
...
_Remember to export with the `glTF 2.0 (.glb/.gltf)` format._
...
_Generating a `.gltf` file from the original `.glb` file we exported in Blender with the **glTF Tools** extension_
...
_The newly created `.gltf` file with the option to view the model directly in Visual Studio Code (in the upper-right corner, circled in red)_
...
_Viewing the Blender model with **glTF Tools** in Visual Studio Code_
...
_Creating an `Image Texture` node and assigning a new image to it with default settings_
...
_Selecting all faces of the object in the **UV Editing** tab and applying `Smart UV Project` on it_
...
_The texture after `Smart UV Project`_
...
_`Bake` option in the **Shading** tab within the `Render Properties`_
...
_`Image Texture` node with the "baked" texture is connected with the `Material Output` node instead of the `Glossy BSDF` node_
...
_Finalized view with **glTF Tools**, including the "baked" texture for the large sphere_
