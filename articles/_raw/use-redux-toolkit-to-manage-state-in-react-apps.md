...
State management is one of the most important things you'll deal with in front end development. 
...
* Have Node installed so you can download the package using `npm`.
...
After the installation is complete, create a file named `src/redux/store.jsx` . Import `configureStore` from Redux Toolkit and then create an empty Redux store which will be exported like you can see in the code below:
...
_App Showing the function of Redux Toolkit_
...
Let’s connect on [Twitter](https://twitter.com/Quincyoghenex) and [LinkedIn](https://www.linkedin.com/in/quincy-oghenetejiri).
