---
title: 'How to Integrate RTK Query with Redux Toolkit: A Step-by-Step Guide for React
  Developers'
subtitle: ''
...
2. [Understanding RTK Query and core concepts](#heading-understanding-rtk-query-and-core-concepts)
    
3. [Integrating RTK Query with Redux Toolkit](#heading-integrating-rtk-query-with-redux-toolkit)
    
4. [Handling Data Caching with RTK Query](#heading-handling-data-caching-with-rtk-query)
    
5. [Error Handling and Loading States](#heading-error-handling-and-loading-states)
    
6. [Best Practices](#heading-best-practices)
...
You can read more about `json-server` [here](https://github.com/typicode/json-server/tree/v0).
...
In the root directory of your application, create a **data** folder. Inside this folder, create a `db.json` file. This will be where your "backend" is stored.

In the `src` directory, create two folders: **component** and **state**.

Inside the `component` folder, create two folders: **CardComponent** and **Modal,** and a file:`Movies.tsx`.

Inside the state folder, create a **movies** folder and a file: `store.ts`.

After creating the folders and files, your app structure should look like this:
...
Open the `db.json` file and paste in the following code:
...
Navigate to the **movies** folder and create a `movieApiSlice.ts` file. Open the`movieApiSlice.ts` file and paste in the following code:
...
In this case, you are using [`http://localhost:8080`](http://localhost:8080), which is the URL of the JSON server.
...
In your browser, open your [localhost](http://localhost) and you should see something like this:
