---
title: The Redux Book – The World’s Easiest Guide to Beginning Redux
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
...
For an expanded **Table of Contents** please [visit this link](https://medium.com/@ohansemmanuel/table-of-contents-for-understanding-redux-ea0667e1453d), & for more **advanced Redux** concepts check out my [Redux books](https://thereduxjsbooks.com).
...
_A Tweet on Redux’s learining curve from Eric Elliot._
...
_A basic Hello World Redux application._
...
_Sample exercise apps we will work on together._

Effective learning isn’t about just reading and listening. Effective learning is mostly about practice!

Think of these as homework, but without the angry teacher. While practicing the exercises, you can [Tweet](https://twitter.com/OhansEmmanuel) me with the hashtag #UnderstandingRedux and I’ll definitely have a look!
...
_Skypey: The Skype clone we will build together._
...
_Gif by Jona Dinges_

### Prerequisite

The only prerequisite is that you already know React. If you don’t, [Dave Ceddia](https://www.freecodecamp.org/news/understanding-redux-the-worlds-easiest-guide-to-beginning-redux-c695f45546f6/undefined)’s [Pure React](https://daveceddia.com/pure-react/) is my personal recommendation if you have some $$ to spare. I’m no affiliate. It’s just a good resource.
...
1. Visit the [book sales page](https://gumroad.com/l/Ocgbb) .
2. Use the coupon **FREECODECAMP** to get 100% off the price so that you get a $29 book for $0.
3. If you want to say thanks, please recommend this article by sharing it on social media.
...
_What is Redux? As seen on the Redux Documentation_
...
Redux’s author, [Dan Abamov](https://twitter.com/dan_abramov), also warns about the danger of introducing Redux [too early](https://medium.com/@dan_abramov/you-might-not-need-redux-be46360cf367) into your application. You may not like Redux, and that is fair enough. I have friends who don’t.
...
Dan’s talk called [Hot Reloading with Time Travel](https://youtu.be/xsSnOQynTHs) will give you a good sense of how these work.

Also, [Mark Ericsson](https://twitter.com/acemarke?lang=en), one of Redux’s maintainers, says that [over 60%](http://blog.isquaredsoftware.com/2018/03/redux-not-dead-yet/) of React apps in production use Redux. That’s a lot!
...
If you want some more reasons to use Redux, Dan, the Redux creator, has a few more reasons highlighted in [his article](https://medium.com/@dan_abramov/you-might-not-need-redux-be46360cf367) on Medium.
...
_Young Joe heads to the bank._
...
_Young Joe heads to the bank with the intention to withdraw some money._
...
_Young joe is at the bank! He goes straight to see the Cashier and makes his request known._
...
_If only Young Joe got into the Vault. He’ll cart away with as much as he finds._
...
_Here’s how you get money. Not from the Vault, sorry._
...
_The bank vault can be likened to the Redux Store!_
...
_The Cashier and Vault communication!_
...
You may grab the repo from [Github](https://github.com/ohansemmanuel/hello-redux) if you want to follow along — which I recommend.
...
_The basic Hello World App with the default state, “React”_
...
_The basic Hello World App with the tech prop changed to “Redux”_
...
_You have how much you even want to withdraw?_
...
![Image](https://cdn-media-1.freecodecamp.org/images/1*9U0ntjx6hpWKc7deAFYsdQ.png)
_The Cashier and Vault in sync!_
...
_The reducer is a mandatory argument passed into “createStore”_
...
Well, according to the Redux official [docs](https://redux.js.org/glossary):

> Reducers are the most important concept in Redux.

![Image](https://cdn-media-1.freecodecamp.org/images/1*ZfzLq5UcqweCssO8ulgHHw.png)
_Reducers are the most important concept in Redux. A more experienced engr. may argue in favour of middlewares._

Our Cashier is a pretty important person, huh?

So, what’s the deal with the Reducer. What does it do?

In more technical terms, a reducer is also called a reducing function. You may not have noticed, but you probably already use a reducer — if you’re conversant with the `[Array.reduce()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce)` method.
...
If I lost you at any point in the previous section, please read the section just one more time and I’m sure it’ll sink in. Better still, you can [ask me](https://twitter.com/OhansEmmanuel) a question.
...
_Uh, I need a new account with a $500 initial deposit_
...
_Replace “this.state” with “store.getState()”_
...
For a quicker overview, have a look at this [Github diff](https://github.com/ohansemmanuel/hello-redux/compare/solution?expand=1).
...
_The exercise: User card app built with React. Refactor to use Redux._
...
![Image](https://cdn-media-1.freecodecamp.org/images/1*FrKpOqcpaxYoOK3Xh2XELw.png)
_Updated design of the Hello world app._
...
Lastly, there’s a `data-tech` attribute on each `button` too. This is called a [data attribute](https://developer.mozilla.org/en-US/docs/Learn/HTML/Howto/Use_data_attributes). It is a way to store some extra information that doesn’t have any visual representation. It makes it slightly easier to grab certain values off of an element.
...
In one of the most beautiful [PR comments](https://github.com/reduxjs/redux/pull/1289) I’ve seen in a long time, Dan Abramov_,_ in one of the Redux application examples, said:
...
_The humble result with the Sidebar and Main sections laid out._
...
_The multiple contacts a user may have._
...
_Clicking a contact displays their message in the Main pane._
...
_Hmmm….A giant user box with nested contacts_
...
_A giant user array with nested contacts_
...
_A visual display of some of user’s properties._
...
There’s some advantages to using [objects over arrays](https://youtu.be/aJxcVidE0I0). There’s also downsides.

In this application, I’ll mostly be using objects to describe the fields within the state object.

If you’re not used to this approach, [this lovely video](https://youtu.be/aJxcVidE0I0) explains some of the advantages to it.
...
_Note how some messages are positioned on the left, and others on the right._
...
_Example state object for 2 user contacts. In the application we’ll build, there’ll be 10 contacts with 10 initial messages._
...
_We should have something like this when we successfully build out the list of users._
...
So, as opposed to fetching data remotely from a server, I have created a few [functions](https://gist.github.com/ohansemmanuel/d940d45c541ae8bc49e16b2fe0a55a2d) that will handle the creation of data for the App. We will be using this static data to build the Application.

For example, there’s a `contacts` variable already created within [static-data.js](https://gist.github.com/ohansemmanuel/d940d45c541ae8bc49e16b2fe0a55a2d), that will always return a randomly generated list of contacts. All you have to do is import this into the App. No Ajax calls.

Thus, create a new file in the root directory of the project and call it `static-data.js`

Copy the contents of [the gist here](https://gist.github.com/ohansemmanuel/d940d45c541ae8bc49e16b2fe0a55a2d) into that file. We’ll be making use of it pretty soon.
...
_The contacts object successfully passed as props to Sidebar_
...
_Note that the contacts props is now an array._

You can read more about the `Lodash` [.values method](https://lodash.com/docs/4.17.10#values) if you want. In a nutshell, it creates an array out of all key values of the object passed in.
...
_The sidebar rendered with unstyled contacts._
...
Since this is not a CSS book, I’m skipping some of the styling explanations. However, if anything confuses you, just ask me [on Twitter](https://twitter.com/ohansemmanuel), and I’ll be happy to help.
...
_The sidebar contacts well styled!_
...
The quickest way to reach me will be to tweet your question [via Twitter](https://twitter.com/ohansemmanuel), with the hashtag, **#UnderstandingRedux**. This way I can easily find and answer your question.
...
_An high level structure of the Skypey layout_
...
_Pass props from App, as done in normal React apps._
...
_With Redux, you dont have to pass props. Just get the required values directly from the store_
...
_“user” and “contacts” now exist in the state object._
...
_The unstyled Empty screen being displayed._
...
_Here’s the entire chat window._
...
_Hurray! We’ve got this working for now ;)_
...
_The sub components, Header, Chats and MessageInput_
...
_The information in the header are those of the clicked contact._
...
_The Chats component to be built_
...
_Messages now exist in the state object._
...
_Have a look at the messages field one more time_
...
_The current result of our iterations. Still needs some work._
...
_ChatWindow — not the best of looks yet._
...
![Image](https://cdn-media-1.freecodecamp.org/images/1*Dd2Eqk84pggsQekT0pcq8Q.png)

However, when an action with type `SET_TYPING_VALUE` is dispatched, the value in the payload will be returned.

![Image](https://cdn-media-1.freecodecamp.org/images/1*p8nWaVl1e4GYwZfTmhpOTQ.png)
_handle the SET_TYPING_VALUE type_

Otherwise, the default state `""` will be returned.

![Image](https://cdn-media-1.freecodecamp.org/images/1*6023LKlOqne2OQdoBFA-rA.png)
_As a default, return the same state_
...
![Image](https://cdn-media-1.freecodecamp.org/images/1*q-QbR2FPaOo1L8Uqpp5M3g.gif)
_The results so far!_
...
_an overview of what’s to be done._
...
_have another look at the return statement. Can you make sense of this now?_
...
If you’re not familiar with `React.createRef()`, it is perfectly normal. This is because React 16 introduced a [new way to create Refs](https://reactjs.org/docs/refs-and-the-dom.html).
...
_Final results looking all good! Gorgeous!_
...
When you’re done, [send me a tweet](https://twitter.com/OhansEmmanuel) and I’ll be delighted to cheer you up.
...
_Include the edit message functionality shown here._
...
_Include the delete message functionality shown here._
...
_The sequel to this current book_
...
The following section is an excerpt from, [Understanding Redux 2](http://thereduxjsbooks.com/)_._
...
I cover this deeply in the follow up book, [Understanding Redux 2](http://thereduxjsbooks.com/)_._
