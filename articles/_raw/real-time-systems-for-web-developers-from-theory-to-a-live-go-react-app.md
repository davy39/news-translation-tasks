...
* [What a Real-Time System Really Means](#heading-what-a-real-time-system-really-means)
    
    * [Types of Real-Time Systems](#heading-types-of-real-time-systems)
        
    * [Why Most Web Apps Are Not Real-Time](#heading-why-most-web-apps-are-not-real-time)
        
* [What We Are Going to Build](#heading-what-we-are-going-to-build)
    
* [System Architecture](#heading-system-architecture)
    
* [Why Go is a Good Fit for Our Use Case](#heading-why-go-is-a-good-fit-for-our-use-case)
    
    * [Generating Events with Go](#heading-generating-events-with-go)
        
    * [Deadline-aware Processing](#heading-deadline-aware-processing)
        
    * [Applying Back-Pressure](#heading-applying-back-pressure)
        
    * [Streaming Events to the Browser](#heading-streaming-events-to-the-browser)
        
    * [Consuming a WebSocket event (React + TypeScript)](#heading-consuming-a-websocket-event-react-typescript)
        
    * [Making React Real-Time Friendly](#heading-making-react-real-time-friendly)
        
    * [Creating the StatsBar Component](#heading-creating-the-statsbar-component)
        
    * [Creating the Events Table](#heading-creating-the-events-table)
...
* **Go Concurrency and Goroutines:** The official Go blog covers concurrency patterns, including goroutines and channels, which are [fundamental for writing concurrent Go code](https://go.dev/blog/concurrency-patterns).
    
* **WebSockets with Go:** A thorough WebSocket tutorial using the popular gorilla/websocket package that shows how to set up a WebSocket server in Go and manage connections and messages. [Here’s a Go WebSocket tutorial (with gorilla/websocket)](https://tutorialedge.net/golang/go-websocket-tutorial).
    
* **WebSockets in React:** A complete guide to WebSockets in React, explaining how to open and manage a WebSocket connection and handle incoming messages in components. [Here’s a complete guide to WebSockets with React](https://ably.com/blog/websockets-react-tutorial).
...
<table><tbody><tr><td colspan="1" rowspan="1"><p><strong>Realtime System</strong></p></td><td colspan="1" rowspan="1"><p><strong>Niche applicable&nbsp;</strong></p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Hard Real-time: </strong>Missing a deadline is catastrophic here.</p></td><td colspan="1" rowspan="1"><p>Flight Control &amp; Pacemakers</p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Soft Real-time:</strong> Missing deadline degrades quality but does not crash the system.&nbsp;</p></td><td colspan="1" rowspan="1"><p>Video Streaming &amp; Trading Dashboards</p></td></tr><tr><td colspan="1" rowspan="1"><p><strong>Firm Real-time</strong>: Late results are useless and should be discarded.</p></td><td colspan="1" rowspan="1"><p>Car Auction Web/Mobile Apps</p></td></tr></tbody></table>
...
Next, we’ll build the Websocket broadcast system to push processed events to the frontend using the [Gorilla](http://github.com/gorilla/websocket) `Go` websocket package (but feel free to use any package of your choice).
...
The `requestAnimationFrame()` takes in a callback function `flush` which is regulated by the animation frame, ensuring that we don’t cross the refresh rate threshold before the next repaint. You can learn more about it `requestAnimationFrame()` [here](https://developer.mozilla.org/en-US/docs/Web/API/DedicatedWorkerGlobalScope/requestAnimationFrame).
...
The React frontend is scaffolded using [create-react-app](https://react.dev/learn/creating-a-react-app), but the same approach can be used for other frameworks, such as `Next.js` or `Vite`. The complete source code, including the frontend and backend, is available in the repository [here](https://github.com/emmanueletukudo/realtime-go-react). You can reach out to me on the [X platform](https://x.com/eetukudo_) if you need my assistance.
