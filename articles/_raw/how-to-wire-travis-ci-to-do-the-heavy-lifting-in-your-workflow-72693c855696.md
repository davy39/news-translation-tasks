...
The `npm run build` command does what we asked for. It builds our app, and optimizes and [minifies](https://en.wikipedia.org/wiki/Minification_(programming)) our assets. And it places everything in a folder called **build**.
...
There is an exception. Your repository name should not be `<your_user_name>.git`hub.`io or <orgname&g`t;.github.io
...
_We are done here! Time to celebrate._
