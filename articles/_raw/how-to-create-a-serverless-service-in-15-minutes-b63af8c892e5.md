...
After installation, we will setup AWS by using the `aws configure`command:
...
> Note: You need to click the plus sign on the left side to add the `_counter_value_` attribute, and don’t forget to set the type of `_counter_value_` to **Number**.
...
> Note: If you’ve been following along with this tutorial, the bucket name myfrontend.example.com may not be available, as bucket names are globally unique. Also, you’ll need to create a bucket name based on your public domain. For example,`_myfrontend.**[yourdomain]**.com_`
