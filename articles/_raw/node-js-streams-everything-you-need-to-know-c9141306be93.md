...
>   
> Read the updated version of this content and more about Node at [**jscomplete.com/node-beyond-basics**](https://jscomplete.com/g/node-streams).
...
_Composability with Linux commands_
...
We’re basically pushing all the data in the stream _before_ piping it to `process.stdout`. The much better way is to push data _on demand_, when a consumer asks for it. We can do that by implementing the `read()` method in the configuration object:
