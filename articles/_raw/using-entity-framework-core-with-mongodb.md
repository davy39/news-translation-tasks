...
There is also a video version of this article that you can [watch on the freeCodeCamp.org YouTube channel](https://youtu.be/fv2-A5e-KHA).
...
1. **Create a MongoDB Atlas Account**: Sign up for a free account at [MongoDB Atlas](https://www.mongodb.com/cloud/atlas/register?utm_campaign=freecodecamp_ef&utm_source=freecodecamp&utm_medium=referral).
...
* Free [**MongoDB Atlas account and free tier cluster**.](https://www.mongodb.com/try)
...
After the existing line [`builder.Services`](http://builder.Services)`.AddControllersWithViews();`, add the following code:
...
[ASP.NET](http://ASP.NET) Core MVC uses a convention pattern whereby you name the .cshtml file the name of the endpoint/method it uses and it lives inside a folder named after its controller.
