...
Notice how I’ve embedded a [github.com web page](https://github.com/Jasonette) but the rest of the layout is all native UI components, such as [the navigation header](https://docs.jasonette.com/document/#bodyheader) and the [bottom tab bar](https://docs.jasonette.com/document/#tabs). And the transition is automatically native without you having to rewrite the website using any APIs.
...
Some parts of your app may be better implemented using the web engine. For example, [Websocket](https://en.wikipedia.org/wiki/WebSocket) is a web-native feature that’s designed for the web environment. In this case it makes sense to use the built-in web engine (**WKWebView for iOS** and **WebView for Android**) instead of installing a 3rd party library that essentially **“emulates”** Websocket.
...
You can learn more about Jasonette [here](https://medium.freecodecamp.org/how-to-build-cross-platform-mobile-apps-using-nothing-more-than-a-json-markup-f493abec1873).
...
[**JSON Web Container**](http://jasonette.com/webcontainer/)  
[_HTML inside JSON Turns into Native App Components_jasonette.com](http://jasonette.com/webcontainer/)
...
Previously in version 1, to use web container as a background view component, you had to first [set the `$jason.body.background.type` to `"html"` and then hard-code the HTML text under `$jason.body.background.text` attribute](https://jasonette.com/webcontainer/) like this:
...
`[$agent.request](https://docs.jasonette.com/agents/#1-agentrequest)` is the native API that triggers a JSON-RPC request into the web container. To use it, we must pass an `options` object as its parameter.

The `options` object is the actual [JSON-RPC request](http://www.jsonrpc.org/specification#conventions) that will be sent to the web container. Let’s look at what each attribute means:

* `id`: Web container is built on top of a lower level architecture called [agent](https://jasonette.com/agent/). Normally you can have multiple agents for a single view, and each agent can have its unique ID. But [Web container is a special type of agent which can only have the id of `$webcontainer`](https://docs.jasonette.com/web/#1-background-web-container-is-an-agent), which is why we use that ID here.
...
When the view loads (`[$jason.head.actions.$load](https://docs.jasonette.com/actions/#1-load)`), make a JSON-RPC request into the web container agent (`[$agent.request](https://docs.jasonette.com/agents/#1-agentrequest)`) where the request is specified under `options`.

The web container is defined under `[$jason.body.background](https://docs.jasonette.com/web/#in-depth-on-background-web-container)`, which in this case loads a local file called `file://index.html`.
...
I’ve only explained how the parent app can trigger the child web container’s JavaScript function calls, but you can also do the opposite and [let the web container trigger the parent app’s native API](https://docs.jasonette.com/agents/#3-agenttrigger).

To learn more, check out the [agent documentation](https://docs.jasonette.com/agents/).
...
1. The [footer input component is 100% native](https://docs.jasonette.com/document/#input).
2. The QR code is generated by the web container [as a web app](https://github.com/Jasonette/Jasonpedia/blob/gh-pages/webcontainer/agent/fn/agent.html).
3. When a user enters something and presses “Generate,” it calls `$agent.request` action into the web container agent, calling the [JavaScript function “qr”](https://github.com/Jasonette/Jasonpedia/blob/gh-pages/webcontainer/agent/fn/agent.html#L22)

You can check out the example [here](https://github.com/Jasonette/Jasonpedia/blob/gh-pages/webcontainer/agent/fn/index.json).
...
As mentioned in the previous section, the `$jason.body.background` web container is just another `agent`. This means you can use the same `[$agent.inject](https://docs.jasonette.com/agents/#7-agentinject)` method available to regular agents.
...
This means you can selectively trigger different actions by [checking the `$jason.url` value.](https://docs.jasonette.com/web/#b-intercept-url-visits)
...
Check out the full JSON markup for the web browser [here](https://github.com/Jasonette/Jasonpedia/blob/gh-pages/webcontainer/agent/hijack.json) (it’s only 48 lines!).
...
In this example, I’ve created an app that displays [jasonbase.com](https://www.jasonbase.com) in a web container as the home view.
...
You can check out the code [here](https://github.com/Jasonette/Jasonpedia/blob/gh-pages/webcontainer/agent/hybrid.json).
...
Overall, it’s been great and I’m satisfied with how it turned out. [The documentation is up to date](https://docs.jasonette.com/web/) to reflect all the new features, so feel free to dig in and play around.
...
Going forward I’m planning to share more of these use cases and tutorials, so if you’re interested, please follow along on [medium](https://medium.com/@gliechtenstein) or [twitter](https://twitter.com/jasonclient).
