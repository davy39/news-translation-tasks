...
![76% reduction in time taken to run tests in Jest vs Node.js test runner](https://cdn.hashnode.com/res/hashnode/image/upload/v1738830673460/1560f7a3-38c1-42f3-8944-df06b40d73e4.png align="center")
...
* [The Node.js Test Runner](#heading-the-nodejs-test-runner)
    
* [MongoDB In-Memory Server](#heading-mongodb-in-memory-server)
    
* [How to Write the Tests](#heading-how-to-write-the-tests)
    
    * [1\. Set Up the Project](#heading-1-set-up-the-project)
        
    * [2\. Set up Mongoose Schema](#heading-2-set-up-mongoose-schema)
        
    * [3\. Set Up Services](#heading-3-set-up-services)
        
    * [4\. Set Up Tests](#heading-4-set-up-tests)
        
    * [5\. Write Tests](#heading-5-write-tests)
        
    * [6\. Pass Tests](#heading-6-pass-tests)
        
    * [7\. Use TypeScript (Optional)](#heading-7-use-typescript-optional)
...
Writing and fetching data using a real database is slower [compared to writing and fetching data from memory](https://www.mongodb.com/resources/basics/databases/in-memory-database). When running automated tests, using a real MongoDB server will be slower than using an in-memory database server, and that is where [mongodb-memory-server](https://github.com/typegoose/mongodb-memory-server) becomes useful.

![Comparison between memory and database communication with CPU](https://cdn.hashnode.com/res/hashnode/image/upload/v1738832586702/62360547-70e8-4e74-854f-c7ad74d182ea.png align="center")

According to its documentation, mongodb-memory-server creates and starts a real MongoDB server programmatically from within Node.js, but uses an in-memory database by default. It also allows you to connect to the database server it creates using your preferred object data mapper such as Mongoose, Prisma, or TypeORM. In this guide, we’ll use [Mongoose](https://mongoosejs.com/) (v8.9.6).
...
Note: Make sure to install v9.1.6 of mongodb-memory-server to follow this guide. v10 currently has issues with cleaning up resources after tests are done. See this issue titled [Node forking will include any --import from the original command](https://github.com/typegoose/mongodb-memory-server/issues/912).
...
I created a GitHub repository to make it easier for you to follow this guide. Clone the repository at [nodejs-test-runner-mongoose](https://github.com/orimdominic/nodejs-test-runner-mongoose) and checkout branch `01-setup`.
...
We’ll set up the schema for two collections (Task and User) in branch [`02-setup-schema`](https://github.com/orimdominic/nodejs-test-runner-mongoose/tree/02-setup-schema) using Mongoose. The [`task/model.mjs`](https://github.com/orimdominic/nodejs-test-runner-mongoose/blob/02-setup-schema/task/model.mjs) and [`user/model.mjs`](https://github.com/orimdominic/nodejs-test-runner-mongoose/blob/02-setup-schema/user/model.mjs) files contain the schema for the Task and the User collection, respectively. We’ll also set up a database connection in `index.mjs` to ensure that the schema setup works correctly.
...
You can see the differences between `01-setup` and `02-setup-schema` via the [01-setup &lt;&gt; 02-setup-schema diff on GitHub](https://github.com/orimdominic/nodejs-test-runner-mongoose/compare/01-setup...02-setup-schema).

### 3\. Set Up Services

Next, we create service files ([`task/service.mjs`](https://github.com/orimdominic/nodejs-test-runner-mongoose/blob/03-setup-services/task/service.mjs) and [`user/service.mjs`](https://github.com/orimdominic/nodejs-test-runner-mongoose/blob/03-setup-services/user/service.mjs)) in branch [`03-setup-services`](https://github.com/orimdominic/nodejs-test-runner-mongoose/tree/03-setup-services). Both files currently contain empty functions that we’ll write tests for later. These functions will contain business logic and also communicate with the database. We’re using [JSDoc](https://jsdoc.app/) comments for typing parameters and return values.

Click [02-setup-schema &lt;&gt; 03-setup-services diff](https://github.com/orimdominic/nodejs-test-runner-mongoose/compare/01-setup...02-setup-schema) to see the code changes between `02-setup-schema` and `03-setup-services`.

### 4\. Set Up Tests

In branch [`04-set-up-tests`](https://github.com/orimdominic/nodejs-test-runner-mongoose/tree/04-set-up-tests), we set up the codebase to run tests. We create [`test.setup.mjs`](https://github.com/orimdominic/nodejs-test-runner-mongoose/blob/04-set-up-tests/test.setup.mjs) which contains code that will be run before each test file is executed.
...
The `connect` and `closeDatabase` functions get executed in the [`t.before`](https://nodejs.org/api/test.html#beforefn-options) hook and the [`t.after`](https://nodejs.org/api/test.html#afterfn-options) hook respectively. This ensures that, before a test file is run, a database connection is established through `t.before`. Then after tests for the file have been completely run, the database connection is dropped and the resources used are cleared up through `t.after`.

In `package.json`, we’ll update the npm `test` script to `node --test --import ./test.setup.mjs`. This command ensures that the `test.setup.mjs` ES Module is preloaded and executed through the [\--import](https://nodejs.org/api/cli.html#--importmodule) CLI command before each test file is run.

Then we’ll create the test files with empty tests in the `__tests__` folders for `user` and `task`. After implementing the [new changes in 04-set-up-tests](https://github.com/orimdominic/nodejs-test-runner-mongoose/compare/03-setup-services...04-set-up-tests), running the `test` script with `npm run test` should display output similar to the snippet below:
...
You can see the code we wrote to pass tests in the [code changes between 05-write-tests and 06-pass-tests](https://github.com/orimdominic/nodejs-test-runner-mongoose/compare/05-write-tests...06-pass-tests).

### 7\. Use TypeScript (Optional)

If you intend to run tests with TypeScript, you can checkout branch [`07-with-typescript`](https://github.com/orimdominic/nodejs-test-runner-mongoose/tree/07-with-typescript). You need to have Node.js `>=v22.6.0` installed because we’re using the `--experimental-strip-types` option in the `test`. To set up tests to run with TypeScript, go through the following steps:
...
* [`--experimental-strip-types`](https://nodejs.org/docs/latest-v22.x/api/cli.html#--experimental-strip-types) helps strip out types before each test file is executed.
...
The rest of the [changes from 06-pass-tests to 07-with-typescript](https://github.com/orimdominic/nodejs-test-runner-mongoose/compare/06-pass-tests...07-with-typescript) involve updating types, changing file extensions from `.mjs` to `.ts` and updating import statements.
...
If you find the [nodejs-test-runner-mongoose](https://github.com/orimdominic/nodejs-test-runner-mongoose) repository useful, kindly give it a star. It encourages me. Thank you.
