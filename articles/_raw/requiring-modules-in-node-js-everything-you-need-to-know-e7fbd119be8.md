...
>   
> Read the updated version of this content and more about Node at [**jscomplete.com/node-beyond-basics**](https://jscomplete.com/g/node-modules).
...
You can use the `node-gyp` package to compile and build the `.cc` file into a `.node` file. You just need to configure a [binding.gyp](https://nodejs.org/api/addons.html#addons_building) file to tell `node-gyp` what to do.
