...
Boom! ? There are other benefits to this aside from the fact that we went from 22 HTTP requests to one. For instance, your User may have other information attached to it. Maybe you expose the timestamp of when a user signed up. Maybe another client doesn’t care about a post’s categories. If a client doesn’t need to query for a piece of data, n_either does your server._ So when a client saves, you can save too by simplifying your own database queries.
...
* Play around with an example, the [Star Wars GraphQL API](http://graphql-swapi.parseapp.com/?query=%7B%0A%20%20film(id%3A%20%22ZmlsbXM6MQ%3D%3D%22)%20%7B%0A%20%20%20%20title%0A%20%20%20%20releaseDate%0A%20%20%7D%0A%7D)
