...
1. [Why is CI/CD Important?](#heading-why-is-cicd-important)
    
2. [Step 1: Set Up a Webhook in Bitbucket](#heading-step-1-set-up-a-webhook-in-bitbucket)
    
3. [Step 2: Set Up the Flask Listener on Your Linux Server](#heading-step-2-set-up-the-flask-listener-on-your-linux-server)
    
4. [Step 3: Expose the Flask App (Optional)](#heading-step-3-expose-the-flask-app-optional)
    
5. [Step 4: Test the Setup](#heading-step-4-test-the-setup)
    
6. [Step 5: Security Considerations](#heading-step-5-security-considerations)
...
6. In the **URL** field, provide the URL to your server where the webhook will send the request. If you’re running a Flask app locally, this would be something like [`http://your-server-ip/pull-repo`](http://your-server-ip/pull-repo). (For production environments, it’s highly recommended to use HTTPS to secure the communication between Bitbucket and your server.)
...
![Bitbucket screen showing the user the creation of a webhook, where your server will pull the modifications when you push or merge in your reposiroty.](https://cdn.hashnode.com/res/hashnode/image/upload/v1738092826221/e0d96fd3-d843-4064-a08d-4de95b985800.png align="center")
...
Create a new Python script (for example, [`app_repo_pull.py`](http://app.py)) on your server and add the following code:
...
* [`subprocess.run`](http://subprocess.run)`(["git", "-C", "/path/to/your/repository", "fetch"])`: This command fetches the latest changes from the remote repository without affecting the local working directory.
    
* [`subprocess.run`](http://subprocess.run)`(["git", "-C", "/path/to/your/repository", "reset", "--hard", "origin/test"])`: This command performs a hard reset, forcing the local repository to match the remote `test` branch. Replace `test` with the name of your branch.
...
You can [fork the code from here](https://github.com/jpromanonet/ci_cd_fcc/tree/main).
