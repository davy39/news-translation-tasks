...
However, [learning Rust](https://www.secondstate.io/articles/a-rusty-hello-world/) typically requires you to install a bunch of command line tools on your computer. The Rust compiler is slow because the whole Rust paradigm is set up to deeply analyze the source code and find bugs at compile time, instead of crashing at runtime.
...
* Of course, the best way to run Rust programs on the server is [inside a WebAssembly Virtual Machine](https://www.secondstate.io/articles/rust-and-webassembly/). Checkout the open source [Second State VM](https://www.secondstate.io/) for that!
