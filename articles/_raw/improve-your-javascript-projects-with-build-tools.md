...
* [What Tools and Configs Are We Looking At?](#heading-what-tools-and-configs-are-we-looking-at)
* [Why These Conventions Are Useful](#heading-why-these-conventions-are-useful)
* [How to Setup Coding Conventions](#heading-how-to-setup-coding-conventions)
* [What is ESLint?](#heading-what-is-eslint)
* [What are Git Hooks?](#heading-what-are-git-hooks)
* [Set Up the Project](#heading-set-up-the-project)
* [Rule #1: `no-unused-vars`](#heading-rule-1-no-unused-vars)
* [Rule #2: `no-console`](#heading-rule-2-no-console)
* [Rule #3: `no-duplicate-imports` and Sorting Imports](#heading-rule-3-no-duplicate-imports-and-sorting-imports)
* [How to Set Up Git Hooks](#heading-how-to-set-up-git-hooks)
* [Gitleaks: Remove Secrets Before Commits](#heading-gitleaks-remove-secrets-before-commits)
* [Run Unit Tests Before Commits](#heading-run-unit-tests-before-commits)
...
* Basics of [ESLint](https://eslint.org/docs/latest/use/core-concepts/)
* Setting up a simple JS project with [npm](https://docs.npmjs.com/cli/v10/commands/npm-init) or [yarn](https://classic.yarnpkg.com/lang/en/docs/cli/init/)
* [Bash Scripting](https://www.freecodecamp.org/news/bash-scripting-tutorial-linux-shell-script-and-command-line-for-beginners/)
* [Testing code with Jest](https://www.youtube.com/watch?v=IPiUDhwnZxA)

## What Tools and Configs Are We Looking At?

I have seen many repositories that enforce their own strict conventions – and I totally agree with them. One such example I found is the [Cesium repository](https://github.com/CesiumGS/cesium) and [their style guides](https://github.com/CesiumGS/cesium/blob/main/Documentation/Contributors/CodingGuide/README.md#coding-guide).
...
You can read more about ESLint’s core concepts [here](https://eslint.org/docs/latest/use/core-concepts/).
...
You can read more about Git hooks [here](https://www.atlassian.com/git/tutorials/git-hooks).
...
You can read more about flat file configuration [here](https://eslint.org/docs/latest/use/configure/configuration-files).
...
Right now it enables [no-unused-vars](https://eslint.org/docs/latest/rules/no-unused-vars#rule-details) which is set to a `warn` value. This `warn` value tells ESLint to show warning message while linting. You can also set this value to `error` if you want the linter to show this case as an error.
...
_Output of running ESLint CLI_
...
So this is how linting works. But wouldn’t it be awesome if you could get these error messages in your IDE itself with a squiggly line below each variable name that is unused? Well, yes – it's absolutely possible. In VS Code you can do this by adding the [ESLint VS code extension](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint).
...
_Photo by [Unsplash](https://unsplash.com/@v2osk?utm_content=creditCopyText&amp;utm_medium=referral&amp;utm_source=unsplash">v2osk</a> on <a href="https://unsplash.com/photos/assorted-armchair-on-wall-near-door-1hUY8SpJ8Cw?utm_content=creditCopyText&amp;utm_medium=referral&amp;utm_source=unsplash)_

This is one of those ESLint rules that doesn’t allow you to keep unused variables in your codebase. You can read more about this rule [here](https://eslint.org/docs/latest/rules/no-unused-vars#rule-details).
...
_Output of rule#1 is configured_
...
So to avoid all this hassle, ESLint has the [no-console](https://eslint.org/docs/latest/rules/no-console#rule-details) rule. This rule will provide linting whenever it finds console statement in your code base.
...
_console.log becomes an error when rule #2 is configured_
...
You might even have different imports that are from the same library. These kinds of ways of importing libs can be chaotic and hard to follow. This is where the [no-duplicate-imports](https://eslint.org/docs/latest/rules/no-duplicate-imports#rule-details) ESLint rule and [eslint-plugin-simple-import-sort](https://github.com/lydell/eslint-plugin-simple-import-sort) ESLint plugin comes into play.
...
ESLint doesn’t have any rule that will help you sort your imports. In this case, you can get help from different community-based plugins on [awesome-eslint](https://github.com/dustinspecker/awesome-eslint?tab=readme-ov-file).
...
You can also set the sorting order of this plugin to something different, which you can read more about [here](https://github.com/lydell/eslint-plugin-simple-import-sort?tab=readme-ov-file#sort-order).
...
Git hooks are nothing but Git features on steroids. All the details and the origin or Git hooks are out of the scope of this article, so I highly recommend that you read more about them [here](https://git-scm.com/docs/githooks).

There are many libraries out there that will help you manage your Git hooks. I will use [Husky](https://typicode.github.io/husky/) here. To install Husky in your codebase, run the below commands:
...
Gitleaks already has one example in the Python `precommit` hook. It checks if the Gitleaks hook is enabled. If it is enabled, then it runs the Gitleaks `protect` function on the staged files. You can find that code [here](https://github.com/gitleaks/gitleaks/blob/26f34692fac6e9daec13c770421b4ed990d1c321/scripts/pre-commit.py).
...
_gitleaks tool running before commit_
...
`git diff` helps you find the difference between the modified changes and the current file. You can read more about `git diff` and its options [here](https://git-scm.com/docs/git-diff).

Next, using the [pipe symbol](https://superuser.com/a/756259), we filter the output of the previous `git diff` command with the help of [grep](https://www.freecodecamp.org/news/grep-command-in-linux-usage-options-and-syntax-examples). We tell grep to find all the file names that end with the `.test.js` extension.

### Step 2: Run the unit test on staged files

Now to run the unit test, make sure that you have installed [Jest](https://jestjs.io/docs/getting-started) in your project. To run the unit test on the staged files and the files related to it, run the below command:
...
You can find the final code [here](https://github.com/keyurparalkar/eslint-githooks-example).

Thanks a lot for reading my article! You can follow me on [Twitter](https://twitter.com/keurplkar), [GitHub](http://github.com/keyurparalkar), and [LinkedIn](https://www.linkedin.com/in/keyur-paralkar-494415107/).
