...
[It is said](https://www.totaltypescript.com/books/total-typescript-essentials/the-weird-parts#the-type-and-value-worlds) that there are two different worlds in TypeScript that exist side by side: the type world and the value world.
...
And, here it is in the type world, used for extracting the type of the function `increment`, which is then passed to [a utility type called `ReturnType`](https://www.typescriptlang.org/docs/handbook/utility-types.html#returntypetype):
...
1. [Recursive Types for Trees and Linked Lists](#heading-recursive-types-for-trees-and-linked-lists)
    
2. [Recursion with Mapped and Conditional Types](#heading-recursion-with-mapped-and-conditional-types)
    
3. [Use Cases for Recursive Types](#heading-use-cases-for-recursive-types)
    
    * [Use Case 1: DeepPartial](#heading-use-case-1-deeppartial)
        
    * [Use Case 2: UnwrapArray](#heading-use-case-2-unwraparray)
        
4. [Conclusion (and a Warning!)](#heading-conclusion-and-a-warning)
...
![A binary tree where the root node has two children, left and right. The left child has left and right children, each of them also has left and right children of their own. The right child of the root node has a left child that has a left child of its own, and a right child that is a leaf node.](https://cdn.hashnode.com/res/hashnode/image/upload/v1746454664280/0eed3e38-151e-403f-8d88-028519e23f3e.png align="center")
...
![A linked list where the head points to the node with the value 1, which points to the node with the value 2, which points to the node with the value 3 that points to null.](https://cdn.hashnode.com/res/hashnode/image/upload/v1746456962646/9996894c-162a-4c71-b2b3-3c9369592551.jpeg align="center")
...
![A screenshot of the code block that's defined above when it's hovered over the type `Result`. Keys are the same as the ones in the `colors` object, and all the values are `boolean`.](https://cdn.hashnode.com/res/hashnode/image/upload/v1746481133106/390515bd-4f98-4072-a8f8-94a3ef39cec4.png align="center")
...
Before going further, let's also take a look at the conditional types, which look very similar to the conditional expressions that use [the ternary operator](https://www.freecodecamp.org/news/javascript-ternary-operator-explained/):
...
> Keep in mind that while these recursive types are powerful, they should be used responsibly and sparingly. [(Source)](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-4-1.html#recursive-conditional-types)
