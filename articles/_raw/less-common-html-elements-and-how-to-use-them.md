...
1. [The `<q>` Tag: Short Inline Quotes](#heading-the-tag-short-inline-quotes)
    
2. [The `<s>` Tag: Strikethrough Text](#heading-the-tag-strikethrough-text)
    
3. [The `<mark>` Tag: Highlighted Text](#heading-the-tag-highlighted-text)
    
4. [The `<ruby>` Tag: Annotating East Asian Language Text](#heading-the-tag-annotating-east-asian-language-text)
    
5. [The `<time>` Tag: Semantic Time and Date](#heading-the-tag-semantic-time-and-date)
    
6. [The `<bdi>` Tag: Bi-Directional Text Isolation](#heading-the-tag-bi-directional-text-isolation)
    
7. [The `<dfn>` Tag: Defining Terms](#heading-the-tag-defining-terms)
    
8. [The `<wbr>` Tag: Line Break Opportunities](#heading-the-tag-line-break-opportunities)
    
9. [The `<ins>` Tag: Inserted Text](#heading-the-tag-inserted-text)
    
10. [The `<del>` Tag: Deleted Text](#heading-the-tag-deleted-text)
...
*This product was <s>$50</s> now only $30!*
...
1. **Blog posts**: You can display when an article was published or last updated:
    
    ```html
    <p>Last updated on <time datetime="2024-09-28">September 28, 2024</time>.</p>
    ```
    
2. **Event listings**: Event websites can use the `<time>` tag to list when an event will take place:
    
    ```html
    <p>Our next meetup will be on <time datetime="2024-11-10">November 10, 2024</time> at <time datetime="18:00">6:00 PM</time>.</p>
    ```
    
3. **Deadlines**: When presenting important deadlines, use the `<time>` tag for clarity:
...
In this example, the price change is clear. The old price ($50) is struck through, and the new price ($40) follows right after.
...
If you have any questions or suggestions, feel free to reach out on [LinkedIn](https://ng.linkedin.com/in/joan-ayebola). If you enjoyed this content, consider [buying me a coffee](https://www.buymeacoffee.com/joanayebola) to support the creation of more developer-friendly contents.
