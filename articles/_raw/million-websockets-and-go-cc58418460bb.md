...
> _Hint: in some cases the `go:linkname` can be used to return the buffers to the `sync.Pool` inside `net/http` through the call `net/http.putBufio{Reader,Writer}`._
...
> _On github.com/golang/go, there is the [issue](https://github.com/golang/go/issues/15735#issuecomment-266574151) of exporting netpoll functions._
...
> _Note that we do not handle cases when operating system returns `EAGAIN` on `write()` system calls. We lean on Go runtime for such cases, cause it is actually rare for such kind of servers. Nevertheless, it could be handled in the same way if needed._
