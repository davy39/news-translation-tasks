...
_Python micro-frameworks (blue), Dark side of force (green) and Japronto (purple)_
...
_HTTP pipelining (image credit Wikipedia)_
...
When many small GET requests are pipelined together by the client, there’s a high probability that they’ll arrive in one TCP packet (thanks to [Nagle’s algorithm](https://en.wikipedia.org/wiki/Nagle's_algorithm)) on the server side, then be **read** back by one **system call**.
...
_Japronto gives a 1,214,440 RPS median of grouped continuous data, calculated as the 50th percentile, using interpolation._
...
_Picohttpparser relies on SSE4.2 and CMPESTRI x86_64 intrinsic to do parsing_
