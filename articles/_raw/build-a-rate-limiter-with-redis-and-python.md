...
While not absolutely required to follow along, I’d recommend setting up a Proxmox server on an old laptop to implement the topics you learn and code along with the article. I recommend this [YouTube playlist](https://www.youtube.com/watch?v=5j0Zb6x_hOk&list=PLT98CRl2KxKHnlbYhtABg6cF50bYa8Ulo) for getting started. Please note that I am in no way affiliated with this channel. I just found it helpful for me.
...
Before we dive into the code, let's look at the architecture we're building. I will be using [Proxmox Virtual Environment](https://www.proxmox.com/en/products/proxmox-virtual-environment/overview) to setup a server cluster just like you would have in a datacenter.
...
To setup your own cluster, all you need is an old computer. You can download the ISO image from [here](https://www.proxmox.com/en/downloads) and boot from the USB drive. Once you install it, you can configure the host machine via a web browser on any other computer on the same network.

For example, my proxmox server is located at `10.0.0.108` and I can access it via the browser on my laptop.

![Example Proxmox cluster](https://cdn.hashnode.com/res/hashnode/image/upload/v1759194790299/35e9363f-b739-4085-a589-c1bafbac0504.png align="center")

We define all our virtual machines in our `main.tf` file. And run a simple command `terraform apply` to spin these servers up. For more reading on how to use Terraform with Proxmox, I recommend this [blog post](https://spacelift.io/blog/terraform-proxmox-provider)
...
![A Small diagram depicting the architecture we'll form with all these virtual nodes](https://cdn.hashnode.com/res/hashnode/image/upload/v1758476002871/1d70ce5b-e19c-4d7d-9c0b-cc18840a07bf.png align="center")
...
Our [main.tf](https://github.com/sravankaruturi/system-design/blob/main/infra/main.tf) file defines all the components of our system. Let's look at a key piece: the Redis VM.
...
Let's break down the code from [`app.py`](https://github.com/sravankaruturi/system-design/blob/main/web-servers/app.py).
...
![A gif showing the test run of the load testing script](https://cdn.hashnode.com/res/hashnode/image/upload/v1758477504110/3a2f3f0f-8db0-453d-8900-42a6d0966a11.gif align="center")

We can also check the logs on our webserver to see all the requests that were sent to it.

![A small gif demonstrating Web Server Logs](https://cdn.hashnode.com/res/hashnode/image/upload/v1758477959201/80a39d07-1c4e-4d45-8a42-9ac2ce6f360d.gif align="center")
...
* **Infrastructure as Code** with Terraform to define our virtual machines. (check out my repo [here](https://github.com/sravankaruturi/system-design) for all the code and any updates I make).
...
If you’d like to learn more of the concepts that are used when building large scale systems please follow me at @[Sravan Karuturi](@sravankaruturi).
