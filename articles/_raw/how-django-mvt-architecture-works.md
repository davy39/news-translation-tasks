---
title: 'How Django''s MVT Architecture Works: A Deep Dive into Models, Views, and
  Templates'
subtitle: ''
...
* [What is the MVT Architecture?](#heading-what-is-the-mvt-architecture)
    
* [The Model Component](#heading-the-model-component)
    
* [The View Component](#heading-the-view-component)
    
* [The Template Component](#heading-the-template-component)
    
* [Diagram Showing the MVT WorkFlow](#heading-diagram-showing-the-mvt-workflow)
    
* [Real-World Analogy of MVT](#heading-real-world-analogy-of-mvt)
    
* [Putting it All Together into a Project](#heading-putting-it-all-together-into-a-project)
...
Django models utilize a powerful feature called [Object-Relational Mapping (ORM)](https://www.freecodecamp.org/news/what-is-an-orm-the-meaning-of-object-relational-mapping-database-tools/), which bridges the gap between a relational database and Python code. It converts the Python objects (classes) into database tables, their attributes into columns, and instances into rows within those tables.
...
![MVT architecture diagram showing how data flows between client and server](https://cdn.hashnode.com/res/hashnode/image/upload/v1733727208715/06e27a48-ba51-421c-acc6-f0949b37a954.png align="center")
...
First, ensure you have Python installed. You can visit [Python’s Official Website](https://www.python.org/downloads/) to download the latest Python version.
...
In your `myapp`, create a file, name it `forms.py`, and insert this code:
...
In your `myapp/views.py`, insert this code:
...
* If the method is `POST`, it creates an instance of `TaskForm` using the submitted data ([`request.POST`](http://request.POST)).
    
* The form is then validated using [`form.is`](http://form.is)`_valid()`, and if valid, the form is saved to the database.
...
In your `myapp` directory, create a `templates` folder. Inside the templates folder, create a file and name it `task_list.html`. We need to add a form element that collects the user inputs and displays them in a list on the UI.

In the `task_list` HTML file, we have:
...
In your `myproject/urls.py`, include the URL in your `myapp`:
...
Add your `myapp` to the list of **Installed apps** in your `myproject/settings.py`:
...
Apply migration and run the server by entering these commands:
...
Visit [`http://127.0.0.1:8000/`](http://127.0.0.1:8000/items/) in your browser to test out your app.
...
![Final task app interface](https://cdn.hashnode.com/res/hashnode/image/upload/v1733688592866/d481df60-143e-42c7-acfe-0c329130c591.png align="center")
...
If you enjoyed reading this article, you can follow me on [X](https://x.com/SmoothTee_DC) or connect with me on [LinkedIn](https://www.linkedin.com/in/timothy-olanrewaju750/) for more programming articles and posts.
