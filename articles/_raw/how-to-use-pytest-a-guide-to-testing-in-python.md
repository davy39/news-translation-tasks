...
* [Why Use pytest?](#heading-why-use-pytest)
    
* [How to Write Your First Tests with pytest](#heading-how-to-write-your-first-tests-with-pytest)
    
* [How to Run pytest Tests](#heading-how-to-run-pytest-tests)
    
* [How to Interpret pytest Results](#heading-how-to-interpret-pytest-results)
    
* [How to Handle Exceptions in pytest](#heading-how-to-handle-exceptions-in-pytest)
    
* [Advanced pytest Features](#heading-advanced-pytest-features)
    
    * [1\. pytest Markers](#heading-1-pytest-markers)
        
    * [2\. pytest Fixtures](#heading-2-pytest-fixtures)
        
    * [3\. Parametrization](#heading-3-parametrization)
        
    * [4\. pytest Plugins](#heading-4-pytest-plugins)
...
![An image of pytest logo.](https://cdn.hashnode.com/res/hashnode/image/upload/v1751221734601/f5d6093a-37d2-4d49-85f2-c1a41a98ab67.png align="center")
...
**NOTE:** Following Python's best practices, it’s recommended you install pytest within a virtual environment. [Here's a guide](https://packaging.python.org/en/latest/guides/installing-using-pip-and-virtual-environments/) to help you set it up.
...
1. **Checking for** `ValueError`: In Python, a `ValueError` is raised when a function receives an argument with an incorrect value. In the example below, we can verify that a `ValueError` is raised when attempting to calculate the square root of a negative number.
...
2. **Checking for** `ZeroDivisionError`: Dividing a number by zero raises a `ZeroDivisionError`. In this example, we check that this error is raised when dividing a number by zero.
...
3. **Checking for** `TypeError`: A `TypeError` is raised when an operation is applied to an object of an inappropriate type. Here, we check that this error is raised when adding incompatible data types, such as a string and an integer given in the example.
...
4. **Checking for** `KeyError`: A `KeyError` is raised when we try to access a dictionary key that doesn’t exist. We can verify and handle this error using the following code:
...
While pytest comes along with some built-in markers, you can also create your own custom marker (but we won’t cover that in this tutorial). Kindly refer to the documentation for more information on [working with custom markers](https://docs.pytest.org/en/stable/example/markers.html)
...
pytest has a vast ecosystem of plugins, each designed to suit your different testing needs. You can find the full list of available plugins on [PyPI](https://pypi.org/) in the [pytest Plugin List](https://docs.pytest.org/en/stable/reference/plugin_list.html#plugin-list).
...
If you're ready to go a step further, I’ve written an in-depth article on [Test Driven Development in Python](https://judeolowo.hashnode.dev/test-driven-development-in-python-a-complete-guide-to-unittest). It is a powerful approach where writing tests guides your entire coding process.
