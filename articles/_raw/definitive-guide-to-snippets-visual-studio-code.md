---
title: Visual Studio Code Snippets – the Definitive VS Code Snippet Guide for Beginners
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
...
[Emmet](https://www.emmet.io/) is integrated into VS Code and has it's own CSS-selector inspired syntax for inserting HTML and CSS snippets. 

Emmet is it's own thing really, but the mechanics are the same. You can learn about Emmet with the [Emmet in Visual Studio Code guide](https://code.visualstudio.com/docs/editor/emmet).
...
These are the most important settings for snippets, but there are a few more. You can check out this [list of the default settings](https://code.visualstudio.com/docs/getstarted/settings#_default-settings) to explore more, or do a search in the Settings UI.
...
Long story short, I was frustrated by this scenario, so I wrote an extension called [**Snippets Ranger**](https://marketplace.visualstudio.com/items?itemName=robole.snippets-ranger) to give a nice UI to explore snippets easily. Think of it as a *Marauder's Map* for snippets!
...
The Visual Studio Marketplace has a [snippets category](https://marketplace.visualstudio.com/search?target=VSCode&category=Snippets&sortBy=Installs) where you can find snippets for almost anything.
...
VS Code's snippet syntax is the same as the [TextMate snippet syntax](https://manual.macromates.com/en/snippets). However, it does not support 'interpolated shell code' and the use of the `\u` transformation.
...
The format of a transformation is: `${«variable or placeholder»/«regex»/«replacement string»/«flags»}`. It is similar to [String.protoype.replace()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace) in JavaScript. The "parameters" do the following:
...
- `«replacement string»`: This is the string you want to replace the original text with. It can reference capture groups from the `«regex»`, perform case formatting (using the special functions: `/upcase`, `/downcase`, and `/capitalize`), and perform conditional insertions. See [TextMate Replacement String Syntax](https://macromates.com/manual/en/regular_expressions#replacement_string_syntax_format_strings) for more in-depth information.
- `«flags»`: Flags that are passed to the regular expression. The [JavaScript regex flags](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions#Advanced_searching_with_flags_2) can be used: 
  - `g` : Global search, 
...
| ------------------------------------------------------------ | ---------------------- | ---------------------- | ------------------------------------------------------------ |
| `["${TM_SELECTED_TEXT/^.+$/• $0/gm}"]`                       | line1<br>line2         | • line1<br>• line2     | Put a bullet point before each non-empty line of the selected text. |
...
| `[`<br>` "[",`<br>` "${CLIPBOARD/^(.+)$/'$1',/gm}",`<br>` "]"`<br>`]` | line1<br>line2         | ['line1', 'line2',]    | Turn the contents of the clipboard into a string array. Each non-empty line is an element. |
...
You define a shortcut by specifying the key combination you want to use, the command ID, and an optional [when clause context](https://code.visualstudio.com/docs/getstarted/keybindings#_when-clause-contexts) for the context when the keyboard shortcut is enabled. 

Through the `args` object, you can target an existing snippet by using the `langId` and `name` properties. The `langId` argument is the [language ID](https://code.visualstudio.com/docs/languages/identifiers#_known-language-identifiers) of the language that the snippets were written for. The `name` is the snippet's name as it is defined in the snippet file. 
...
What do I mean by abbreviated prefixes?  The table below list a few of the snippets from the [JavaScript (ES6) code snippets](https://marketplace.visualstudio.com/items?itemName=xabikos.JavaScriptSnippets) VS Code extension. You can see in the *Trigger* column, the prefixes listed are abbreviations, for example *fre* to represent a "for each" loop.
...
[AutoHotkey](https://www.autohotkey.com/) is a **free, open-source scripting language for Windows** to do all kinds of tasks. 

It has it's own unique syntax. You can install the [AutoHotKey extension](https://marketplace.visualstudio.com/items?itemName=slevesque.vscode-autohotkey) to add language support to VS Code for a better editing experience.
...
You can read the [docs](https://www.autohotkey.com/docs/AutoHotkey.htm) to learn more.

#### PhraseExpress (Windows, Mac, iOS)

[PhraseExpress](https://www.phraseexpress.com/) is "a text expander software, that manages frequently used text templates for insertion into any program". 
...
It uses a **file-based configuration approach**. The config files are written in [YAML](https://en.wikipedia.org/wiki/YAML).
