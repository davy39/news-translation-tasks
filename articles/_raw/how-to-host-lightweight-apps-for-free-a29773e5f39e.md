...
_As of May 2017, the programming languages you can host under Heroku_
...
_Running the script will return a static page with ‘hello world’ output_
...
_Once you run the “git push heroku master”, the deployment logs will be shown including the url where it is deployed_

For the command #1 (`heroku create…`), the “webechecker01” part is the unique name you’ll need to provide for the name of the app.

For command #3 (`git status`) this will tell you what files are ready to be deployed. Make sure all files are there, if not add them using `git add <filena`me>.
...
_The hello world program running on the web_
...
_“heroku logs” is a crucial command to run to see what’s going on within your app server_
...
_Creating the database, then using \d switch to see the list of tables and then “\d webecheckerdb” to see the columns_
...
_You can then do a select statement (include the semicolon at the end!) to see the data_
...
If you have any errors with missing libraries, you can add them via: `pip install <libra`ry> from the command line.
...
_If you get the DATABASE_URL error, then set the environment variable_
...
_SendGrid addon that can enable sending emails will be at the bottom_
...
_This is a great way to ensure your code is running as well on the heroku cloud as locally_
...
_The scheduler addon has been included as well which you can double click_
...
_You can schedule this with a few options._
...
_Thanks for reading! If you like what you read, hit the❤ button below so that others may find this (you can also find me on [Twitter](http://www.twitter.com/pubs12) )_
