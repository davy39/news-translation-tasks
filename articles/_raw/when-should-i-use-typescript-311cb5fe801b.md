...
_This article is now available in [Japanese](http://postd.cc/when-should-i-use-typescript/) and [Chinese](http://www.luxingyun.xyz/2016/08/17/%E4%BD%95%E6%97%B6%E5%BA%94%E8%AF%A5%E4%BD%BF%E7%94%A8typescript/)._
...
For standard ES6, it’s a whole different story. When [ES6 is supported by most browsers](https://kangax.github.io/compat-table/es6/), the current ES6 to ES5 transpilation will become unnecessary (update: [yes indeed](https://medium.freecodecamp.org/you-might-not-need-to-transpile-your-javascript-4d5e0a438ca)!).
...
3. The JavaScript code that comes out of the TypeScript transpiler is very readable. I am not a fan of Sourcemaps, so I do most of my debugging on the generated JavaScript. It’s absolutely awesome. I can totally understand why Angular 2 [chose TypeScript over Dart](https://jaxenter.com/angular-typescript-dart-115426.html).
4. TypeScript’s development experience is fantastic. [VS Code](https://code.visualstudio.com/) is very smart when dealing with JavaScript (some may argue it’s the smartest JS IDE). But TypeScript pushes the limits to a whole new level. The autocompletion and refactoring features in VSCode work much more accurately, and it’s not because the IDE is super smart. That’s all thanks to TypeScript.
5. The type annotations are like a basic level documentation. They declare the type of data that each function expects, their results and various other hints like `readonly` , `public`or `private` attributes. In my experience, trying to refactor a JavaScript code to TypeScript, I usually end up with cleaner code with nicer structure. In fact writing TypeScript has improved how I write plain JavaScript.
