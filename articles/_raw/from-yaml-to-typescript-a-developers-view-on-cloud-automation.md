---
title: 'From YAML to TypeScript: a developer’s view on cloud automation'
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
...
_URL Shortener with AWS Lambda and DynamoDB_
...
_API Gateway, Lambda, DynamoDB, and S3_
...
_All cloud resources to be provisioned_
...
The [AWS Command Line Interface](https://aws.amazon.com/cli/) (CLI) is a unified tool to manage all AWS services from a command prompt. You write the calls like this:
...
Here is a snippet from [a private URL shortener example](https://aws.amazon.com/blogs/compute/build-a-serverless-private-url-shortener/) kindly provided on the AWS blog:
...
Amazon’s [AWS Serverless Application Model](https://docs.aws.amazon.com/serverless-application-model/index.html) (SAM) looks very similar to the Serverless Framework but is tailored to be AWS-specific.
...
> _Infrastructure as code (IaC) is the process of managing and provisioning computer data centers through machine-readable definition files, rather than physical hardware configuration or interactive configuration tools._
...
Now, isn’t that beautiful? Only the essential options remained, while all the machinery is gone. Well, it’s not completely gone, it’s been hidden behind an _abstraction_.
...
The coolest thing here is that the actual _implementation code_ of AWS Lambdas is [intertwined](https://blog.pulumi.com/lambdas-as-lambdas-the-magic-of-simple-serverless-functions) with the _definition of resources_. The code looks very similar to an Express application. AWS Lambdas are defined as TypeScript lambdas. All strongly typed and compile-time checked.
...
_Originally published at [mikhail.io](https://mikhail.io/2019/02/from-yaml-to-typescript-developers-view-on-cloud-automation/)._
