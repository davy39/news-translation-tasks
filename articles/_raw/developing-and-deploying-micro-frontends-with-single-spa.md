...
In this article, we’re going to develop an app composed of micro-frontends using [single-spa](https://single-spa.js.org/) and deploy it to [Heroku](https://www.heroku.com/). 

We’ll set up continuous integration using [Travis CI](https://travis-ci.org/). Each CI pipeline will bundle the JavaScript for a micro-frontend app and then upload the resulting build artifacts to [AWS S3](https://aws.amazon.com/s3/). 
...
![Demo app — end result](https://www.freecodecamp.org/news/content/images/2020/07/mfe1.png)
_Demo app — end result_
...
Now, we’ll navigate in the browser to [http://localhost:9000](http://localhost:9000/) to view our app.

We should see… some text! Super exciting.

![Demo app — main page](https://www.freecodecamp.org/news/content/images/2020/07/mfe2.png)
_Demo app — main page_

On our main page, the navbar is showing because the navbar app is always active.

Now, let’s navigate to [http://localhost:9000/page1.](http://localhost:9000/page1.) As shown in our activity functions above, we’ve specified that the page 1 app should be active (shown) when the URL path begins with “page1.” So, this activates the page 1 app, and we should see the text for both the navbar and the page 1 app now.

![Demo app — page 1 route](https://www.freecodecamp.org/news/content/images/2020/07/mfe3.png)
_Demo app — page 1 route_

One more time, let’s now navigate to [http://localhost:9000/page2.](http://localhost:9000/page2.) As expected, this activates the page 2 app, so we should see the text for the navbar and the page 2 app now.

![Demo app — page 2 route](https://www.freecodecamp.org/news/content/images/2020/07/mfe4.png)
_Demo app — page 2 route_
...
This is because we haven’t actually specified _where_ each app should be mounted. The apps are simply loaded by [SystemJS](https://github.com/systemjs/systemjs), and then whichever app finishes loading fastest gets appended to the page first.
...
![Demo app — styled and using React Router](https://www.freecodecamp.org/news/content/images/2020/07/mfe5.png)
_Demo app — styled and using React Router_
...
I’ve named my bucket “single-spa-demo,” but you can name yours whatever you’d like. You can follow the AWS guides for [how to create a new bucket](https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingBucket.html) for more info.

![AWS S3 bucket](https://www.freecodecamp.org/news/content/images/2020/07/mfe6.png)
_AWS S3 bucket_

Once we have our bucket created, it’s also important to make sure the bucket is public and that [CORS (cross-origin resource sharing) is enabled for our bucket](https://docs.aws.amazon.com/AmazonS3/latest/dev/cors.html#how-do-i-enable-cors) so that we can access and use our uploaded assets in our app. 
...
![Uploaded files as a result of a Travis CI job](https://www.freecodecamp.org/news/content/images/2020/07/mfe8.png)
_Uploaded files as a result of a Travis CI job_
...
![Four directories within our S3 bucket](https://www.freecodecamp.org/news/content/images/2020/07/mfe9.png)
_Four directories within our S3 bucket_
...
So, using the original import map as a template, we can create a new file called `importmap.json`, place it _outside of our repos_ and add JSON that looks like this:
...
![Import map manually uploaded to the S3 bucket](https://www.freecodecamp.org/news/content/images/2020/07/mfe10.png)
_Import map manually uploaded to the S3 bucket_
...
![Demo app — up and running in production](https://www.freecodecamp.org/news/content/images/2020/07/mfe11.png)
_Demo app — up and running in production_
...
If we then navigate in our browser to [https://thawkin3-single-spa-demo.herokuapp.com/page1](https://thawkin3-single-spa-demo.herokuapp.com/page1), we’ll now see… drum roll please… our updated app!

![Demo app — successfully updating one of the micro-frontend apps](https://www.freecodecamp.org/news/content/images/2020/07/mfe12.png)
_Demo app — successfully updating one of the micro-frontend apps_
