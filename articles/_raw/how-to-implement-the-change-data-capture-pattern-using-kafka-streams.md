...
_Photo information storage architecture_
...
First we need [Kafka Connect](https://docs.confluent.io/current/connect/index.html). We can use the container provided by Confluence in the docker-compose file:
```yml
connect:
...
We also need to map a volume to the `/connect-plugins` path, where we will place the [Elasticsearch Sink Connector](https://docs.confluent.io/current/connect/kafka-connect-elasticsearch/index.html) to write to Elasticsearch. This is reflected also in the `CONNECT_PLUGIN_PATH`. 
