...
A helpful analogy is to think of variables as labels for our values. Think of a container of blueberries with a label on it marked blueberries. In this example, the variable, _blueberries_, points to a value, which is the blueberries themselves.
...
Scope refers to where in our code variables are available for use. When a variable is _globally scoped_, that means it is available anywhere in your program. Let’s look at an example. 
...
First, `const` is _block_-scoped, whereas var is _function_-scoped.

What is a **block**? 

A _block_ refers to any space between an opening and closing bracket. This might seem confusing at first. Let's write out our previous example, but this time using const instead of let when declaring our `doubleAge` variable.
...
Now, type `doubleAge` into your console and hit enter. You should get an error, `doubleAge is not defined.` This is because const is block-scoped: _it only exists in the block it was defined._ 
...
This is a strength of the `const` keyword that was introduced as an updated and better way of creating variables in JavaScript. However, what about the times when you _do_ want to update your variable? 
...
We cannot reassign our `age` variable, and `const` is working as expected. However, what if we _do_ want to reassign this variable?
