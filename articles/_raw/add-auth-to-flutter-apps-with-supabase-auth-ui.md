...
* A basic understanding of [Flutter](https://docs.flutter.dev/)
* Flutter [installed](https://docs.flutter.dev/get-started/install) and ready to go
* A basic understanding of [Backend-as-a-Service concepts](https://www.cloudflare.com/en-gb/learning/serverless/glossary/backend-as-a-service-baas/)
* A basic understanding of [authentication](https://www.freecodecamp.org/news/set-up-authentication-in-apps-with-supabase/)
* An IDE (Integrated Developer Environment) or a [text editor](https://code.visualstudio.com/download) to work in
...
_A code snippet showing a Flutter `pubspec.yaml` file with dependencies. The `supabase_auth_ui` dependency is highlighted with a pink arrow, indicating version `^0.4.4`._
...
Go to your Supabase dashboard or [create an account](https://supabase.com/) if you don't have one. In the dashboard, go to Project settings at the bottom then click on API under configuration. Copy the URL and the project anon key from the right hand side of the page as illustrated below:

![Image](https://cdn.hashnode.com/res/hashnode/image/upload/v1716932440241/1f5c49a5-5ef8-449f-8bec-557e0492e950.png)
_A screenshot of the API settings page in Supabase dashboard. The left sidebar shows various menu options under "Project Settings," "Configuration," and "Billing." The "API" option under "Configuration" is highlighted. The main section displays fields for "Project URL" and "Project API keys," with options to copy or reveal the keys._

Back in your Flutter app, create a `.env` file at the root of the folder and paste the following, replacing with the values you copied above:
...
Add the environment file to the .`gitignore` file to keep it out of version control, then add the `flutter_dotenv` package to the list of dependencies right below `supabase_auth_ui` dependency. Finally add the `.env` file as a path under the assets key in the pubspec.yaml file. 
...
_A screenshot of a code editor displaying a `pubspec.yaml` file for a Flutter project. The file lists dependencies such as `flutter`, `supabase_auth_ui`, and `flutter_dotenv`. Two pink arrows point to the `supabase_auth_ui: ^0.4.4` and `flutter_dotenv: ^5.1.0` dependencies. The file also includes sections for `dev_dependencies`, `flutter_lints`, and `assets`._
...
This loads the `.env` file and initialises Supabase.
...
Create a `splash_screen.dart` file in the `lib` folder and paste the following in it:
...
Finally, create `auth_screen.dart` in the `lib` folder and paste the following into it:
...
_Supabase Auth UI login screen with fields for email and password, options to sign in, reset password, sign up, and buttons to continue with Google or GitHub._
...
1. Create a new OAuth App [here](https://github.com/settings/developers), fill in the required information, and add the copied URL under the authorization URL field.
2. Add the homepage URL as "[http://localhost:3000/](http://localhost:3000/)".
...
1. Create a new [Google cloud project](https://www.cloud.google.com/).
...
_Authorization screen for "Test Flutter auth ui" requesting access to a GitHub account. Options to "Cancel" or "Authorize FatumaA" are available._
...
_Supabase authentication interface with an email input field and a 'Continue with magic Link' button._
...
_A mobile app screen displaying the Supabase Auth UI on an iOS emulator. The interface includes fields for entering an email and password, a "Sign In" button, options for password recovery, account sign-up, and login buttons for Google and GitHub._
...
_A login interface for Supabase Auth UI. It includes fields for entering an email and password, with icons indicating their respective purposes. Below the fields is a purple "Sign In" button. Links for "Forgot your password?" and "Don't have an account? Sign up" are located at the bottom._
...
_A login interface for Supabase Auth UI with fields for email and password, a "Sign In" button, options to reset the password or sign up, and buttons to continue with Google or GitHub layout vertically with text and icons._

To this:

![Image](https://cdn.hashnode.com/res/hashnode/image/upload/v1717081856997/ec454669-c11c-4861-83cc-9cd7ca805140.png)
_Login screen for Supabase Auth UI with fields for email and password, a "Sign In" button, options for password recovery, account creation, and login via Google or GitHub in circular icons layout horizontally_

### Localization and Translation Ready

Supabase auth UI is [translation ready](https://github.com/supabase-community/flutter-auth-ui/pull/76), and you can change the labels to any language you wish.
...
_A login interface titled "Supabase Auth UI" with fields for entering email and password in different languages. The email field is labeled "Ingiza barua pepe yako" and the password field is labeled "Ingresa tu contraseña." There is a purple login button with Arabic text. Below the button, there are links for forgotten password and account registration in Swahili and Arabic._
...
_A login interface titled "Supabase Auth UI" with fields for email and password. The email field contains "ggg" and has an error message in multiple languages. The password field is empty with a placeholder in Spanish. Below, there is a purple login button with text in Arabic. Additional text in Swahili and Arabic is present below the button._
...
It is open-source and always looking for more contributions. Remember to leave a star on the [repository](https://github.com/supabase-community/flutter-auth-ui).

## Resources

Here are some links that might be useful:

* [Supabase docs on native deep linking](https://supabase.com/docs/guides/auth/native-mobile-deep-linking?platform=flutter&queryGroups=platform&queryGroups=os&os=android#setting-up-deep-linking)
* [Supabase Flutter auth UI docs](https://supabase.com/docs/guides/auth/auth-helpers/flutter-auth-ui)
* [Supabase video on Google Sign In](https://www.youtube.com/watch?v=utMg6fVmX0U)
