...
To see how to create a service account and give it access to the Google Play Publisher API, look [here](https://developers.google.com/identity/protocols/oauth2/service-account).
...
The main thing to figure out is how to sign the Android App Bundle. Google has fairly decent documentation on it that you can find [here](https://developer.android.com/studio/build/building-cmdline#sign_cmdline).
...
Google has documentation around this that you can find [here](https://developer.android.com/studio/publish/app-signing#secure-shared-keystore).
...
Now, we need to load this file into `build.gradle`. This came as a surprise to me initially, but Gradle actually [works as a DSL](https://docs.gradle.org/current/dsl/index.html). So, it makes it easier to write configuration using Gradle.
...
To learn how to set up a Slack app, view the documentation [here](https://api.slack.com/start/overview). The process is super easy and you should have an app up and running in a few minutes. 
