---
title: Data Analysis with Python – How I Analyzed My Empire State Building Run-Up
  Performance
subtitle: ''
...
A [tower running race](https://en.wikipedia.org/wiki/Tower_running) is a race that you run up the stairs of a building. These happen around the world. I got the chance to participate in the Empire State Run Up in NYC, 2023 edition.
...
I had a blast and got great satisfaction from having this race ticked off my bucket list, the same way I felt after running the [NYC Marathon](https://results.nyrr.org/event/40/finishers#search=Jose%2520Nunez%2520Zuleta).
...
You should have some experience in a programming language to get the most out of this article. My code is written in Python (you will need version 3.8+) and runs on Linux (I used [Fedora 37 distribution](https://fedoraproject.org/)).
...
I strongly recommend that you [get the source code](https://github.com/josevnz/tutorials/tree/main/docs/EmpireStateRunUp) (It is [Open Source](https://github.com/josevnz/tutorials/tree/main?tab=Apache-2.0-1-ov-file#readme)!). Get your hands dirty, break the scripts, and have fun. You will need Git to clone the repository:
...
I decided to use [Selenium Web Driver](https://www.selenium.dev/documentation/webdriver/) as it calls a real browser, like Firefox, to navigate the website. Selenium allows you to automate browser actions while you get the same rendered HTML you see when you navigate the site.
...
[Getting the data](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/test/raw_data.csv) is just the first battle of many more to come. [You will notice inconsistencies on the data](https://en.wikibooks.org/wiki/Statistics/Data_Analysis/Data_Cleaning) and missing values. In order to make your numeric results good, you need to make assumptions.

Luckily for me, the dataset is very small (375+ records, one for each runner) so I was able to come up with a few rules to tidy up the [data file](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/results-first-level-2023.csv) I was going to use during my analysis.

I also supplemented my data with another data set that has the [3-digit country codes](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/country_codes.csv) as well as other details, for a nicer presentation.
...
By all means, we are not done massaging the data. A simple function takes care of this stage inside the [data](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/data.py) module:
...
I strongly recommend that you check out [10 minutes to pandas](https://pandas.pydata.org/pandas-docs/stable/user_guide/10min.html) if you are not familiar with the tool. For my DataFrame, I made the BIB an index as it is unique, and it has no special value for aggregation functions – but the 'id' attribute is unique.
...
* A few entries did not have the gender defined. That affected other fields like 'gender\_position'. To avoid distortions, these were filled with the median.
...
All the analysis logic [was kept together on a single module called 'analyze'](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/analyze.py), separate from presentation, data loading, or reports, to promote reuse.
...
> The unittest unit testing framework was originally inspired by JUnit and has a similar flavor as major unit testing frameworks in other languages. It supports test automation, sharing of setup and shutdown code for tests, aggregation of tests into collections, and independence of the tests from the reporting framework. (From the [Python docs](https://docs.python.org/3/library/unittest.html))

I tried to have a simple [unit test](https://docs.python.org/3/library/unittest.html) for every method I wrote on the code. This saved me lots of headaches down the road. As I refactored the code, I found better ways to get the same results, producing correct numbers.
...
So far we got the data, and made sure [it meets the expectations](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/test/test_data.py). I wrote [separate tests](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/test/test_analyze.py) for the analytics code and also for the scraper.
...
I wanted to use the terminal as much as possible to visualize my findings, and to keep requirements to a minimum. I decided to use the [Textual](https://textual.textualize.io/) framework to accomplish that.
...
Let's see some code. One of the applications is called `esru_outlier`. TUI code lives on the [apps](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/apps.py) module that shows several tables together with the outliers we found before, using the z-score.
...
![Screen shot of the OutlierApp table that shows outliers on the race results](https://www.freecodecamp.org/news/content/images/2024/05/esrm_outlier_first_screen.png align="left")
...
![Screen shot of the OutlierApp runner details that shows outliers on the race results](https://www.freecodecamp.org/news/content/images/2024/05/esrm_outlier_runner_detail.png align="left")
...
We can customize the appearance using CSS (yes, like a web application). It looks a lot like a web application's [CSS](https://en.wikipedia.org/wiki/CSS) (but it's not exactly the same). For example to add style to a button, here's the code:
...
As you can see, Textual is a pretty powerful framework. It reminds me a lot of [Java Swing](https://en.wikipedia.org/wiki/Swing_(Java)), but without the extra complexity.
...
I wanted to use some charts to display the data, and I made them with [matplotlib](https://matplotlib.org/). The code to generate an age plot box, that shows how old the participating runners were, is very straightforward.

![Box plot showing age distribution among racers](https://www.freecodecamp.org/news/content/images/2024/05/esru_age_box_plot.png align="left")
...
Finally, it's time to get familiar with mini applications (you can see an animated [demonstration of the TUI applications here](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/EmpireStateRunUp.svg)).
...
![Raw runners data in a table](https://www.freecodecamp.org/news/content/images/2024/05/esru_browser.png align="left")

*The esru\_browser window shows all runners' results. Here you can sort, search for runners, and click to get more details*

And the command palette allows searching for runners by name (it's basically a search bar with fuzzy logic):

![race_runners_2023-12-31T18_35_53_558956.svg, searching for runners by name](https://www.freecodecamp.org/news/content/images/2024/05/race_runners_2023-12-31T18_35_53_558956.svg align="left")
...
![Statistics of interest, like average age, wave they belong, finishing time](https://www.freecodecamp.org/news/content/images/2024/05/esru_numbers.svg align="left")

*esru\_numbers gives a bird's eye view of all the racers, categorized by buckets*
...
![Table with outliers details](https://www.freecodecamp.org/news/content/images/2024/05/esru_outlier-1.svg align="left")

*the esru\_outlier main screen shows you racers that did not follow regular patterns*

Because these results drill down to the BIB number, you can click on a row and get more details about a runner:

![Outlier racer details, including BIB](https://www.freecodecamp.org/news/content/images/2024/05/esru_outlier-2.svg align="left")
...
The [esru\_plot](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/apps.py) application offers a few plot graphics to help you visualize the data. Inside, the class `Plotter` does all the heavy lifting

#### Age plots

The program can generate two flavors for the same data, one is a Box diagram:

![Age plot, Pie chart](https://www.freecodecamp.org/news/content/images/2024/05/esru_age_box_plot-1.png align="left")

*The age box diagram we saw before*

The second is a regular histogram:

![Age histogram](https://www.freecodecamp.org/news/content/images/2024/05/age_histogram.png align="left")
...
![Histogram](https://www.freecodecamp.org/news/content/images/2024/05/participants_per_country.png align="left")
...
![Gender pie](https://www.freecodecamp.org/news/content/images/2024/05/gender_distribution.png align="left")
...
* You saw just the tip of what you can do with Textual. I encourage you to explore the [apps.py](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/apps.py) module. Take a look at the [example applications](https://github.com/Textualize/textual/tree/main/examples) as well.
    
* [Selenium Web driver](https://www.selenium.dev/documentation/webdriver/) is not just a tool for web scraping but for automated testing of web applications. It doesn't get better than having your browser perform automated testing for you. It is a big framework, so be prepared to spend time reading and running your tests. I strongly suggest you look [at the examples](https://github.com/SeleniumHQ/seleniumhq.github.io/tree/trunk/examples/python). Trial an error will give you better results.
