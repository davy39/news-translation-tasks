...
You’ll notice that the _project path_ field in the terminal defaults to the root directory. Surge assumes that the whichever directory you’re running the `surge` command in is the directory you wish to deploy. In your case, you need to point Surge to the `build` directory created when you ran `npm run build` .
...
_initial output after running`npm run build`_
...
_new output after adding a `homepage` field in `package.json`_
...
_An example of my React todo list using the gh-pages branch_
...
* Centralizes all projects underneath your _<username>.gith_ub.io domain
...
_Thanks for reading! I’m a self-taught developer & spend most of my free time diving into front-end tools and [writing my way out](https://www.jakewiesler.com/). Reach out to me on [Twitter](https://twitter.com/jakewies) if you have any questions!_
