...
co_authors: []
series: null
date: '2024-06-19T13:52:18.000Z'
originalURL: https://freecodecamp.org/news/for-else-loop-in-python
coverImage: https://www.freecodecamp.org/news/content/images/2024/06/fimg-1-.png
tags:
- name: Loops
  slug: loops
- name: Python
  slug: python
seo_title: null
seo_desc: 'Python supports the for-else loop construct that is lesser known but super
  helpful.

  If you’ve programmed in Python, you may have used the for loop to iterate over items
  in iterables such as lists. But for some use cases, using the for loop in conjunc...'
---
...
Here, the `is_prime()` function first checks if the input number `n` is less than or equal to 1. If so, it returns `False` since prime numbers are greater than 1. Remember, the smallest prime number is 2.

We use the `for` loop to iterate over the range of numbers from 2 to the square root of `n` (inclusive). 
...
### 📑 A Note on Checking for Prime Numbers
