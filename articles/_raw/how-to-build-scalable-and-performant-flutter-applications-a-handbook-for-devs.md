---
title: 'How to Build Scalable and Performant Flutter Applications: A Handbook for
  Devs'
subtitle: ''
...
4. [State Management: Single Source of Truth and Isolation](#heading-state-management-single-source-of-truth-and-isolation)
    ...
    * [Provider example (simple, explicit state updates)](#heading-provider-example-simple-explicit-state-updates)
        
    * [BLoC (flutter\_bloc) example: separating UI events from state logic](#heading-bloc-flutterbloc-example-separating-ui-events-from-state-logic)
        
    * [Riverpod Short Example (modern, testable, no context)](#heading-riverpod-short-example-modern-testable-no-context)
        
5. [Minimize Widget Rebuilds: Techniques and Patterns](#heading-minimize-widget-rebuilds-techniques-and-patterns)
    
    * [Narrowing Rebuild Scope with ValueListenableBuilder](#heading-narrowing-rebuild-scope-with-valuelistenablebuilder)
        
    * [Avoiding Expensive setState on Entire Screens](#heading-avoiding-expensive-setstate-on-entire-screens)
        
    * [Code Optimization: Idioms and Examples](#heading-code-optimization-idioms-and-examples)
        
    * [Using final and const Properly](#heading-using-final-and-const-properly)
...
10. [Platform-Specific Code and Native Integration](#heading-platform-specific-code-and-native-integration)
    ...
    * [Platform Channels Example (Dart + Android)](#heading-platform-channels-example-dart-android)
        
    * [Android (Kotlin) side (embedding code sample):](#heading-android-kotlin-side-embedding-code-sample)
        
    * [Native libraries with dart:ffi](#heading-native-libraries-with-dartffi)
...
11. [Code Splitting and Lazy Loading: Reduce Initial App Weight](#heading-code-splitting-and-lazy-loading-reduce-initial-app-weight)
    ...
    * [Deferred Import Example (Dart)](#heading-deferred-import-example-dart)
...
14. [Performance Profiling and Tooling](#heading-performance-profiling-and-tooling)
    ...
    * [How to Approach Profiling: A Workflow](#heading-how-to-approach-profiling-a-workflow)
        
15. [Network Optimization](#heading-network-optimization)
    
    * [Use a Robust HTTP Client: Example with Dio](#heading-use-a-robust-http-client-example-with-dio)
        
    * [Caching and Compression](#heading-caching-and-compression)
...
17. [Testing: Quality Gates for Scalability](#heading-testing-quality-gates-for-scalability)
    ...
    * [Widget Tests](#heading-widget-tests)
...
20. [App Distribution and Build-size Optimization](#heading-app-distribution-and-build-size-optimization)
    ...
    * [Production Checklist for Build Size:](#heading-production-checklist-for-build-size)
...
29. [References and Further Reading](#heading-references-and-further-reading)
...
If you are new to Flutter, make sure you have it installed by following the [official Flutter installation guide](https://flutter.dev/docs/get-started/install).
...
4. `const Greeting({super.key, required` [`this.name`](http://this.name)`});`: The constructor is marked `const`. This allows Flutter to create canonical instances of `Greeting` at compile time when its inputs (like `name` if it's a compile-time constant) are also compile-time constants.
...
* **Medium to large apps with event-driven logic:** BLoC (flutter\_bloc) offers a robust and highly testable solution, especially when dealing with complex asynchronous flows and clear event-state transitions.
...
### BLoC (flutter\_bloc) Example: Separating UI Events from State Logic
...
For Android and web platforms, Flutter also provides more advanced "deferred components" (Android App Bundles and web deferred loading) to download entire code and asset packages when needed. You can find more comprehensive details in the [official Flutter documentation on deferred components](https://docs.flutter.dev/data-and-backend/deferred-components) and their platform-specific requirements.
...
* **Firebase Analytics for event tracking**: Firebase Analytics is a free and powerful tool for tracking user engagement and behavior. You can log custom events (for example, 'item\_added\_to\_cart', 'feature\_x\_used'), track screen views, and analyze user demographics. This data helps you understand feature usage, user flows, and identify areas for improvement.
...
* [Deferred components and deferred loading in Flutter (official docs)](https://docs.flutter.dev/data-and-backend/deferred-components)
    
* [Code splitting and deferred imports in Flutter/Dart (community guides)](https://insight.vayuz.com/code-splitting-and-deferred-loading-in-flutter-and-dart/)
    
* [Dart FFI and DynamicLibrary for native libraries](https://api.flutter.dev/flutter/dart-ffi/DynamicLibrary-class.html)
    
* [Packages for lazy loading and UI lazy strategies on](https://pub.dev/flutter/packages?q=lazy) [pub.dev](http://pub.dev) (for example, `flutter_lazy_loading`, `lazy_indexed_stack`)
    
* [Dio package on](https://pub.dev/packages/dio) [pub.dev](http://pub.dev) and other widely used packages on [pub.dev](http://pub.dev).
