...
In [**Build Your First Vue.js Component**](https://frontstuff.io/build-your-first-vue-js-component) we made a star rating component. We’ve covered many fundamental concepts to help you create more complex Vue.js components.
...
A few things have changed since the [initial tutorial](https://frontstuff.io/build-your-first-vue-js-component). [Vue CLI 3](https://cli.vuejs.org/) was released. [Vue Test Utils](https://vue-test-utils.vuejs.org/) — the official Vue.js unit testing utility library — has matured to beta version. In the first tutorial, we used [webpack-simple](https://github.com/vuejs-templates/webpack-simple), a prototyping template that doesn’t include testing features. For these reasons, the simplest thing to do is to wipe the slate clean and migrate the project from the tutorial to a more recent Vue.js install.

I re-created the project from the first tutorial so you can download it directly from [GitHub](https://github.com/sarahdayan/star-rating-vue-js-tutorial). Then, navigate to the unzipped directory and install dependencies.

**Note:** make sure you install [Node.js](https://nodejs.org/) before going further:
...
For this tutorial, we’ll use [Vue Test Utils](https://vue-test-utils.vuejs.org/), the official Vue.js testing toolkit, along with [Jest](https://jestjs.io/), a JavaScript test runner backed by Facebook.
...
Vue CLI 3 (which I used to generate the [boilerplate](https://github.com/sarahdayan/star-rating-vue-js-tutorial)) allows you to pick your favorite test runner, and sets it up for you. If you want to use another test runner (like [Mocha](https://mochajs.org/)), install [Vue CLI 3](https://cli.vuejs.org/) and generate your own starter project. Then you can migrate the source files from [my boilerplate](https://github.com/sarahdayan/star-rating-vue-js-tutorial) right in it.
...
This is also the official recommendation from the [Vue Test Utils guides](https://vue-test-utils.vuejs.org/guides/#common-tips). Therefore, we’ll only test what we can access from the outside of the component:
...
Let’s analyze what’s happening here. First, we’re using Jest’s `[expect](https://jestjs.io/docs/en/expect#expectvalue)` function, which takes the value we want to test as an argument. In our case, we call the `[findAll](https://vue-test-utils.vuejs.org/api/wrapper/#findall-selector)` method on our `wrapper` to fetch all elements with an `active` class. This returns a `[WrapperArray](https://vue-test-utils.vuejs.org/api/wrapper-array/)`, which is an object that contains an array of `[Wrappers](https://vue-test-utils.vuejs.org/api/wrapper/)`.
...
Here, we first get our fourth star with `findAll` and `[at](https://vue-test-utils.vuejs.org/api/wrapper-array/#at-index)`, which returns a `Wrapper` from a `WrapperArray` at the passed index (zero-based numbering). Then, we simulate the `click` event on it — we’re mimicking the action from a user who would click or tap the fourth star.

Since we set the `grade` prop to 3, the fourth star should be inactive before we click, therefore the click event should make it active. In our code, this is represented by a class `active` which we append on stars only when they’re activated. We test it by calling the `[classes](https://vue-test-utils.vuejs.org/api/wrapper/#classes-classname)` method on the star, which returns its class names as an array of strings. Then, we use the `[toContain](https://jestjs.io/docs/en/expect#tocontainitem)` matcher to make sure the `active` class is here.
...
As their names suggest, `[beforeEach](https://jestjs.io/docs/en/api#beforeeachfn-timeout)` and `[afterEach](https://jestjs.io/docs/en/api#aftereachfn-timeout)` run before and after each test, respectively. This way we can be 100% sure we’re using a fresh wrapper whenever we run a new test.
...
One way to handle this is to create a [custom Vue directive](https://vuejs.org/v2/guide/custom-directive.html).

The Vue instance has a `directive` method which takes two arguments — a **name**, and an **object of functions** for each [hook of the component lifecycle](https://vuejs.org/v2/guide/custom-directive.html#Hook-Functions) when injected in the DOM. You can also pass a single function if you don’t care about a specific hook.
...
I won’t go into the detail of each test, because they all share a similar structure. You can find the [full spec file on GitHub](https://github.com/sarahdayan/star-rating-vue-js-tutorial/blob/tests/tests/unit/Rating.spec.js), and I strongly recommend you try to implement them yourself first. Software testing is an art as much as it is a science, and requires twice as much practice as it requires theory.

Don’t worry if you didn’t get everything, or if you struggle with writing your first tests: **testing is notoriously hard**. Also, if you have a question, don’t hesitate to hit me up on [Twitter](https://twitter.com/frontstuff_io)!

Originally published at [frontstuff.io](https://frontstuff.io/unit-test-your-first-vuejs-component).
