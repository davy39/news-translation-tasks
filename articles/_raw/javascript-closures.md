...
There is a reason why it can be difficult to make sense of closures—because they are often taught _backwards_. You may have been taught what a closures is, but you might not understand how they are useful to the average developer or within your own code.
...
1. The `handleLikePost` outer function is executed, creating an instance of the inner function `addLike`; that function _closes_ over the variable `likeCount`, which is one scope above.
...
_This is why closure makes functions so powerful_, because it is a special property that isn't present in anything else in the language.
