...
You can easily clone GitHub repositories without entering your password each time by using HTTPS with your [15-minute cached credentials](https://help.github.com/en/articles/caching-your-github-password-in-git) or, my preferred method, by [connecting to GitHub with SSH](https://help.github.com/en/articles/connecting-to-github-with-ssh). For brevity I’ll assume we’re going with the latter, and our SSH keys are set up.
...
GitLab, unlike GitHub, lets us do this nifty thing where we don’t have to use the website to make a new repository first. We can [create a new GitLab repository from our terminal](https://gitlab.com/help/gitlab-basics/create-project#push-to-create-a-new-project). The newly created repository defaults to being set as Private, so if we want to make it Public on GitLab, we’ll have to do that manually later.
