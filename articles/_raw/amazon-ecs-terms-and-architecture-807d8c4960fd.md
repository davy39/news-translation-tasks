...
_Adapted from Docker’s ‘[bins/libs](https://docs.docker.com/get-started/#container-diagram" rel="noopener" target="_blank" title="">get started</a>’, see here for ‘<a href="https://en.wikipedia.org/wiki/Filesystem_Hierarchy_Standard" rel="noopener" target="_blank" title=")’_
...
> _…a highly scalable, fast, container management service that makes it easy to run, stop, and manage Docker containers on a cluster._
...
_One Task Definition creates several identical Tasks_
...
_Service definition defining alarms of when to scale capacity_
...
_One ECS Container Instance running 8 Tasks from multiple different Services_
...
_An example ECS cluster, with one Service running four Tasks across two ECS Container Instances_
...
_Multiple Services allocated across multiple ECS Container Instances running on one Cluster_
...
_A Cluster running 3 Services, each running a different amount of Tasks, across two ECS Container Instances_
