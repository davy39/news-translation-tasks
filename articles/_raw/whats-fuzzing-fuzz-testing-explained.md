...
I was recently looking through some of Google's open source [repositories on their GitHub](https://github.com/google). And I saw that they had a repository for continuous fuzzing. I had no idea what fuzzing even was, let alone continuous fuzzing.
...
It's quite an integrated test, too, as you can test the complete flow of storing something like a [zero-width space](https://en.wikipedia.org/wiki/Zero-width_space) (U+200B in Unicode) in your site to check for issues. 
...
Have a look at [this](https://github.com/minimaxir/big-list-of-naughty-strings) GitHub called the **Big List of Naughty Strings**. It is a list of strings that have a high probability of causing issues. 
...
For example, there are some strings that are written upside-down "uʍopǝpᴉsd∩" which you can do [here](http://www.upsidedowntext.com). There are strings that may be flagged as profanity or inappropriate but are actually innocent (this is called the [S[cunt](https://en.wikipedia.org/wiki/Scunthorpe_problem)horpe problem](null)). Or even strings that can reveal system files if they are parsed by a poorly configured XML parser.
...
Just getting it to crash isn't the end goal here. Once those hackers know what kind of inputs cause crashes, they can investigate why those things cause crashes and see if they can utilise those exploits to do something sinister, or access something they shouldn't have access to. You can read more about the above example [here](https://www.wired.com/2016/06/hacker-lexicon-fuzzing/).

Google [currently](https://google.github.io/clusterfuzz/) **fuzzes** their own applications on 30,000 VM's! So you aren't likely to have any progress due to how extensively they already **fuzz**.

Google's [OSS-Fuzz](https://github.com/google/oss-fuzz) has found over 25,000+ bugs in Google Chrome code, and roughly 22,500 bugs in other open source codebases that use OSS-Fuzz.
...
If you want to research deeper into this topic, you can find a huge list of resources [here](https://github.com/secfigo/Awesome-Fuzzing) on GitHub. There's everything from courses, articles, videos and tools to help you learn how to **fuzz**.
