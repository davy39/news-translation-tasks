...
> _This article was originally posted on [My Blog](https://hakaselogs.me/2018-04-20/building-a-web-app-with-go-gin-and-react)_
...
We have our API in place, so let’s build a frontend to present the data from our API. For this, we’ll be using React. We won’t go too deep into React, as it will be out of the scope of this tutorial. If you need to learn more about React, checkout the official [tutorial](https://facebook.github.io/react/docs/tutorial.html). You can implement the UI with any frontend framework you’re comfortable with.
...
We can easily set up authentication in our GIN app by using Auth0. You’ll need an account to follow along with this part. If you don’t already have an Auth0 account, [sign up](https://auth0.com/signup) for one now.

> _Disclaimer: This isn’t sponsored content._

### Creating the API client

Our tokens will be generated with Auth0, so we need to create an API and a Client from our Auth0 dashboard. Again, if you haven’t already, [sign up](https://auth0.com/signup) for an Auth0 account.
...
To create a new Client, navigate to the [clients section](https://dev.to/codehakase/building-a-web-app-with-go-gin-and-react-5ke) in your dashboard, and click the **Create Client** button. Select the type `Regular Web Applications`.
...
> _You can find the `AUTH0_CLIENT_ID`, `AUTH0_DOMAIN`, and `AUTH0_API_AUDIENCE` data from your Auth0 [management dashboard](https://manage.auth0.com/)._

We need to set a `callback` which Auth0 redirects to. Navigate to the Clients section in your dashboard. In the settings, let's set the callback to `[http://localhost:3000](http://localhost:3000:)`[:](http://localhost:3000:)
...
You can say Hi to me on Twitter [@codehakase](https://twitter.com/codehakase)
