...
In this article, you will learn how to create an interactive terminal-based portfolio and a résumé in JavaScript. We'll use the [jQuery Terminal library](https://terminal.jcubic.pl/) (and a few other tools) to create a website that looks like a real terminal.
...
* [What is the terminal and its history?](#heading-what-is-the-terminal-and-its-history)
    
* [What is jQuery Terminal?](#heading-what-is-jquery-terminal)
    
* [Base html file](#heading-base-html-file)
    
* [How to Initialize the Terminal](#heading-how-to-initialize-the-terminal)
    
* [Welcome message](#heading-welcome-message)
    
* [Line Gaps](#heading-line-gaps)
    
* [How to Add Colors to ASCII Art](#heading-how-to-add-colors-to-ascii-art)
    
    * [Terminal Formatting](#heading-terminal-formatting)
        
    * [How to Use the Lolcat Library](#heading-how-to-use-the-lolcat-library)
        
    * [Rainbow ASCII Art Greetings](#heading-rainbow-ascii-art-greetings)
        
    * [How to Make the Greeting Text White](#heading-how-to-make-the-greeting-text-white)
        
* [How to Make Your First Command](#heading-how-to-make-your-first-command)
    
* [Default Commands](#heading-default-commands)
    
* [How to Make Help Commands Executable](#heading-how-to-make-help-commands-executable)
    
* [Syntax Highlighting](#heading-syntax-highlighting)
    
* [Tab Completion](#heading-tab-completion)
    
* [How to Add Shell Commands](#heading-how-to-add-shell-commands)
    
* [How to Improve Completion](#heading-how-to-improve-completion)
    
* [Typing Animation Command](#heading-typing-animation-command)
    
* [Credits Command](#heading-credits-command)
    
* [Prefilled Commands](#heading-prefilled-commands)
    
* [Sharing Link to Terminal Session](#heading-sharing-link-to-terminal-session)
    
* [Working Terminal Portfolio Demo](#heading-working-terminal-portfolio-demo)
    
* [Adding Executables to Home Directory](#heading-adding-executables-to-home-directory)
    
* [What Next?](#heading-what-next)
    
* [Share what you’ve created](#heading-share-what-youve-created)
    

## What is the terminal and its history?

Terminals have a long history. It started as an upgrade from [punch cards](https://en.wikipedia.org/wiki/Punched_card). Computers back in the day used teletypes, which was just a keyboard and a printer. You'd type on the keyboard, and the keystrokes would be sent to the computer (usually mainframe) and the output got printed on a printer.
...
If you're new to the command line, you can read this article: [Command Line for Beginners – How to Use the Terminal Like a Pro \[Full Handbook\]](https://www.freecodecamp.org/news/command-line-for-beginners/).

## What is jQuery Terminal?

jQuery Terminal is a JavaScript library. It's a plugin for the [jQuery library](https://en.wikipedia.org/wiki/JQuery). jQuery Terminal is more like a framework that has jQuery as its dependency. We'll mostly use JavaScript and very little jQuery in this article.
...
The first argument to the terminal method is called an interpreter. It's a way to add your commands. An object is the simplest way to create them. See [creating the interpreter](https://github.com/jcubic/jquery.terminal/wiki/Getting-Started#creating-the-interpreter) to learn more.
...
The first thing we need to do is to get rid of the default greeting message and replace it with nice-looking custom [ASCII Art](https://en.wikipedia.org/wiki/ASCII_art). We will use the [Filget library](https://en.wikipedia.org/wiki/FIGlet) written in JavaScript to do this.

There are a few Figlet libraries on npm. We will use a package named [figlet](https://www.npmjs.com/package/figlet).

The first thing you should do is pick the right font. Go to [figlet playground](https://patorjk.com/software/taag/) and write the text you want for your greeting. We will use "Terminal Portfolio" and click "Test All". It should display your text with all the fonts. Scroll through the list and pick the font you like.

I picked a font "slant" that looks like this:

![Terminal Portfolio ASCII Art](https://www.freecodecamp.org/news/content/images/2024/04/Przechwycenie-obrazu-ekranu_2024-04-26_22-18-26.png align="left")
...
**NOTE**: If you run this code, you will notice that there is an empty line after the ASCII art. This is because the figlet library adds some spaces after the text.
...
![Image: ASCII Art with line gaps](https://www.freecodecamp.org/news/content/images/2024/05/Przechwycenie-obrazu-ekranu_2024-05-08_14-06-41.png align="left")

You can remove the gaps by adding the `ansi` option set to `true`. The option was added specifically to fix an issue with displaying [ANSI Art](https://en.wikipedia.org/wiki/ANSI_art).
...
![Image: ASCII Art with gaps removed](https://www.freecodecamp.org/news/content/images/2024/05/Przechwycenie-obrazu-ekranu_2024-05-08_14-57-16.png align="left")

## How to Add Colors to ASCII Art

You can spice up your ASCII Art by using a library called lolcat. lolcat is a Linux command that can style text in the terminal with rainbow colors. There is also a library called [isomorphic-lolcat](https://www.npmjs.com/package/isomorphic-lolcat), that you can use in JavaScript to make your ASCII Art in rainbow colors.
...
The whole text is wrapped in brackets and the formatting of the text is in additional brackets, where each argument is separated by a semicolon. To learn more about the syntax, you can read the Wiki Article: [Formatting and Syntax Highlighting](https://github.com/jcubic/jquery.terminal/wiki/Formatting-and-Syntax-Highlighting).
...
You'll notice that when you resize the window, the rainbow changes randomly. This is the default behavior of lolcat. To change it, you need to set the [random seed](https://en.wikipedia.org/wiki/Random_seed).
...
The XML formatting supports more tags like links and images. See [Extension XML Formatter](https://github.com/jcubic/jquery.terminal/wiki/Formatting-and-Syntax-Highlighting#extension-xml-formatter) for more info.
...
This will be our help command where we'll add a list of commands available to our terminal portfolio. We will use [Intl.ListFormat](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/ListFormat), which creates a list of elements with and before the last element.
...
Next is to add [affordance](https://en.wikipedia.org/wiki/Affordance). To indicate that the user can click the command, we need to change the cursor in CSS:
...
If you would like to make command arguments in different colors, you'll need a function, where you will use [String::replace()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace).
...
**NOTE**: If you add the class `<white class="command">` to the formatter, you will be able to click on the typed command to execute it again.
...
**NOTE**: if you have long command and want to get the text for that command, it's better to use: `$(this).data('text')`. When the single formatting is wrapped (when text is longer than the width of the terminal) the `.text()` will no longer have full text, but the full text is always in the `data-text` HTML attribute.
...
We will use the [Joke API](https://jokeapi.dev/) for this purpose.
...
You can read more about typing animation in this article: [Typing Animation](https://github.com/jcubic/jquery.terminal/wiki/Typing-Animation)[.](https://github.com/jcubic/jquery.terminal/wiki/Typing-Animation#sequence-of-animations)
...
This is an example of another way to print something on the terminal – if you return something from a function it will be printed. You can also return a [Promise](https://www.freecodecamp.org/news/javascript-promises-explained/), so you can send an [AJAX](https://en.wikipedia.org/wiki/Ajax_\(programming\)) request to the server and print the results.
...
Another cool thing that I will show you is recording commands in the URL. You can create whole terminal session and save it in a [URL hash](https://developer.mozilla.org/en-US/docs/Web/API/URL/hash). To start recoding a session you need to execute the following:
...
To share the link, it’s better to use a URL shortener like [**TinyURL**](https://tinyurl.com/)**.** Make sure you test the shortened URL to see if it works.
...
Here is a fully working Demo of our [Interactive Terminal Portfolio Website](https://codepen.io/jcubic/full/ZEZPWRY).
...
* CodePen Collection with [jQuery Terminal Demos](https://codepen.io/collection/LPjoaW).
    
* [Retro (Vintage) Terminal CodePen Demo](https://codepen.io/jcubic/pen/BwBYOZ).
    
* [jQuery Terminal Examples Page](https://terminal.jcubic.pl/examples.php).
    
* [Terminal 404 Error Page](https://terminal.jcubic.pl/404).
    
* [Fake GNU/Linux Terminal](https://fake.terminal.jcubic.pl/).
    

If you have an idea that is not listed here, you can ask on [StackOverflow with jquery-terminal tag](https://stackoverflow.com/questions/tagged/jquery-terminal). If you have something more time consuming, you can also ask for [paid support](https://support.jcubic.pl/).

## Share what you’ve created

If you create a cool terminal portfolio, you can [share it and tag me on Twitter](http://twitter.com/jcubic). I would love to take a look. Especially if you create something more than what’s included in the tutorial. You can also share on a [terminal chat on my website](https://jcu.bi/chat) (it’s a similar terminal portfolio, but with chat).
