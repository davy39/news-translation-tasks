...
In this handbook, you’ll dive into one such advanced concept in Next.js called [Streaming](https://nextjs.org/learn/dashboard-app/streaming). Technically, this is a feature from React.js, but when used correctly with Next.js, it can massively improve the User Experience of any web application.

After reading this, you’ll understand what streaming is and how it works. You’ll also be able to implement a super smooth User Experience on your own website. And your app? It’ll feel blazing fast and perform like a champ! So, without any further ado, let’s get started.

## Here’s What We’ll Cover

* [Prerequisites](#heading-prerequisites)
    
* [What is Streaming](#heading-what-is-streaming)?
    
* [Why Streaming Matters](#heading-why-streaming-matters)
    
* [How Streaming Works in Next.js 15](#heading-how-streaming-works-in-nextjs-15)
    
* [Project Setup — Demo SSR Page](#heading-project-setup-demo-ssr-page)
    
    * [Home Page](#heading-home-page)
        
    * [Streaming Demo Page](#heading-streaming-demo-page)
        
    * [ToolsCards Component](#heading-toolscards-component)
        
    * [Simulate Fake Delay With getTools() Function](#heading-simulate-fake-delay-with-gettools-function)
        
    * [Installing Shadcn Card Component](#heading-installing-shadcn-card-component)
        
    * [IconCard Component](#heading-iconcard-component)
        
    * [IconComponent](#heading-iconcomponent)
        
    * [LikeButton Client Component](#heading-likebutton-client-component)
        
* [Discovering SSR Issues – UX and False Interaction](#heading-discovering-ssr-issues-ux-and-false-interaction)
    
    * [Breaking Down SSR Issues](#heading-breaking-down-ssr-issues)
...
* [Next.js Automatic Streaming – loading.js](#heading-nextjs-automatic-streaming-loadingjs)
    ...
    * [Create the loading.js File](#heading-create-the-loadingjs-file)
        
    * [Structure the Loading Skeleton Component](#heading-structure-the-loading-skeleton-component)
        
    * [CardSkeleton Component](#heading-cardskeleton-component)
        
    * [Render the Cards](#heading-render-the-cards)
        
    * [How Automatic Streaming was Applied](#heading-how-automatic-streaming-was-applied)
        
    * [Issues with Next.js Automatic Streaming](#heading-issues-with-nextjs-automatic-streaming)
        
* [Manual Streaming with Custom Suspense Boundary](#heading-manual-streaming-with-custom-suspense-boundary)
    
    * [Remove Promise.all()](#heading-remove-promiseall)
        
    * [How to Implement Suspense for Concurrent Data Fetching in Next.js Components](#heading-how-to-implement-suspense-for-concurrent-data-fetching-in-nextjs-components)
        
    * [Summary of Steps to Implement Manual Streaming in Next.js](#heading-summary-of-steps-to-implement-manual-streaming-in-nextjs)
        
    * [Final Demo](#heading-final-demo)
...
When you use React’s Server Components, Next.js can begin rendering your component tree on the server. Wherever you introduce a boundary (or implicitly via a `loading.js` file), React can pause, flush the HTML up to that point, and immediately stream it to the browser.

### 2\. Automatic vs Manual Streaming

With automatic streaming, you drop a `loading.js` file alongside any route or layout segment. Next.js will detect it, render your loading skeleton first, and stream in the rest of the page as data becomes available.
...
We’ll cover both Automatic Streaming with `loading.js` and Manual Streaming via custom Suspense boundaries, so you can choose the pattern that fits your need. You’ll also have hands-on code examples to make your Next.js 15 site feel lightning fast.
...
Once the development server is running, open the `app/page.js` file and update it with the following code:
...
Now let’s create the `streaming-demo` page. Create another `page.js` file inside `app/streaming-demo` folder and write the below code inside it:
...
Inside the above `ToolsCard` component, tools are fetched using a function called `getTools()`. Now let’s write the `getTools()` function inside a file called `lib/getTools.js`:
...
The `getTools()` function is a [Server Function](https://nextjs.org/docs/app/getting-started/updating-data#what-are-server-functions). It maps over an array called `TOOLS`. If you check that `TOOLS` array, it’s just a simple array of strings – names of different tools like JavaScript, React, Vue, and so on.
...
Now get back in the `tools-card.js` file, and you’ll see that you’re getting an array of `Promises` from `getTools()`. Then, you’re passing that array to `Promise.all()`, which resolves all the `Promises` together. Finally, you get back an array of strings – one for each tool – but each one had a delay before resolving.
...
Now, using the `toolsWithData` array, you’re running a map() again, and for each tool (which is just a string), you’re rendering an `IconCard` component. The `tool` name is passed as a prop into `IconCard`. IconCard can be just a simple presentational component that renders a card. You can use the [`Card`](https://ui.shadcn.com/docs/components/card) component from the [Shadcn](https://ui.shadcn.com/) UI library.
...
Now create a new file inside the `components` folder called `icon-card.js` and write the below code inside it:
...
Create a new file `components/icon-component.js` and write the below code to it:
...
Inside the `icon-card.js` file, there’s another component called `LikeButton`. Let’s write the code for this:
...
![Discovering SSR Issues](https://cdn.hashnode.com/res/hashnode/image/upload/v1754078491159/c3c17d5d-add0-4a1b-973e-cdcd7f4b7533.gif align="center")
...
![False Interaction - SSR](https://cdn.hashnode.com/res/hashnode/image/upload/v1754078793243/44fcd1d1-a58f-427c-a2f3-a3c5749fb3fc.gif align="center")
...
Now, if these terms (like **Page Rendering** and **Hydration**) aren’t clear to you, I recently made a [full video covering Next.js rendering and Caching](https://youtu.be/g3nj8SIO7Vs). It’s a must-watch for any Next.js, React, or general web developer. You’ll learn how your web page is rendered, how Hydration works, and how the whole rendering process is coordinated between browser and server.

Everything will become crystal clear to you. So make sure you watch that video if any of this feels confusing.

![Server Side Rendering (SSR)](https://cdn.hashnode.com/res/hashnode/image/upload/v1754079434726/99146526-1387-405d-9b1d-228d29b27128.png align="center")
...
![SSR Blocking Behaviour](https://cdn.hashnode.com/res/hashnode/image/upload/v1754079470587/15e69637-a83f-4abe-bc0f-1e5c9fc86782.png align="center")
...
![SSR - Bad UX](https://cdn.hashnode.com/res/hashnode/image/upload/v1754079647605/23b58563-1583-464b-b4d2-1d3c837d8c3c.gif align="center")
...
![Streaming in Action](https://cdn.hashnode.com/res/hashnode/image/upload/v1754079758843/1f1130f4-351a-4b80-8205-b56550c53232.png align="center")
...
Now here's something important: streaming only works with **Server Components**. To implement this, React gives us a tool called [React Suspense](https://react.dev/reference/react/Suspense). You’ll now learn how to use React Suspense to upgrade your current demo and see how you can take this streaming experience to the next level.

## Two Types of Streaming in Next.js

Let me bring you back to the code again. The first thing you’re going to do is get the demo I showed you earlier – that’s our starter code. I’ve saved that exact starter code in the `starter` branch of the [GitHub repository](https://github.com/logicbaselabs/nextjs-streaming).

So from the `starter` branch, you’ll get the exact code you’re starting with. Now, to implement streaming, you’ll start with Next.js’s default Streaming system. There are two ways to do Streaming in Next.js:
...
I’ll first show you the Automatic Streaming demo. Then I’ll walk you through how custom streaming works. Each approach will live in its own separate branch. The primary starter code stays in the `starter` branch. Now create a new branch and call it `“automatic-streaming”`, for the Automatic Streaming demo. All the code changes I make from now will go into this automatic branch.
...
First, inside the `streaming-demo` folder, you’ll create a `loading.js` file. Inside that, you’ll return regular JSX like in any standard React page.
...
Now you need to insert 9 cards inside this layout just like the UI. To understand what each card should look like, let’s open up the `icon-card.js` file. You’ll notice each card is just an `IconCard`, right? So you’ll use that same structure to build your loading skeletons.

To save time, let me share code for the `CardSkeleton` component inside the `components`/ui folder – named `card-skeleton.jsx`. Inside that component, I’m using the [`Skeleton`](https://ui.shadcn.com/docs/components/skeleton) component from Shadcn. Pretty straightforward!
...
So in reality, you didn’t do much. You just created a `loading.js` file inside the `app/streaming-demo` folder. And that alone triggered Automatic Streaming in Next.js. How?

Let’s take a look. Remember what you did? You created a `loading.js` file, right? Because of this file structure, behind the scenes Next.js automatically wraps the page component with a React Suspense boundary.

![Automatic Streaming with loading.js](https://cdn.hashnode.com/res/hashnode/image/upload/v1754079977767/5aea584f-3045-4539-ac67-e647e3afb304.png align="center")
...
> You don’t have to do all the heavy lifting. Just place a `loading.js` file in the folder – I’ll handle the rest for you.
...
![Serial Rendering](https://cdn.hashnode.com/res/hashnode/image/upload/v1754080238122/8e3ca78a-cc44-4f31-806f-3c1554bdc26c.png align="center")
...
![Concurrent Rendering](https://cdn.hashnode.com/res/hashnode/image/upload/v1754080259781/4612905e-516d-452b-a6ec-97c0ef3c9ec1.png align="center")
...
Done! You’ll now find the code under the `'automatic-streaming'` branch in the [GitHub repo](https://github.com/logicbaselabs/nextjs-streaming/tree/automatic-streaming). Now let’s move on to learning **Custom Streaming**. For that, I’m creating a new branch:

```bash
git checkout -b custom-streaming
```

Our new branch `“custom-streaming”` is ready. Let’s start our Next.js dev server:
...
![Final Demo - Manual Streaming Demo](https://cdn.hashnode.com/res/hashnode/image/upload/v1754080577713/2d97114e-a1b1-49f7-8b65-2aa8bd7e66e8.gif align="center")
...
![No False Interaction in Streaming SSR](https://cdn.hashnode.com/res/hashnode/image/upload/v1754080760808/7223cdae-1107-47d8-b477-0a33d4b98e1e.gif align="center")

With just a small change, you now have a complete Streaming experience. If you want to stream at the `page` level, just use a `loading.js` file. That way, the whole page shows a loading state covering the entire area.
...
Why? Because there’s nothing dynamic going on here. So according to Next.js’s Rendering logic, this becomes an **SSG (Static Site Generation)** page. Right? If you’re not familiar with **SSG** or **SSR**, please do check out [the video](https://youtu.be/xTT_Sd_xqh0) I recommended earlier. It explains everything clearly.
...
Now, if this page were a true Server Side rendered page, then Streaming would make a lot more logical sense. So how can you force that? Easy! At the top of the `app/streaming-demo/page.js` file, just add this line:
...
You can find all the source code from this tutorial in [this GitHub repository](https://github.com/logicbaselabs/nextjs-streaming). If it helped you in any way, consider giving it a star to show your support!
