...
_Golang popularity according to Google Trends_
...
[GoDoc](https://godoc.org/) is a static code analyzing tool that creates beautiful documentation pages straight out of your code. A remarkable thing about GoDoc is that it doesn’t use any extra languages, like JavaDoc, PHPDoc, or JSDoc to annotate constructions in your code. Just English.
...
All you can do is to add a good old `// MyFunc transforms Foo into Bar` kind of comment which would be reflected in the documentation, too. You can even add [code examples](https://blog.golang.org/examples) which are **actually runnable** via the web interface or locally.
...
Here, for example, is the GoDoc page for my recent pet project: [pullkee — GoDoc](https://godoc.org/github.com/kirillrogovoy/pullkee).

### Static code analysis

Go heavily relies on static code analysis. Examples include [godoc](https://godoc.org/) for documentation, [gofmt](https://golang.org/cmd/gofmt/) for code formatting, [golint](https://github.com/golang/lint) for code style linting, and many others.

There are so many of them that there’s even an everything-included-kind-of project called [gometalinter](https://github.com/alecthomas/gometalinter#supported-linters) to compose them all into a single utility.
...
You can learn more from this talk: [GothamGo Kickoff Meetup: Go Static Analysis Tools by Alan Donovan](https://vimeo.com/114736889).
...
You might already know about Goroutines, which are used in Go to achieve concurrent code execution. If you don’t, [here’s](https://gobyexample.com/goroutines) a really brief explanation.
...
You can read more about it and learn how to use it here: [Introducing the Go Race Detector — The Go Blog](https://blog.golang.org/race-detector).
...
The project has [marvelous documentation](https://golang.org/doc/), and most of the advanced topics have already been covered on their blog: [The Go Programming Language Blog](https://blog.golang.org/).
...
Moreover, Go [doesn’t implement a concept called Generics](https://golang.org/doc/faq#generics) which makes it even more challenging to work with multiple types in an abstract way. Nevertheless, many people think it’s actually beneficial for the language because of the amount of complexity Generics bring along. And I totally agree.
...
In order to pull that off, you need a tool to examine all the data in runtime that acts differently depending on its type and structure. Reflection to rescue! Go has a first-class [reflect](https://golang.org/pkg/reflect/) package to enable your code to be as dynamic as it would be in a language like Javascript.

An important caveat is to know what price you pay for using it — and only use it when there is no simpler way.

You can read more about it here: [The Laws of Reflection — The Go Blog](https://blog.golang.org/laws-of-reflection).

You can also read some real code from the JSON package sources here: [src/encoding/json/encode.go — Source Code](https://golang.org/src/encoding/json/encode.go)
...
Please, give Go a try with [A Tour of Go](https://tour.golang.org) which is an incredible place to start.
...
Check out my [Github](https://github.com/kirillrogovoy/) and follow me on [Twitter](https://twitter.com/krogovoy)!
