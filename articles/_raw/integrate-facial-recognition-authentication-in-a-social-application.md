---
title: How to Integrate Facial Recognition Authentication in a Social App with Face
  API
subtitle: ''
author: Oluwatobi
co_authors: []
...
* [What to expect](#heading-what-to-expect)
    
* [Prerequisites](#heading-prerequisites)
    
* [A Brief Intro to the Face API tool](#heading-a-brief-intro-to-the-face-api-tool)
    
* [Project Setup](#heading-project-setup)
    
* [Demo Project: Integrating Facial Recognition and Authentication](#heading-demo-project-integrating-facial-recognition-and-authentication)
    
* [Additional Information and Tips](#heading-additional-information-and-tips)
...
In this article, I’ll walk you through creating a multi-factor authentication system for a chat application powered by [Stream.io](https://getstream.io), and ensuring efficient user face ID authentication to allow only authorized access to your app. I will illustrate all these with relevant code examples.
...
* [Stream.io](https://getstream.io) API key
    

Before we get started, we’ll briefly highlight the facial authentication tool of choice: [Face-Api.js](https://justadudewhohacks.github.io/face-api.js/docs/index.html).
...
In addition to all these features, it's friendly to use and can also be used locally with its predefined models. Here is a link to its [documentation page](https://justadudewhohacks.github.io/face-api.js/docs/index.html), which provides relevant code examples.

It provides features such as face detection, face capture, and face match, which use the [Euclidean algorithm](https://en.wikipedia.org/wiki/Euclidean_algorithm) to make precise distinctions. We'll now set it up alongside our chat application project in the next section.
...
After successful completion, we are finally done with the project structure scaffold. To aid ease of local testing of our frontend application, we will have to host the face models needed by the Face package locally. Here is a [link](https://github.com/justadudewhohacks/face-api.js-models) to the models. Kindly copy the model's folder and paste it into the public folder in the code project. Next, we’ll set up our backend project.

### Backend

The backend is built to store user details and ensure user authentication before accessing the chat application. MongoDB will be the database of choice, and we will use the Express.js library as the backend API development environment of choice. For the ease of setup, kindly clone this [code-base](https://github.com/oluwatobi2001/stream-backend.git) and install it on the local PC. It comes preloaded with the necessary installation files. To further enjoy a seamless backend experience, you can utilize the MongoDB [Atlas](https://www.mongodb.com/products/platform/atlas-database) option as the database for storing user details. With that, we will now begin the code project in the next section.
...
In the code snippet above, we imported useful React hooks and initialized our installed `Face-api.js` tool. [Axios](https://www.npmjs.com/package/axios) will serve as our API request tool of choice for this project. The `useRef` hook will be used to track the user inputs. We then defined the register function and initialized the various `useRef` hooks for the various input fields to be inputted.
...
![facial authentication page ](https://lh7-rt.googleusercontent.com/docsz/AD_4nXcPFsPVo9dymrTmMCyskCszbMf_SdG2n_j5gd7ayT1nQ6jOlhX8a_KFRG51cnqMCxUqFaVgTR2hrdGipmudd9B2TQpNfm4FrFMlYRo7bbu1gtRq1bKB5FmPi4QcbEPTLyDtAPbNEA?key=bLpVfispbJQQ4phtxWLC7w align="left")
