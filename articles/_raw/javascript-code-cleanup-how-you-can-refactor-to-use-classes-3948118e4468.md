...
I made a big demo for this post. You can see the code [on Github](https://github.com/AmberWilkie/class-demo). The initial commit shows all of the functionality inside the main component (`App.js`) and the subsequent commits refactor the code to use a class.
...
You can imagine that this becomes really useful if we have a lot of these methods to deal with. In my case, I needed to call `.then(res => myFunction(r`es) after everything I was trying to extract. Defini`ng myFunct`ion on the class instance instead of passing it to each function saved me a lot of code.
