---
title: 'How to Use the Compound Components Pattern in React: Prop Soup to Flexible
  UIs'
subtitle: ''
...
This is going to be a complete hands-on tutorial. So get your favourite code editor ready, and let’s get started. This article is also available as a video tutorial as part of the [15 Days of React Design Patterns](https://www.youtube.com/playlist?list=PLIJrr73KDmRyQVT__uFZvaVfWPdfyMFHC) initiative. Please check it out.

%[https://www.youtube.com/watch?v=LglWulOqh6k] 

## Table of Contents

1. [React 19 Code Set Up](#heading-react-19-code-set-up)
    
2. [A Messy Modal Component](#heading-a-messy-modal-component)
    
3. [The Problems with this messy Modal Component](#heading-the-problems-with-this-messy-modal-component)
    
4. [The Compound Components Pattern](#heading-the-compound-components-pattern)
    
5. [How to Build a Modal Component using the Compound Components Pattern](#heading-how-to-build-a-modal-component-using-the-compound-components-pattern)
    
    * [Why Didn’t We Create Separate Files for the SubComponents?](#heading-why-didnt-we-create-separate-files-for-the-subcomponents)
        
    * [How to Use the Modal Component](#heading-how-to-use-the-modal-component)
        
    * [How to build an Accordion Component using the Compound Components Pattern](#heading-how-to-build-an-accordion-component-using-the-compound-components-pattern)
        
    * [Add the Accordion to the Modal](#heading-add-the-accordion-to-the-modal)
        
6. [The Use Cases](#heading-the-use-cases)
    
7. [The Pitfalls and Anti-Patterns](#heading-the-pitfalls-and-anti-patterns)
    
8. [15 Days of React Design Patterns](#heading-15-days-of-react-patterns)
    
9. [Before We End...](#heading-before-we-end)
...
Note: you can find all the source code used in this tutorial on the [tapaScript GitHub](https://github.com/tapascript/15-days-of-react-design-patterns/tree/main/day-03/compound-components-patterns). Feel free to follow along with it side by side.
...
If you get an output with the installed Node.js version, you are all set. Otherwise, just download and install Node.js from [here](https://nodejs.org/en/download).
...
Now the project should be running locally and should be accessible on the default URL, [`http://localhost:5173`](http://localhost:5173). You can access the URL in your browser. Now we’re all set to start coding.

## A Messy Modal Component

Let’s get started by creating a Modal component. Start by creating a directory called `messy` under the `src/` directory. Now, create a file called `Modal.jsx` under `src/messy/` with the following code snippet:
...
Next, open the `App.jsx` file and replace the existing code with the following code snippet:
...
1. `Lack of Flexibility`: The modal has a rigid structure that dictates exactly what it renders. What if you want a modal without a title? Or a modal with a custom layout? Or more than two action buttons? You need to write additional logic and pass additional props every time you think of enhancing the modal for another use case. These changes in the component will bring maintainability issues and increase code smell.
    
2. `Mixed Responsibilities`: The modal tries to do multiple things. It handles both layout and content. This violates the separation of concerns principle that we learn from other design patterns, such as the [Container-Presenter Pattern](https://www.youtube.com/watch?v=1UHbhikwg-s).
    
3. `Hard Reusability`: The modal lacks reusability due to its rigidness. Right now, if you want a modal with this:
...
4. `Poor Scalability`: The modal component is not scalable. Think, for example, if you’re creating a component library and you end up creating multiple modal instances like ConfirmationModal, InfoModal, FormModal, ImageModal, and so on. It would be a huge ding on the scalability of that component library to create and maintain every new version of the modal.
    
5. `Hard to Test`: This modal implementation is hard to test due to its tight coupling with props.
...
```javascript
// File Location: src/with-pattern/modal/Modal.jsx
...
Open the `App.jsx` file and replace the content of it with the following code snippet:
...
Create a folder called `accordion` under `src/with-pattern` folder. Now, create a file called `Accordion.jsx` inside the `src/with-pattern/accordion` with the following code snippet:
...
Great, let’s now use the Accordion component. Create a new file called `AccordionDemo.jsx` under the folder `src/with-pattern/accordion` with the following code snippet:
...
I have some great news for you! After the *40 days of the JavaScript* initiative, I have now started a brand new initiative called [15 Days of React Design Patterns](https://www.youtube.com/playlist?list=PLIJrr73KDmRyQVT__uFZvaVfWPdfyMFHC).
...
* Subscribe to my fortnightly newsletter, [The Commit Log](https://tapascript.substack.com/subscribe?utm_medium=fcc).
