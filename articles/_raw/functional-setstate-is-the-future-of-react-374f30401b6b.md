...
Going by [Dan Abramov](https://www.freecodecamp.org/news/functional-setstate-is-the-future-of-react-374f30401b6b/undefined)’s words in describing this pattern, **Functional setState** is a pattern where you
...
First, “_multiple `setState()` calls”_ could mean calling `setState()` inside a single function more than once, like this:
...
Now when React, encounters “_multiple `setState()` calls”,_ instead of doing that “set-state” **three whole times**, React will avoid that huge amount of work I described above and smartly say to itself: “No! I’m not going to climb this mountain three times, carrying and updating some slice of state on every single trip. No, I’d rather get a container, pack all these slices together, and do this update just once.” And that, my friends, is **batching**!

Remember that what you pass to `setState()` is a plain object. Now, assume anytime React encounters “_multiple `setState()` calls”,_ it does the batching thing by extracting all the objects passed to each `setState()` call, merges them together to form a single object, then uses that single object to do `setState()` .
...
Here is a pen by [Sophia Shoemaker](https://www.freecodecamp.org/news/functional-setstate-is-the-future-of-react-374f30401b6b/undefined) that demos this problem. Play with it, and pay attention to both the bad and the good solutions in this pen:
...
So, when React encounters “_multiple `functional setState()` calls” ,_ instead of merging objects together, (of course there are no objects to merge) React _queues_ the functions “_in the order they were called.”_
...
[_A Play with the code in this bin will be fun. Remember! we’re just faking React to get the idea..._jsbin.com](http://jsbin.com/najewe/edit?js,console)
...
Thanks to [Dan Abramov](https://www.freecodecamp.org/news/functional-setstate-is-the-future-of-react-374f30401b6b/undefined)!
