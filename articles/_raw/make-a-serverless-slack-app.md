...
For this tutorial, I will be using AWS Lambda, so first, we'll create an [AWS account](https://aws.amazon.com/). I find AWS's UI hard to understand and difficult to navigate, so I will be adding screenshots for each step.
...
Finally, we'll add these credentials to command line AWS. Use this [guide](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-install.html) to get aws cli setup.
...
I have the default region as `us-east-2` already set up, but you can use [this](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.RegionsAndAvailabilityZones.html) to determine what your region is.
...
If you had trouble following the steps, you can also check out [AWS's documentation](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html).

---

## Set up serverless

Go to your terminal and install the `serverless` package globally using `npm`: `npm i -g serverless`. ([More info on serverless here](https://serverless.com/))  
...
Next, we'll explore actually using serverless by building our slack app. Our slack app will post a random [Ron Swanson](https://en.wikipedia.org/wiki/Ron_Swanson) quote to slack using a [slash command](https://api.slack.com/slash-commands) like this:
...
I'm using [this API](https://github.com/jamesseanwright/ron-swanson-quotes#ron-swanson-quotes-api?ref=public-apis) to generate Ron Swanson quotes since the docs are fairly simple (and of course, it's free). To see how requests are make and what gets returned, you can just put this URL in your browser:
...
_Spoiler: There was a wrong way to consume alcohol_
...
To create your slack app, follow this [link](https://api.slack.com/apps?new_app=1). It'll make you sign into a slack workspace first, so be sure you're a part of one that you can add this app to. I've created a testing one for my purposes. You'll be prompted with this modal. You can fill in whatever you want, but here's what I have as an example:
...
_Display information can be found from the "Basic Information" tab on the app_
...
Scroll below that to the section that says "[Environment Variables](https://docs.aws.amazon.com/lambda/latest/dg/env_variables.html)", and put your Slack OAuth Access Token here (you can name the key whatever you'd like):
...
`process.env` just grabs our environment variables ([additional reading](https://nodejs.org/dist/latest-v8.x/docs/api/process.html#process_process_env)). Next, let's take a look at the [Slack API](https://api.slack.com/methods/chat.postMessage) to figure out how to post a message to a channel.
...
_We're going to add an API Gateway (as I already have)._
...
_where's waldo: spot the param edition_
...
_Final Repo with code:_ [https://github.com/lsurasani/ron-swanson-slack-app/](https://github.com/lsurasani/ron-swanson-slack-app/)
