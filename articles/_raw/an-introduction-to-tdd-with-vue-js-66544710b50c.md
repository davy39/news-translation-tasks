...
**Testing a component can be counter-intuitive**. As we saw in [Unit Test Your First Vue.js Component](https://frontstuff.io/unit-test-your-first-vuejs-component), it requires a mental shift to wrap your head around testing components versus testing plain scripts, knowing what to test, and understanding the line between unit tests and end-to-end.

**TDD makes all this easier**. Instead of writing tests by examining all bits and pieces of a finished project and trying to guess what you should cover, you’re doing the opposite. You’re starting from actual specs, a list of things that the component should _do_, without caring about how it does it. This way, you’re ensuring that all you test is the public API, but you’re also guaranteeing you don’t forget anything.
...
_Design inspired from [Chris Castillo](https://dribbble.com/shots/2908891-Custom-Color-Picker-Exploration" rel="noopener" target="_blank" title="">Custom Color Picker Exploration</a> by <a href="https://dribbble.com/_ChrisCastillo" rel="noopener" target="_blank" title=")_
...
In this article, we’ll deep dive into TDD. We’ll [put some specs together](https://frontstuff.io/an-introduction-to-tdd-with-vuejs#write-down-your-specs) before we write a single line of code. Then, we’ll [test every public feature](https://frontstuff.io/an-introduction-to-tdd-with-vuejs#write-test-driven-code) in a test-driven fashion. Finally, we’ll reflect on what we did and [see what we can learn from it](https://frontstuff.io/an-introduction-to-tdd-with-vuejs#afterthoughts).

### Before we start

This tutorial assumes you’ve already built something with Vue.js before, and written unit tests for it using [Vue Test Utils](https://vue-test-utils.vuejs.org/) and [Jest](https://jestjs.io/) (or a similar test runner). It won’t go deeper into the fundamentals, so make sure you get up to speed first. If you’re not there yet, I recommend you go over [Build Your First Vue.js Component](https://frontstuff.io/build-your-first-vue-js-component) and [Unit Test Your First Vue.js Component](https://frontstuff.io/unit-test-your-first-vuejs-component).

**_TL;DR:_** _this post goes in-depth in the how and why. It’s designed to help you understand every decision behind testing a real-world Vue.js component with TDD and teach you how to make design decisions for your future projects. If you want to understand the whole thought process, read on. Otherwise, you can go directly to the [afterthoughts](https://frontstuff.io/an-introduction-to-tdd-with-vuejs#afterthoughts) at the end, or look at the final code on [GitHub](https://github.com/sarahdayan/colorpicker-tdd-tutorial)._
...
First, you need to create a new Vue project with [Vue CLI](https://cli.vuejs.org/). You can check [Build Your First Vue.js Component](https://frontstuff.io/build-your-first-vue-js-component) if you need a step by step guide.
...
We’ll need to use SVG files as components, so you also need to install the right loader for them. Install [vue-svg-loader](https://www.npmjs.com/package/vue-svg-loader) as a dev dependency, and add a rule for it in your `vue.config.js` file.
...
This loader doesn’t play well with Jest by default, which causes tests to throw. To fix it, create a `svgTransform.js` file [as documented on the website](https://vue-svg-loader.js.org/faq.html#how-to-use-this-loader-with-jest), and edit your `jest.config.js` as follows:
...
Additionally, you need to install [color-convert](https://www.npmjs.com/package/color-convert) as a dependency. We’ll need it both in our code and in our tests later on.
...
When tests re-run, they should still pass ? This means w**e’ve successfully refactored the code without affecting the output.** Congratulations, you’ve just completed your first TDD cycle!
...
It’s understandable that you’re feeling confused by the process. Yet, try to look at things from a different angle: the point here isn’t to _prove_ that the test doesn’t pass. We know it won’t. What we want to look at is what our test _expects_, make them happy in the simplest possible way, and finally write smarter code without breaking anything.
...
Another question that may come to mind is **how we’re deciding what to test**. In [Unit Test Your First Vue.js Component](https://frontstuff.io/unit-test-your-first-vuejs-component), we saw that we should only be testing the public API of our component, not the internal implementation. Strictly speaking, this means we should cover **user interactions** and **props changes**.
...
One solution could be to access the component’s data to iterate on the modes dynamically. Vue Test Utils lets us do that through the [vm](https://vue-test-utils.vuejs.org/api/wrapper/#properties) property, but again, this tightly couples our test with the internal implementation of the modes. If tomorrow, we decided to change the way we define modes, the test would break.

Another solution is to keep going with black box testing and only expect the class name to match a given _pattern_. We don’t care that it’s `color-mode-hex`, `color-mode-hsl` or `color-mode-xyz`, as long as it looks like what we expect from the outside. Jest lets us do that with regular expression matchers.
...
Then, run the app by executing the following script, and open it in your browser at `[http://localhost:8080/](http://localhost:8080/)`.
