...
This article includes a step-by-step video walkthrough that I made for you. You can watch the video on my [YouTube channel](https://www.youtube.com/@FahimAmin).

%[https://www.youtube.com/watch?v=LADT0Y_IcNU] 

## Table of Contents

* [Requirements For Self-Hosting Coolify](#heading-requirements-for-self-hosting-coolify)
    
* [Set Up an AWS EC2 Instance For Coolify](#heading-set-up-an-aws-ec2-instance-for-coolify)
    
* [Security Group of AWS EC2](#heading-security-group-of-aws-ec2)
    
* [Install Coolify In AWS EC2](#heading-install-coolify-in-aws-ec2)
    
* [Access Coolify From Anywhere Using The Public IP](#heading-access-coolify-from-anywhere-using-the-public-ip)
    
* [How to Deploy A Website Via Coolify](#heading-how-to-deploy-a-website-via-coolify)
...
* A Raspberry Pi (see our [**Raspberry Pi OS Setup Guide**](https://coolify.io/docs/knowledge-base/how-to/raspberry-pi-os#prerequisites))
...
I recommend that you check the [official documents,](https://coolify.io/docs/get-started/installation) as there might be an update regarding the specifications, and you do not want to miss that part!
...
![Create EC2](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003070325/e9f40038-12d7-4918-828a-3114d43f8185.png align="center")

Give it any suitable name you want. I am going with `coolify-yt` for the purpose of this article.

![EC2 name](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003129368/d9726de2-f5e9-4d87-abbd-15660438c510.png align="center")

For the Amazon Machine Image (AMI), I will use the latest LTS of Ubuntu (Ubuntu Server 24.04 LTS).

![AMI Type](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003224058/cf23f6b2-c099-40b8-bec1-a39fb0384907.png align="center")

The architecture will be 64-bit (x86).

![AMI Architecture](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003299729/06970567-5523-4976-9156-d621ccf19899.png align="center")
...
![Instance Type](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003366450/465a7aad-c049-486a-baf4-6fb9a8ee1537.png align="center")

I will create a new key pair for key pair. I will go with RSA, and the file format will be `.pem`.

![Key Pair](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003554621/6ea1ccba-aeb9-496f-a207-37bf9d554e17.png align="center")

When you click on `Create key pair`, it will download a `.pem` file. Make sure to keep that safely.
...
![Storage configuration](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003637365/4238a847-271e-4f4b-a96f-5a84ec9bdf74.png align="center")
...
![Network settings](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003752198/d67af805-bef4-4273-b592-dd1703e56cfd.png align="center")
...
![Launch instance](https://cdn.hashnode.com/res/hashnode/image/upload/v1751003903936/277c79c0-f393-4d0f-a78e-5614915f9359.png align="center")
...
![Security Group](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004072271/0a1294e6-ae5c-4f61-ab55-6fc3cb1db6be.png align="center")

Now, go to the security group tab and find out which security group is associated with your newly created EC2.

![security group](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004120094/3a328a4b-3da6-4012-928e-0cc8a9414b29.png align="center")

As my EC2’s security group name was “launch-wizard-7”, I will click on that security group ID.

![Security group ID](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004179202/9919cb9f-a77d-47bd-a47d-52ac51f8a9f2.png align="center")

It will take me to its configuration page. Now, click on “Edit inbound rules”.

![Inbound rules](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004258702/da5ea5a7-ecdf-4358-8e0b-f428bf61bf85.png align="center")

Add a new rule. The type will be “Custom TCP”. The port will be “8000”. The source will be “Anywhere-IPv4”.

![Edit inbound rules](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004306106/249b91f7-c0fe-4797-80a0-6d059d55f922.png align="center")

Now save the rules.

![save changes](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004359517/fbaf4dc7-c608-424b-a890-e8c106971097.png align="center")

Now, go to the EC2 page and make sure that the newly created EC2 is running.

![EC2 status](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004404168/0321f546-a40e-4ba6-8815-414547e93346.png align="center")

If you want to connect to the EC2 server from your local machine, you have to use the `.pem` key. For that, you have to go to the “SSH Client” tab in the connect settings.

![SSH Client](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004603377/99a0a7ac-0d99-433d-bae8-e357cdedd2f2.png align="center")
...
![Connect EC2](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004526291/a56fe27c-2b7f-400d-94f7-b26dc66ad4a4.png align="center")

Make sure to note the Public IPv4 address of the EC2. Now click on “Connect”.

![Connect](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004626331/f5b8e5a5-5cef-4137-bbf7-807695b79e67.png align="center")

It will open a new tab with the EC2 connection in your browser.

![Connect EC2](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004654940/951377a0-7c3d-4e88-bc38-bc73b53da337.png align="center")
...
There are many ways to install Coolify directly on our EC2. But I usually follow the recommended [installation script of Coolify](https://coolify.io/docs/get-started/installation).
...
![Coolify script](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004776360/3f7ca4e5-e819-4c06-bf91-1b4b04edae9a.png align="center")

It will start installing Coolify into your server. Follow their prompts in the EC2 terminal.

![install coolify in EC2](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004847716/1994a54b-29ae-44a3-94dd-953c2bc9ffc5.png align="center")
...
![finish installation](https://cdn.hashnode.com/res/hashnode/image/upload/v1751004975296/c99da698-0e14-41b3-a32e-761d44edca29.png align="center")
...
![Register](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005087901/473c8834-e8dd-497e-bb29-ed56e50f9d07.png align="center")
...
![Skip onboarding](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005149629/fb3bcf68-d897-44f4-b210-4896ac630651.png align="center")
...
![Add new project](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005414046/2b21ea04-e90c-4f88-b853-81f10142f6b1.png align="center")

Give it a name and a suitable description.

![Name and description for the project](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005442344/dab216e0-10c9-4a05-8528-bfd662dc3094.png align="center")

I will use [one of my public repositories](https://github.com/FahimFBA/RainyRoof_Restaurant_Website) from GitHub for this trial.

![GitHub Repo](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005496252/a4c01732-3f38-4fa3-b3ba-d40ca3a4feba.png align="center")
...
![Project](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005567995/6908baa5-8305-4f3e-a30f-f4adceda5e06.png align="center")

Now, we need to add a resource from which it will pull the data.

![Add new resource](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005595787/123b4efe-a664-435a-9b26-88a14b636473.png align="center")

As this repository is already public, I will select “Public Repository” as my project source.

![Public repo](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005668775/f050c0d5-4c49-499a-895d-b3329c143440.png align="center")

Now, provide the public repo URL in the Repository URL field. Then click on “Check repository”.

![Public repo URL](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005713819/4923f951-4776-4442-9ced-6774b5ee05c7.png align="center")

It will show you the project's basic configuration. Check to ensure that everything is correct.

![Project config](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005789227/ca3fa656-a00c-4ba4-ab07-b892755b0335.png align="center")
...
![Update project spec](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005858468/9b1c5a9c-db49-4648-a53f-c5cfec1d4f6c.png align="center")

Now, deploy the application by clicking on the “Deploy” button.

![Deploy application](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005901660/737fc56a-390b-407a-b41b-fe7f7ce0d986.png align="center")

You can enable/disable the debug log as well.

![Debug log](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005931517/659f4812-2762-4e18-b543-2678a1b3e09b.png align="center")

After the deployment is finished successfully, you can find the website's autogenerated URL from the Links tab.

![Link](https://cdn.hashnode.com/res/hashnode/image/upload/v1751005979512/99b6f561-2d4a-4d24-b6a6-ea025857e909.png align="center")

The site will appear just fine!

![Live site](https://cdn.hashnode.com/res/hashnode/image/upload/v1751006003870/eab0135a-0c3d-4e17-bbe8-1a0df996d58e.png align="center")

You can also auto-generate a new domain or add your own domain/subdomain from the project settings at any time.

![Change domain](https://cdn.hashnode.com/res/hashnode/image/upload/v1751006064485/3a7c6a99-f2c8-43f8-82a9-b4ef3bbbcfcb.png align="center")
...
![Team](https://cdn.hashnode.com/res/hashnode/image/upload/v1751006188731/0d8471f4-7a74-4413-8cc2-81a42da410e9.png align="center")
...
To get more content like this, you can follow me on [GitHub](https://github.com/FahimFBA), [LinkedIn](https://www.linkedin.com/in/fahimfba/), and [YouTube](https://youtube.com/@FahimAmin). My [website](https://www.fahimbinamin.com/) is always available, too.
