---
title: Async Messaging with RabbitMQ and Tortoise in Node.js
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
...
At its most basic level, you’d ideally have two different services interacting with one another through Rabbit - a _publisher_ and a _subscriber_. 
...
Now a publisher typically publishes messages with a _routing key_ to something called an _exchange_. A consumer listens to a _queue_ on the same exchange, bound to the routing key. 
...
You can always just hit `Enter` all the way and use the default options - or you can fill them up. 

Now, let’s install the packages we need. We’re going to use [Tortoise](https://www.npmjs.com/package/tortoise) to interact with RabbitMQ. We’re also going to use [node-cron](https://www.npmjs.com/package/node-cron) to schedule the actual message publishing.
...
That’s simple enough. We’ve defined a dictionary containing a URL to the [RandomUser.me](https://randomuser.me/) API, which is then published to the `random-user-exchange` exchange on RabbitMQ, with the `random-user-key` routing key. 
