...
_Finding the link that lists a user’s followings_
...
With the User ID, I queried the `/_/api/users/<user_id>/fol`lowing endpoint and got the list of usernames from my Followings list.
...
_Medium uses pagination for Followings list_
...
At the end of the JSON response from `/_/api/users/<user_id>/fol`lowing, I saw an interesting key.
...
With the list of posts, I extracted all the responses using `https://medium.com/_/api/posts/<post_id>/res`ponses
...
User IDs are useless when you’re trying to access someone’s profile. I made this next function query the `/_/api/users/<user_`id> endpoint to get the usernames.
...
After I finished all the functions, I created a [pipeline](https://en.wikipedia.org/wiki/Pipeline_(software)) to get my list of recommended users.
...
_Image credit: [Know Your Meme](http://knowyourmeme.com/photos/185885-success-kid-i-hate-sandcastles" rel="noopener" target="_blank" title=")_
...
If you don’t know Python, go read [TK](https://www.freecodecamp.org/news/how-i-used-python-to-find-interesting-people-on-medium-be9261b924b0/undefined)’s [Learning Python: From Zero to Hero](https://medium.freecodecamp.org/learning-python-from-zero-to-hero-120ea540b567).
