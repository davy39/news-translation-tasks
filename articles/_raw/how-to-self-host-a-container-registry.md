...
There are many public and private registries available to developers such as [Docker Hub](https://hub.docker.com/), [Amazon ECR](https://aws.amazon.com/ecr/), and [Google Cloud Artifact Registry](https://cloud.google.com/artifact-registry/docs). But sometimes, instead of relying on an external vendor, you might want to host your images yourself. This gives you more control over how the registry is configured and where the container images are hosted.

This article is a hands-on tutorial that’ll teach you how to self-host a Container Registry.

## Table of Contents

* [What is a Container Image?](#heading-what-is-a-container-image)
    
* [What is a Container Registry?](#heading-what-is-a-container-registry)
    
* [Why you might want to self-host a Container Registry](#heading-why-you-might-want-to-self-host-a-container-registry)
    
* [How to self-host a Container Registry](#heading-how-to-self-host-a-container-registry)
    
* [Step 1: Install Docker and Docker Compose on the server](#heading-step-1-install-docker-and-docker-compose-on-the-server)
    
* [Step 2: Configure and run the registry container](#heading-step-2-configure-and-run-the-registry-container)
    
* [Step 3: Run NGINX for handling TLS](#heading-step-3-run-nginx-for-handling-tls)
    
* [Ready to go!](#heading-ready-to-go)
    
* [Other options](#heading-other-options)
...
Some registries are public, meaning that the images hosted on them are accessible to anyone on the Internet. Public registries such as [Docker Hub](https://hub.docker.com/) are a good option to host open-source projects.
...
There is a growing list of private registries available such as [Amazon ECR](https://aws.amazon.com/ecr/), [GCP Artifact Registry](https://cloud.google.com/artifact-registry/docs), [GitHub Container Registry](https://github.com/features/packages), and Docker Hub also offers a private repository feature.
...
There are several open-source container registry solutions available. The most popular one is officially supported by Docker, called [registry](https://hub.docker.com/_/registry), with its implementation for storing and distributing of container images and artifacts. This means that you can run your own registry inside a container.
...
The password file defined in **REGISTRY\_AUTH\_HTPASSWD\_PATH** is used to authenticate users when they push or pull images from the registry. We should create a password file using the **htpasswd** command. We should also create a folder for storing the images.
...
The Docker Registry requires a valid trusted SSL certificate to work. You can use something like [Let's Encrypt](https://letsencrypt.org/) or obtain it manually. Make sure you have a domain name pointing to your server (**registry.pliutau.com** in my case). For this demo I already obtained the certificates using [certbot](https://certbot.eff.org/) and put it in the **./nginx/certs** directory.
...
You can find the full source code for this demo on [GitHub](https://github.com/plutov/packagemain/tree/master/26-self-hosted-container-registry). Also, you can watch it as a video on [our YouTube channel](https://www.youtube.com/watch?v=TGLfQZ9qRaI).
