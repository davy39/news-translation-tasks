...
Let’s start with a statement from [Redux motivation page](https://redux.js.org/introduction/motivation):

> _This complexity is difficult to handle as **we’re mixing two concepts**_ that are very hard for the human mind to reason about: **mutation and asynchronicity.** I call them [Mentos and Coke](https://en.wikipedia.org/wiki/Diet_Coke_and_Mentos_eruption). Both can be great in separation, but together they create a mess.
...
Let’s first port [Redux VanillaJS counters](https://github.com/reduxjs/redux/blob/master/examples/counter-vanilla/index.html) example to illustrate the idea.
...
This version doesn’t look much different from Redux. The async generator there could be Redux storage middleware. This violates one of Redux [principles](https://redux.js.org/introduction/three-principles) though, namely storing all application state only in the storage. Even if the generator doesn’t have any local variables, it still has its execution state — the position in the code where the execution is suspended in `yield` or `await`.
...
There is [Decouple Business Logic](https://medium.com/dailyjs/decoupling-business-logic-using-async-generators-cc257f80ab33) article with more detailed cases study. At some step there, I added a multi-select feature to drag and drop without changing anything in single element handling. With a single store, this would mean changing its model from storing a single currently dragging element to a list.
...
There is another motivation for Redux described in [You Might Not Need Redux](https://medium.com/@dan_abramov/you-might-not-need-redux-be46360cf367) article by Dan Abramov — namely providing access to the state and it can be serialized, cloned, diffed, patched, etc. This can be used for time travel, hot reloading, universal applications and more.
...
To keep this story practical I don’t digress from the main subject here, there is another article — [Using Generators as syntax sugar for side effects](https://medium.com/@vitaliy.akimov/using-generators-as-monads-do-notation-8600c53648cf).

#### Effectful.js

[Effectful.js](https://effectful.js.org/) is a babel preset implementing do-notation working for any monad without any JavaScript syntax extension. It also supports state persistence with a reference implementation in [es-persist](https://github.com/awto/effectfuljs/tree/master/packages/es-persist) library. For example, this may be used to convert all async generators example above into pure functions.
...
Here is [the summary sample](https://github.com/awto/effectfuljs) on GitHub with all the feature above plus automatic Undo/Redo and storing its full state in `localStorage`. And here is [running transpiled](https://effectful.js.org/demo/alternative/) version (it writes to your browsers local storage but no information is sent to the server side). I’m not giving many details in this article, it is about async generators without dependency, but I suppose the code is straightforward to read. Check for example [undoredo.js](https://github.com/awto/effectfuljs/blob/master/samples/persist-counters/undoredo.js) for easy time traveling implementation details.
