---
title: Structured Data Types in C Explained
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
...
1. [ Definition and Declaration](#heading-1-definition-and-declaration)
2.  [Initialization and Accessing the Members of a Structure](#heading-2-initialization-and-accessing-the-members-of-a-structure)
3.  [Operating with Structure Variable](#3-operating-with-structure-variable)
4.  [Array Of Structure](#4-array-of-structure)
5. [Nested Structure](#heading-5-nested-structure)

**B. Memory Allocation**

1.  [Data Alignment](#heading-1-data-alignment)
2. [Structure Padding](#heading-2-structure-padding)
3. [Structure Member Alignment](#heading-3-structure-member-alignment)
4. [Structure Packing](#heading-4-structure-packing)

**C. Pointers**

1. [ Pointer as a Member](#heading-1-pointer-as-a-member)
2. [Pointer to Structure](#heading-2-pointer-to-structure)
3. [Pointer and Array of Structure](#heading-3-pointer-and-array-of-structure)

**D. Functions**

1. [Function as a Member](#heading-1-function-as-a-member)
2. [Structure as a Function Argument](#heading-2-structure-as-a-function-argument)
3. [Structure as a Function Return](#heading-3-structure-as-a-function-return)
...
For example, a student can have properties of name, age, gender and marks. We could create a `char`acter array for `name`, an `int`eger variable for `roll`, a `char`acter variable for gender, and an `int`eger array for `marks`. 
...
By convention, **uppercase** letters are used for _type definitions_ (such as `STUDENT`).
...
Uninitialized `int`egers and floating-point numbers have a default value of `0`. It is `\0`( NULL) for `char`acters and strings.
...
A better way would be to create an array of `struct student` (just like an array of `int`s).
...
One way to achieve this is by **aligning** the data. Aligning means that _a variable of any primitive data type of size `t` will always (by default) have an address that is a multiple of `t`_. This essentially is data alignment. This takes place every time.
...
This ensures that the members are _aligned_ on a 1-byte boundary. In other words, the address of any data type has to be a multiple of 1 byte or their size (whichever is lower).

## Pointers

> If you want to brush up pointers before going forward, here is a [link](https://www.freecodecamp.org/news/pointers-in-c-are-not-as-difficult-as-you-think/) to an article covering pointers in-depth.
...
Functions **can not** be a member of a structure. However, using _function pointers_, we can call functions using `.` . Just keep in mind that this is not recommended.
...
If the size of the structure is large, then passing a copy of it won't be very efficient. We could pass a **structure pointer** to a function. In this case, the address of the structure is passed as an _actual argument_.
...
Notice we have allocated the memory of size `struct rectangle` dynamically using `malloc()`. Since it returns a _void_ pointer, we have to _typecast_ it to a `struct rectangle` pointer.
...
Suppose we want a different structure variable after `stu1`, that is _insert another structure variable between `stu1` and `stu2`_. This can be done easily.
...
Notice how we have formed a link between `stu1`, `stuBetween` and `stu3`. What we have discussed here is the starting point of a _Linked List_.

Self-Referential Structures are very useful in creating data structures such as _linked list_, _stacks_, _queues_, _graphs_ and so on.
