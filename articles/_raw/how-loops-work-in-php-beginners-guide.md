...
* [Understand How Loops Work in PHP](#heading-understand-how-loops-work-in-php)
    
* [The `while` Loop in PHP](#heading-the-while-loop-in-php)
    
* [The `do...while` Loop in PHP](#heading-the-do-while-loop-in-php)
    
* [The `for` Loop in PHP](#heading-the-for-loop-in-php)
    
* [The `foreach` Loop in PHP](#heading-the-foreach-loop-in-php)
    
* [Use `break` Statement](#heading-use-break-statement)
    
* [Use `continue` Statement](#heading-use-continue-statement)
    
* [Replace Loops with `array_map()` in PHP](#heading-replace-loops-with-array_map-in-php)
    
* [Understand Nested Loops](#heading-understand-nested-loops)
...
![Diagram showing how loops work in PHP](https://cdn.hashnode.com/res/hashnode/image/upload/v1749910672507/5a4f946c-bbe8-407d-bfd0-c524d7fef9e0.png align="center")
...
This loop starts with `$count = 1`. It prints the value and adds 1 each time. It stops when `$count` becomes 4.
...
[PHP’s foreach](https://flatcoding.com/tutorials/php/foreach-loop-in-php/) loop is built to work with arrays. It lets you move through each item in the array, one at a time. You don’t need a counter, and you don’t need to check the size – PHP handles that part for you.
...
**Loop through values only:** this version gives you one value from the array on each run:
...
**Loop through keys and values**: This version gives you both the key and the value for each array item.
...
The [break statement](https://flatcoding.com/tutorials/php/php-break-exit-php-loop/) in PHP ends the loop immediately, even if its condition remains true. Here is its syntax:
...
Otherwise, it prints the value of `$i`. The output is 1 and 2, and the loop exits before it gets to 3.

Here are some common reasons to use the `break` statement:
...
[PHP’s continue statement](https://flatcoding.com/tutorials/php/php-continue-statement/) skips the rest of the loop for the current step. Here’s its syntax:
...
This loop doesn’t print 2 and moves to the next value. Output:
...
[PHP’s array\_map](https://flatcoding.com/tutorials/php/array_map/) function helps you avoid repetitive loops. It applies a [callback function](https://www.freecodecamp.org/news/how-to-use-arrow-functions-in-php/) to every item in one or more arrays and returns a new array with the results.
...
Now hopefully you understand how each [PHP loop](https://flatcoding.com/tutorials/php/php-loop-with-loops-examples/) works based on these examples and clear use cases.
