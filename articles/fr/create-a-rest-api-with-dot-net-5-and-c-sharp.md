---
title: Créer une API REST avec .NET 5 et C#
subtitle: ''
author: Beau Carnes
co_authors: []
series: null
...
- name: youtube
  slug: youtube
seo_title: Créer une API REST avec .NET 5 et C#
...
.NET est un framework logiciel populaire développé par Microsoft.

Nous venons de publier un cours sur la chaîne YouTube freeCodeCamp.org qui vous apprendra à créer une API REST de bout en bout à partir de zéro en utilisant les dernières innovations de .NET 5 et Visual Studio Code. Le cours utilise le langage de programmation C#.

Julio Casal a développé ce cours. Il est ingénieur logiciel principal chez Microsoft et il est également un excellent enseignant.

Voici toutes les sections couvertes dans ce cours complet :

### Installation

* Introduction
* Création du projet
* Exploration des fichiers de projet générés
* Confiance dans le certificat auto-signé
* Exploration de la page d'interface utilisateur Swagger par défaut
* Configuration des paramètres de Visual Studio Code

### Entité, Dépôt, Contrôleur GET

* Introduction
* Ajout d'une entité
* Ajout d'un dépôt en mémoire
* Création du contrôleur
* Implémentation de GET tous les éléments
* Implémentation de GET un seul élément
* Retour d'un code d'état 404 NotFound

### Injection de dépendances, DTOs

* Introduction
* Qu'est-ce que l'injection de dépendances ?
* Extraction de l'interface du dépôt
* Injection du dépôt dans le contrôleur
* Enregistrement du dépôt en tant que singleton
* Ajout d'un objet de transfert de données DTO
* Création de la méthode d'extension AsDto

### POST, PUT, DELETE

* Introduction
* Implémentation de POST
* Ajout de validations via des annotations de données
* Implémentation de PUT
* Implémentation de DELETE

### Persistance des entités avec MongoDB

* Introduction
* Utilisation de Postman
* Création d'un dépôt MongoDB
* Utilisation du package NuGet MongoDB.Driver
* Implémentation de MongoDB Create
* Exécution du conteneur Docker MongoDB
* Configuration des paramètres de connexion MongoDB
* Enregistrement du singleton MongoClient
* Test de l'intégration MongoDB
* Exploration de la base de données créée dans VS Code
* Implémentation de MongoDB Get, Update et Delete

### Tâches, Async et Await

* Introduction
* Utilisation du suffixe Async
* Utilisation des tâches dans le dépôt
* Utilisation de async et await
* Retour des tâches terminées
* Utilisation des tâches dans le contrôleur
* Test des méthodes async dans Postman

### Secrets et Vérifications de santé

* Introduction
* Activation de l'authentification dans MongoDB
* Utilisation du gestionnaire de secrets .NET
* Utilisation des informations d'identification MongoDB dans le service
* Introduction aux vérifications de santé
* Ajout d'un point de terminaison pour les vérifications de santé
* Ajout d'une vérification de santé MongoDB
* Ajout de vérifications pour la préparation et la disponibilité
* Personnalisation de la réponse de vérification de santé
* Exploration d'autres packages NuGet de vérification de santé
...
* Génération d'un Dockerfile dans VS Code
* Construction de l'image Docker
* Ajout d'un réseau Docker
* Exécution des conteneurs dans le réseau Docker
* Exécution de l'API REST dans Docker
* Poussage de l'image du conteneur vers Docker Hub
* Exploration de l'image dans Docker Hub
* Récupération de l'image vers la machine locale

### Kubernetes

* Introduction
* Qu'est-ce que Kubernetes ?
* Activation d'un cluster Kubernetes dans Docker Desktop
* Installation de l'extension Kubernetes pour VS Code
* Déclaration du déploiement Kubernetes de l'API REST
* Création d'un secret dans Kubernetes
* Déclaration des sondes de santé
* Déclaration du service Kubernetes de l'API REST
* Création des ressources de l'API REST dans Kubernetes
* Déclaration du StatefulSet Kubernetes de MongoDB
* Déclaration du service Kubernetes de MongoDB
* Création des ressources MongoDB dans Kubernetes
* Test de l'API REST hébergée dans Kubernetes
* Exploration de la capacité d'auto-guérison de Kubernetes
* Mise à l'échelle des pods Kubernetes
* Ajout de logs via ILogger
* Obtention d'une nouvelle version d'image dans Kubernetes
* Équilibrage de charge des requêtes entre les pods
...
* Restructuration des fichiers et répertoires
* Création du projet de test xUnit
* Construction de plusieurs projets dans VS Code
* Ajout de packages NuGet pour les tests unitaires
* Test de GetItemAsync pour un élément inexistant
* Utilisation du modèle AAA
* Simulation des dépendances via Moq
* Exécution des tests dans VS Code
* Utilisation de l'extension .NET Core Test Explorer
* Test de GetItemAsync pour un élément existant
* Utilisation de FluentAssertions
* Test de GetItemsAsync
* Test de CreateItemAsync
* Test de UpdateItemAsync
* Test de DeleteItemAsync
* Refactorisation et détection des régressions
* Utilisation du TDD pour tester une méthode non encore créée
* Retour au vert en corrigeant le test échoué
* Test de la nouvelle méthode du contrôleur dans Postman

Regardez le cours ci-dessous ou [sur la chaîne YouTube freeCodeCamp.org](https://youtu.be/ZXdFisA_hOY) (6 heures de visionnage).
...
Vous allez apprendre à utiliser .NET 5 pour créer une API Rest. Le créateur de ce cours est un ingénieur logiciel principal chez Microsoft et il est également un excellent enseignant.

L'API REST permet à votre application ou système d'exposer ses fonctionnalités à plusieurs types de clients, à la fois à l'intérieur et à l'extérieur de votre réseau, y compris des clients à travers Internet. C'est ce que vous utiliseriez si vous vouliez écrire un programme pour collecter des données à partir de Twitter, Yahoo Finance, ou même la NASA. Si vous envisagez de construire votre propre API REST et que vous envisagez la plateforme dotnet pour cela, restez avec moi, car je vais vous montrer comment faire cela de bout en bout en utilisant les dernières innovations fournies par dotnet. Five, j'espère que vous allez apprécier.

Dans la première partie de ce tutoriel, vous allez apprendre le scénario à utiliser dans le tutoriel, comment créer un projet Web API dotnet five à partir de zéro, comment utiliser Visual Studio code pour construire et déboguer le projet, nous allons travailler sur la façon de dessiner le certificat de développement installé par dotnet. Five, et cela sera nécessaire pour l'accès HTTPS et comment utiliser swagger UI pour interagir avec l'API.

Pour suivre le tutoriel, vous aurez besoin de quelques éléments, notamment le SDK dotnet five, Visual Studio code et une compréhension de base du langage C sharp.

Maintenant, parlons de ce scénario que nous allons utiliser comme domaine pour ce tutoriel. Imaginons que nous avons une sorte de système de catalogue. Et nous avons un tas d'articles disponibles. Dans ce sens, comme je suis un joueur vidéo, j'aime penser à ces articles comme des objets que j'utiliserai dans un jeu vidéo. Des objets comme des potions, des épées, des boucliers et des choses comme ça. D'accord. Donc c'est un système que nous avons en place, il a un catalogue. Et bien sûr, nous allons avoir des utilisateurs qui voudront gérer ce catalogue via leur navigateur, n'est-ce pas ? Ils ont un navigateur, ils veulent gérer ces éléments de catalogue, d'une manière ou d'une autre.

Donc ce qu'ils voudraient faire, c'est bien, comment allons-nous créer des éléments dans le catalogue ? Comment pouvons-nous récupérer la liste des éléments actuellement disponibles dans ce catalogue ? Comment pouvons-nous mettre à jour les propriétés des éléments. Et comment pouvons-nous supprimer des éléments dans ce catalogue. Comme c'est le cas aujourd'hui, nous avons le catalogue disponible. Mais nous manquons, nous n'avons pas de moyen d'exposer ce catalogue à l'internet afin que les gens puissent aller de l'avant et le gérer à partir du navigateur. C'est là que nous allons introduire notre API REST pour ce catalogue. Et au cours de ce tutoriel, nous allons voir comment construire cette API REST à partir de zéro en utilisant dotnet. Five.

Donc nous voici dans Visual Studio Code. Et la première chose que nous allons faire est d'ouvrir un tout nouveau terminal. Et dans ce terminal, nous allons simplement passer au répertoire où nous allons créer notre projet et créer un projet, nous allons utiliser le CLI dotnet.

Pour ce faire, nous allons simplement dire que, que new et le type de produit que nous voulons créer pour notre API REST, ce sera Web API. Et le nom du projet sera catalogue, appuyez sur Entrée.

Et cela crée tous les fichiers de votre projet, basé sur le modèle Web API. Donc maintenant je vais ouvrir ce dossier qui a été généré, catalogue.

Et, comme d'habitude, lorsque vous ouvrez un projet dotnet dans Visual Studio Code, il vous demandera d'ajouter quelques fichiers supplémentaires pour construire et déboguer le projet. Donc je vais dire Oui, et ces fichiers sont générés sous .vscode. Donc sur le côté gauche, vous pouvez voir tous les fichiers générés. Et prenons un rapide aperçu de chacun de ces fichiers

comme un rapide aperçu. Donc la première partie que nous allons regarder est le fichier .csproj. Ce fichier est utilisé, c'est ce qu'on appelle le fichier de projet. Et il est utilisé pour déclarer comment nous allons construire ce projet. Dans ce cas, nous disons que nous allons utiliser le SDK web dotnet pour construire le projet, qui inclut un tas d'outils et de tâches spécifiquement conçus pour les projets web. La chose intéressante suivante sera le framework cible, un moniteur ou framework cible, qui dans notre cas est net five. Le framework cible définit les API ou les API ou quel type d'API seront disponibles pour votre projet. Dans ce cas, net five est parfaitement adapté pour nous. Et la chose suivante sera un tas de packages NuGet que nous n'allons pas approfondir pour l'instant. Mais ce sont simplement des dépendances que nous avons déjà acquises pour ce projet. Fermons cela et ensuite nous allons jeter un coup d'œil à ce programme. C

Sa prononciation est ce que nous appelons le point d'entrée de l'application. Et ce qu'il fera, c'est simplement démarrer le host. Je veux dire, le processus va devenir l'hôte de notre problème. Et il déclare également un tas de valeurs par défaut. Et il configure également ce que nous appelons la classe de démarrage pour notre projet. Donc allons-y, allons à startup et voyons ce qui se passe là.

Vraiment, les principales choses dans startup sont que

nous avons cette propriété appelée configuration que nous recevons dans le constructeur de startup. Cela peut être utilisé chaque fois que vous devez lire certaines informations de configuration à partir de plusieurs sources, comme des variables d'environnement ou des fichiers, différents types de fichiers, ou un tas d'autres endroits, une configuration que vous ne voulez pas coder en dur dans votre service. La méthode intéressante suivante est configure services. Et c'est l'endroit où vous

enregistrerez tous les services que vous allez utiliser dans votre application. Et nous en parlerons plus tard dans cette vidéo. Et la dernière pièce intéressante est la méthode Configure.

C'est là que nous configurons ce que nous appelons le

pipeline, le pipeline de requête sur ASP. net. Et cela définit simplement un tas de middlewares typiques, qui sont

des composants supplémentaires qui s'exécuteront avant votre contrôleur ou votre code.

Chacun de ces éléments peut s'exécuter

lorsqu'une requête arrive dans le processus ASP. NET. Et de là, jusqu'à ce que votre code s'exécute. Mais nous n'allons pas explorer cette partie dans cette vidéo.

Quelques autres choses sont donc nous avons une prévision météorologique. Donc c'est un modèle qui est généré pour cette application d'exemple ici, il n'y a que quelques propriétés très simples ici. Et avec ce modèle, il y a un contrôleur. Maintenant, le contrôleur dans ASP. NET est simplement la classe qui gère la route.

Oui, assez beaucoup les routes que votre service expose, mais nous n'allons pas les utiliser dans cette vidéo. Donc ne nous attardons pas trop là-dessus.

Quelques autres fichiers, appsettings.json, c'est là que vous pouvez déclarer la configuration, qui va être que vous ne voulez pas utiliser de code dur dans votre programme, dans votre code source. Et pour l'instant, il n'a que quelques configurations pour la journalisation et les hôtes autorisés dans l'application. Il existe également une variante de appsettings JSON, qui est appsettings.Development.json. Donc développement, je veux dire, le fait qu'il dit que cela dit appsettings.Development.json signifie que nous exécutons dans un environnement de développement,

ces paramètres prendront le pas sur le appsettings.json. Donc vous pourriez avoir un tas de ces fichiers appsettings pour chacun de vos environnements comme la production, le test, l'intégration, tous ces environnements.

Et je parlerai des environnements, peut-être un bon moment pour jeter un coup d'œil aux fichiers .vscode, qui sont task.json et launch.json. Donc task.json est simplement un fichier qui déclare les tâches avec ces concepts clés et pièces de votre code, vous pouvez déclarer des tâches et une tâche peut être un tas de choses. Et dans notre cas, la chose la plus intéressante est le fait que nous voulons utiliser la commande dotnet build. Donc dotnet build, qui sera utilisée pour construire notre code. Et en termes de launch.json, ce fichier contrôle ce qui sera lancé ou exécuté lorsque nous faisons un F5. Ou lorsque nous commençons à déboguer le code, dans ce cas, il pointe déjà vers la bonne DLL pour commencer à déboguer.

Enfin,

nous avons également des paramètres de lancement. Donc les fichiers launchSettings.json sont vraiment la seule partie intéressante ici qu'ils veulent regarder, c'est l'URL de l'application. Ici, nous définissons les URLs, URLs impériales, pour notre application, dans ce cas, nous disons que nous allons servir notre serveur en localhost 5004, la version HTTPS sera servie dans laquelle sera la version par défaut sera 5001. Nous déclarons également la variable d'environnement ASPNETCORE_ENVIRONMENT. Et nous la définissons comme développement. Et donc

aussi, dans Visual Studio Code. Ce n'est pas celui qui a été codé en dur, celui qui sera codé en dur est celui qui va être mangé à midi, launchSettings.json. Comme vous le voyez ici. C'est celui qui prend la préséance s'il était de recoder. Donc c'est bien.

Bien, ce que nous voulons faire maintenant, c'est tester ce projet pour nous assurer que tout fonctionne comme prévu. Donc ce que je vais faire, c'est que je vais passer au hub de débogage.

Écoutez-moi développer cela un peu. Et donc ce que je ferais, c'est cliquer et commencer à travailler. Voyons ce qui se passe.

D'accord. Donc une page de navigateur s'ouvre. Et si vous obtenez cette page ici, eh bien, ce que cela signifie est assez normal, cela signifie que nous n'avons pas le certificat auto-signé qui accompagne dotnet, il n'a pas été approuvé. D'accord ? Donc il y a un moyen très simple de contourner cette décision et d'approuver correctement le certificat qui accompagne dotnet. Donc laissez-moi simplement fermer cela, et arrêter. Et revenons au terminal. En fait, laissez-moi ouvrir un nouveau terminal ici. Oui, le voilà.

Pour approuver un certificat qui accompagne le SDK dotnet, ce que vous devez faire, c'est simplement taper dotnet dev-certs https

trust.

Lorsque vous exécutez cela, vous obtiendrez cette fenêtre contextuelle, vous demandant de confirmer que vous souhaitez vraiment approuver ce certificat, je vais dire oui. Et puis cela devrait le faire. Donc je vais l'exécuter à nouveau.

Voyons ce que nous obtenons.

D'accord, donc oui, nous n'obtenons toujours pas grand-chose ici. Mais nous ne voyons plus de problème de confiance. Maintenant, si ce que nous voulons voir quelque chose de significatif ici, bien que cela n'ait pas trop d'importance pour nous à ce stade, ce que vous pouvez faire, c'est simplement aller à swagger.

Et vous obtiendrez cette belle interface utilisateur. Donc c'est ce que nous appelons swagger ou aussi une spécification OpenAPI. Donc c'est un composant qui est maintenant intégré à dotnet five, donc vous n'avez rien à faire pour le rendre disponible. Comme vous l'avez vu, nous n'avons rien fait. Donc il est juste là. Dans l'URL slash swagger. Ce que cela fait, c'est qu'il permet de décrire facilement toutes les opérations, toutes les actions, toutes les routes qui sont disponibles dans votre API, et permet également d'interagir facilement avec elles. Par exemple, si je vais simplement à la route get, et que je clique sur trade out, cliquez sur Exécuter, il ira de l'avant et l'exécutera. Et nous pouvons déjà voir certains résultats pour cette route.

Et une chose que j'aime faire lorsque je travaille sur ces projets, c'est de ne pas ouvrir le navigateur à chaque fois que j'exécute le projet. Donc allons-y, changeons un peu le comportement de VS code pour que nous gardions simplement cette fenêtre ouverte. Et ensuite, chaque fois que nous appuyons sur f5, ou que nous cliquons sur Exécuter, il n'ouvrira plus de fenêtres. Pour ce faire, je vais m'arrêter ici. Et nous pouvons aller à launch.json et fermer cela.

La seule chose que vous avez à faire est de supprimer cette section ServerReadyAction ici.

Et c'est à peu près tout. Si je relance, et au fait, vous pouvez le faire en appuyant simplement sur f5,

que je vais faire maintenant, cinq.

Cela démarre l'hôte, comme vous pouvez le voir, mais nous n'avons pas ouvert plus de fenêtres, ce qui est bien. Parce que nous avons le navigateur prêt à appeler juste là.

Ensuite, la dernière chose que j'aime faire en termes de configuration de pratique est de simplifier la façon dont nous construisons notre projet en morceaux pour enregistrer. Pour cela, laissez-moi minimiser cela et revenir à Visual Studio Code. Arrêtez cela, fermez cela, ce que je vais faire, c'est aller à tasks.json. Et la seule chose que je vais faire est d'ajouter cette petite section ici sous la tâche de construction, qui est simplement ce que nous appelons le groupe, qui va construire et il dit est par défaut vrai. Ce que cela fait, c'est qu'il nous permet de construire plus facilement un projet. Donc maintenant je peux aller à, laissez-moi juste sauvegarder cela. Je peux aller à terminal, exécuter des tâches de construction et construire immédiatement. Sans cela, nous sommes capables de simplement avoir une fenêtre contextuelle encore un autre menu pour faire la construction. Je peux aussi maintenant faire Ctrl Shift B et cela fera la même chose. Oui, cela accélère simplement notre situation.

Dans la deuxième partie du tutoriel sur l'API REST .NET 5, nous avons introduit les fondations de notre API, qui incluent l'entité centrale utilisée pour représenter les éléments de votre catalogue, la classe de dépôt responsable de toutes les opérations liées au stockage des éléments, et le contrôleur qui gérera toutes les requêtes envoyées à une API REST.

Ici, vous apprendrez comment modéliser une entité via les types d'enregistrements C#.

Comment implémenter un dépôt en mémoire.

Sources

et comment implémenter un contrôleur avec une route GET pour récupérer des ressources.

D'accord, maintenant que nous sommes à l'aise avec la configuration initiale de notre projet, il est temps de commencer à configurer les entités et un dépôt qui sera utilisé pour stocker et récupérer les éléments qui seront utilisés dans le service. Donc la première chose que nous allons faire est, eh bien, fermons simplement ceux-ci et le terminal. Et débarrassons-nous de quelques-unes de ces classes qui n'ont vraiment aucun sens pour notre projet. Donc je vais simplement supprimer la prévision météorologique ici. Et je vais supprimer le contrôleur de prévision météorologique. Donc nous pouvons commencer plus proprement que cela.

Donc, introduisons notre entité, l'entité item. Pour cela, ajoutons un dossier ici appelé

entités. Donc les gens diront domaine, comme beaucoup de gens peuvent voir des modèles, dans notre cas, les entités devraient être suffisamment bonnes. Créons un fichier ici, appelons-le

item. C'est Oui.

Suivons le bon espace de noms ici.

Donc la page suivante devrait être catalog comme il porte le dossier ntds, disons entités.

Et puis

ce que vous feriez habituellement à ce stade est de déclarer une classe, n'est-ce pas, donc vous diriez public class et disons-le. Cependant, depuis .NET 5 et C# 9, il y a une nouvelle option ici pour vous, que nous appelons les types d'enregistrements. Donc les types d'enregistrements sont presque des classes. Mais ils ont un meilleur support pour les données immuables, les modèles de données immuables, n'est-ce pas, ce qui signifie que une fois que vous obtenez une instance de cet objet, il n'est pas possible de la modifier vraiment. Et c'est assez pratique, surtout pour les objets que vous recevez par le réseau. Donc tous ceux-ci proviennent du web, généralement, vous les prenez et faites quelque chose avec eux, mais vous ne voulez pas les modifier. N'est-ce pas ? De plus, les types d'enregistrements ont cette chose appelée expression with que vous allez voir plus tard dans ce tutoriel. Et aussi une capacité à comparer sur la base d'une valeur. Donc l'égalité basée sur la valeur est ce qu'ils appellent, ce qui signifie que

quand vous comparez deux instances de ces éléments, dans ce cas,

ces instances seront égales, seulement si toutes les propriétés de cette instance sont les mêmes, par opposition à juste l'identité de l'objet lui-même, ce qui serait le cas des classes. Donc les types d'enregistrements sont assez pratiques, je pense qu'ils sont une excellente option pour les objets que nous allons utiliser ici. Donc

changeons la classe pour l'enregistrement. C'est le moment d'introduire les propriétés pour cet enregistrement. Donc ajoutons quelques propriétés ici. Donc voyons. Donc utilisons un bon pour notre ID.

Et laissez-moi importer cet espace de noms système, l'espace de noms système manquait ici. Et avant d'ajouter plus de propriétés, laissez-moi faire un petit changement ici. Au lieu d'utiliser Get set, passons à init. Pourquoi l'avez-vous vu ? C'est une autre addition dans C# 9 dans .NET 5, qui est un excellent ajustement pour,

pour les initialiseurs de propriétés. Où nous voulons seulement permettre la définition d'une valeur pendant l'initialisation, n'est-ce pas ? Donc cela signifie que, par exemple, dans le passé, vous pouviez dire que vous pouviez dire, Get set. Ce que cela signifie, c'est qu'après avoir créé l'objet, vous pouvez simplement modifier dans ce cas, l'ID à tout moment. Donc ce n'est pas vraiment souhaitable, nous voulons une propriété immuable. Donc pour immuable, nous devons définir

privates, n'est-ce pas ? Donc oui, cela le rend immuable. Mais maintenant, il est assez difficile de construire un ensemble de ces objets. Maintenant, nous devons introduire un constructeur. Et nos clients n'ont pas vraiment un moyen plus agréable de parcourir nos objets. Donc pour trouver un bon terrain d'entente, ils ont introduit l'accesseur init ici. Donc cela signifie que

vous pouvez utiliser une expression de création pour construire cet objet item, comme vous le feriez avec un set. Mais après la création, vous ne pouvez plus modifier cette propriété. C'est un très bel équilibre entre les deux mondes. Je verrai comment cela se passe plus tard. Donc à ce stade, définissons simplement nos propriétés. Donc voyons. Ajoutons bien sûr, le nom.

Nom.

C'est bien d'étudier le nom

Oh,

Aussi dedans, ajoutons,

ajoutons un prix pour notre article.
...
un décalage horaire. C'est

super.

Donc ce sera la date et l'heure où l'article a été noté dans le système. N'oublions pas non plus de changer cela en init.

Maintenant, il est temps d'introduire notre dépôt, qui est la classe qui sera responsable du stockage des articles dans le système. Pour garder les choses simples pour l'instant, nous n'utiliserons qu'un dépôt en mémoire. Et dans quelques épisodes plus tard dans le tutoriel, nous apporterons une base de données appropriée.

Donc, créons un nouveau dossier ici appelé dépôts.

Et ici, nous ajouterons un nouveau fichier. Appelons-le

inmemoryitemrepository.cs.

L'espace de noms ici, n'oubliez pas que ce sera catalog.repositories.

D'accord.

Et maintenant, créons une classe. Donc

classe publique, je vais simplement prendre le nom du fichier principal.

D'accord ?

Nous allons faire ici, comme je l'ai dit, c'est en mémoire. Donc nous allons simplement définir une liste très simple d'articles qui

seront les initialisateurs avec lesquels nous allons travailler. Donc déclarons une liste ici. Donc voyons, écrivons et la rendons en lecture seule car elle ne doit pas changer. Je veux dire, l'instance de la liste ne doit pas changer après que nous avons construit ce dépôt.

objet.

C'est bien d'être une liste d'items

nous avons utilisé pour les combattre.

Et je pense,

oui, donc ces articles.

Et oui, nous devons importer le

quelques espaces de noms, leur collection générique pour la liste. Et cet autre entités pour l'item.

Et nous voulons dire que c'est nouveau.

Et nous voulons déclarer la liste. Maintenant, c'est une autre addition dans C# 9.

Comme vous le voyez ici, dans le passé, vous auriez peut-être eu besoin de dire, d'accord, donc c'est une nouvelle liste d'items, n'est-ce pas ? Mais c'est un peu redondant. Puisque nous savons déjà clairement que c'est une liste d'items. Donc pourquoi tant de cérémonie, donc supprimons simplement cela. Et c'est tout ce que vous avez à faire à ce stade. Belle addition, C# 9.

Donc, ajoutons quelques articles ici. Donc disons nouvel article.

Et faisons,

faisons

l'initialisation ici. Donc faisons simplement un random go it.

Vous allez pour l'ID. Encore une fois, importons l'espace de noms du musée. Et donc pour le nom. Et donc

comme je l'ai mentionné dans l'introduction, j'aime jouer avec ces idées en termes d'objets de jeu vidéo. Donc le premier objet que je vais choisir ici est un avortement. Super classique dans ces jeux vidéo. Et disons que le prix pour celui-ci va être juste neuf. Et pour la date de création. Eh bien, je vais simplement dire

décalage horaire

etc. Maintenant. Donc cela signifie maintenant en termes de UTC, donc pas l'heure locale, juste l'heure UTC.

Et faisons en sorte que nous en ayons besoin, ajoutons un

couple de plus.

Donc même un bon. Je veux dire, obtenez un nouveau GUI pour chacun de ceux-ci. Mais appelons celui-ci. Ce sera un mot d'ordre, resort. Et disons qu'il sera plus cher. Faisons-le à 20. Et pour le dernier, disons que nous allons faire

rose shoot.

Doit être un peu moins cher, disons.

D'accord. Oh, là nous avons notre liste initiale d'articles prête à partir.

Maintenant, dans ce dépôt, nous allons devoir gérer un tas de choses, n'est-ce pas ? Donc comment obtenons-nous un article ? Comment obtenons-nous une collection d'articles ? Comment créons-nous un article, mettons-le à jour, supprimons toutes ces choses. Donc pour simplifier, commençons par un get. Donc nous allons faire deux get metals ici.

Le premier sera get items pour obtenir des items si nous allons faire, nous allons simplement retourner un ienumerable de item.

items. D'accord ? Donc un nominal est assez basique, c'est une interface très basique

que vous pouvez utiliser pour retourner une collection d'items.

Et puis oui, c'est aussi simple que de dire, eh bien, comme le vôtre, de retourner, peu importe ce que nous avons dans les items maintenant, c'est

ce genre de méthode get items. Et le suivant sera similaire. Mais ce gars va retourner juste un item, il va s'appeler get item.

Mais cela devra connaître l'ID de l'item à retourner. D'accord ?

Démarrer. Et puis. Donc pour récupérer le bon item, ce que nous allons faire, c'est dire, d'accord, donc retournons de la collection d'items.

Et je vais dire où ces importations sont encore un autre espace de noms, assises dans ce lien d'importation maintenant.

Et donc nous allons dire, d'accord, donc de cette liste,

où l'item

ID

est égal à l'ID que nous avons obtenu dans le paramètre.

Et cela va retourner une collection. Mais nous ne voulons pas la collection, qui est une, le seul item qu'il devrait trouver, tous les dossiers où le fichier va être ? Non. Donc s'il trouve l'item, il le retournera. S'il ne le trouve pas, il retournera null.

D'accord, donc cela constitue notre dépôt. Et donc la prochaine chose à apporter est le contrôleur. Donc comme je l'ai dit, le contrôleur va être la

classe qui reçoit une requête qui a été envoyée par le client et la gère correctement. Donc ajoutons notre classe de contrôleur. Donc un nouveau fichier sous le dossier des contrôleurs, et

scènes, les raisons que nous traitons ici sont des items. Donc la convention sera de l'appeler simplement. ItemsController.

C'est Oui. D'accord. Et encore, sécurité, espace de noms du catalogue, dans ce cas, il va être des contrôleurs, des contrôleurs. D'accord.

Donc ce contrôleur va être

toute la classe ItemsController. Et puis la chose intéressante à propos des classes de contrôleur est que vous voulez toujours hériter de controllerbase

cela vous donne et cet espace de noms important ici.

Donc cela transformera effectivement cela en une classe de contrôleur. Donc héritons toujours de controllerbase.

C'est la première chose. La chose suivante est de marquer cette classe comme un contrôleur d'API. Juste là. Oui, APIController. Donc cela apporte un tas de comportements par défaut supplémentaires pour votre classe de contrôleur, ce qui facilite simplement notre vie. Donc oui, n'oubliez pas d'ajouter la couche APIController. Et oui, et au fait, il y a des tonnes de documentation sur chacun de ces attributs, ou les

la chose suivante est de déclarer la route. Donc la route définit à quelle route HTTP ce contrôleur va répondre. Et,

par défaut, ce que vous mettrez ici est simplement le nom du contrôleur. Donc si vous faites cela, cela signifiera que quel que soit le nom du contrôleur, ce sera la route. D'accord ? Et donc dans ce cas, par exemple, disons pour un get,

il fera get slash items, n'est-ce pas ? Ce sera une partie de l'URL que nous allons utiliser.

Donc oui, je veux dire, vous pouvez le faire de l'une ou l'autre manière. Ou vous pouvez déclarer explicitement une route que vous voulez utiliser, comme, je pourrais dire, deux items, ce qui est parfaitement valide. Et en fait, restons avec cela.

Et continuons.

Maintenant, ici dans ce contrôleur, bien sûr,

pour toute action, ou toute opération que nous allons faire, nous allons avoir besoin de ce dépôt sur lequel nous avons travaillé il y a un instant. Donc apportons une instance de cela. Nous allons déclarer private readonly.

Encore une fois, parce qu'il ne sera pas modifié après la construction.

IItemRepository. D'accord. Et

Oui, pour tout espace que nous avons prêt. Maintenant, avant de continuer. Laissez-moi simplement vous dire que nous allons faire quelques choix pas si idéaux en avançant. Juste pour expliquer que

Un tas de concepts, n'est-ce pas ? Donc le fait que j'introduise une dépendance explicite dans i ng meme it suppository.

Ce n'est pas idéal, mais je veux juste le faire pour garder les choses simples et puis améliorer au fur et à mesure que nous avançons. D'accord ? Donc ne prenez pas cela comme une parole finale sur la façon dont vous devriez faire cela. Maintenant, nous allons construire ceux-ci dans le

dans le constructeur de ce contrôleur. Donc ajoutons un

constructeur de contrôleur public.

D'accord, et puis disons oui, donc double story

est égal à nouveau nom. C'était, n'est-ce pas. Donc boom, nous avons un dépôt prêt à être utilisé ? Donc définissons notre route d'état pour trouver une route. Donc voyons. Comment trouvons-nous une route pour récupérer tous les articles ? Sur ce stockage ? Déclarons une méthode appelée

Hi enumerable.

Item ? Désolé, item, get items.

D'accord, donc ces méthodes vont retourner un ienumerable d'items comme le dépôt. Importons les bons espaces de noms. En manquons-nous ici ?

D'accord.

Mais alors aussi, pour que cette méthode devienne réellement une route et réagisse à un verbe HTTP, vous devez la déclarer correctement, l'attribut correct dans ce cas, HTTP GET est ce que nous voulons déclarer pour cela. En faisant cela, cela signifie que lorsque quelqu'un effectue un get contre slash items, c'est une méthode qui va réagir à cela, n'est-ce pas ? Je vois comment

quelques cas nous avons presque la même route, mais c'est le verbe qui fait la distinction dans quelle méthode va être invoquée. Alors que faisons-nous dans ce HTTP GET ? Donc assez simple, nous allons dire, d'accord, donc nous avons des items qui vont être un dépôt.

Vous vous souvenez, nous avons créé cette méthode get items. Donc nous l'invoquons simplement, nous avons les items, et puis nous retournons les items.

Et c'est tout.

D'accord, donc avec cela, nous devrions être capables de tester cela et voir comment cela se passe. Donc je vais appuyer sur f5 ici.

D'accord, aller à swagger. Et je vais utiliser la page de rafraîchissement que nous avons ouverte à partir de la vidéo précédente. Comme vous pouvez le voir, tout a été rafraîchi. Il n'y a plus de contrôleur météo. Maintenant nous avons un contrôleur d'items. Et nous avons notre première route ici, nous utilisons un get pour les items.

Non seulement cela, nous avons aussi un schéma qui décrit un

item, comment il sera disposé. Si nous cliquons sur items. Et voyons, essayons-le. Voyons ce que nous obtenons.

Et juste ici, comme vous pouvez le voir, swagger montrera le

router boot exécuté, comme nous l'avons dit, c'est un get sur slash items. Cela

est ici, c'est notre hôte et le port. Donc contre localhost 5001 items. Et
...
And as you can see, we do have the antidote from the previous video still hanging around, that's working just fine. Now let's try to create a brand new item, right, so how would we call it let's say this is going to be
...
And I'm going to actually remove this breakpoint.
...
and they are to publish, and then all the other steps that ended up creating that image. So at this point, we have an image ready. And like I said, each of the lines going back to a local file, each of the lines here represents one layer that maps to each of the steps that you see here. So these are the layers that will be cached from here on,
