---
title: Apprendre les Hooks React – Un guide pour débutants
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
date: '2022-02-14T21:04:27.000Z'
originalURL: https://freecodecamp.org/news/the-beginners-guide-to-react-hooks
coverImage: https://www.freecodecamp.org/news/content/images/2022/02/The-Beginner-s-Guide-to-React-Hooks.png
tags:
- name: JavaScript
  slug: javascript
- name: React
  slug: react
- name: react hooks
  slug: react-hooks
- name: Web Development
  slug: web-development
seo_title: null
seo_desc: "Par Victor Ikechukwu. Les composants fonctionnels n'ont pas toujours été la méthode préférée pour déclarer des composants dans React. Avant l'introduction de la version 16.8 de React, les composants fonctionnels étaient traités un peu comme des citoyens de seconde zone. Ils ne pouvaient pas gérer l'état..."
---

Par Victor Ikechukwu

Les composants fonctionnels n'ont pas toujours été la méthode préférée pour déclarer des composants dans React.

Avant l'introduction de la version 16.8 de React, les composants fonctionnels étaient traités un peu comme des citoyens de seconde zone. Ils ne pouvaient pas gérer l'état, la logique et de nombreuses autres fonctionnalités de React, et nous ne les utilisions que pour le rendu de composants très simples dans l'UI.

La version 16.8 de React a résolu ces problèmes en introduisant les Hooks React, qui permettent aux développeurs d'utiliser ces fonctionnalités de React dans les composants fonctionnels.

Dans cet article, vous apprendrez :

* Ce que sont les Hooks React
* Quatre Hooks React courants avec des exemples sur la façon de les écrire dans vos applications
* Enfin, nous verrons comment écrire vos propres Hooks React personnalisés

## Que sont les Hooks React ?

Les Hooks sont des fonctions intégrées à React introduites dans la version 16.8 de React. Ils vous permettent d'utiliser des fonctionnalités de la bibliothèque React telles que les méthodes de cycle de vie, l'état et le contexte dans les composants fonctionnels sans avoir à vous soucier de les réécrire sous forme de classe.

Chaque nom de Hook React est préfixé par le mot `"use"`. Par exemple, `useState` ou `useEffect`. Ce format a été choisi parce que les Hooks permettent aux développeurs d'utiliser les fonctionnalités spéciales de la bibliothèque React. Vous « utilisez » donc cette fonctionnalité spéciale de la bibliothèque React.

## Pourquoi utiliser les Hooks React ?

De nombreux développeurs sont sceptiques à l'idée d'apprendre les Hooks React. Mais vous ne devriez pas l'être. Voici quelques raisons pour lesquelles vous devriez commencer à utiliser les Hooks React :

### Les classes dans React peuvent être assez déroutantes

Les classes sont un obstacle à l'apprentissage correct de React. Pour les utiliser, vous devez comprendre comment fonctionne le mot-clé `this`. Vous devez également vous rappeler constamment de lier (bind) les gestionnaires d'événements, ainsi que d'autres méthodes redondantes rencontrées lors du travail avec des classes dans React.

### Les composants de classe sont complexes et peuvent être difficiles à comprendre

Les composants de classe sont généralement volumineux et tentent d'effectuer de nombreuses opérations. À long terme, ils deviennent difficiles à comprendre.

Les Hooks résolvent ce problème en vous permettant de séparer les composants volumineux en diverses fonctions plus petites, plutôt que de devoir forcer toute la logique dans un seul composant.

### Les Hooks ont des composants plus courts et une meilleure lisibilité

Les composants de classe s'accompagnent de beaucoup de code répétitif (boilerplate). Considérez le composant compteur ci-dessous :

```javascript
class Counter extends Component {
    constructor(props) {
        super(props)
        this.state = {
        	count: 1,
        }
    }
    render() {
        return (
            <div>
                Le compteur actuel : {this.state.count}
                <div>
                <button onClick={this.setState({ count: this.state.count - 1 })}>
                ajouter
                </button>
                <button onClick={this.setState({ count: this.state.count + 1 })}>
                soustraire
                </button>
                </div>
            </div>
    );
    }
}
```

Voici le code équivalent utilisant un composant fonctionnel et les Hooks React :

```javascript
function Counter  ()  {
    const [count, setCount] = useState(1);
    return (
        <div>
            Le compteur actuel : {this.state.count}
            <div>
                <button onClick={() => setCount(count + 1)}>ajouter</button>
                <button onClick={() => setCount(count - 1)}>soustraire</button>
            </div>
        </div>
    );
};
```

Remarquez à quel point le composant de classe est beaucoup plus complexe. Vous avez besoin d'une classe pour étendre React, d'un constructeur pour initialiser l'état, et vous devez référencer le mot-clé `this` partout.

L'utilisation de composants fonctionnels supprime une grande partie de cela, de sorte que notre code devient plus court et plus facile à lire et à maintenir.

## Règles d'utilisation des Hooks React

Lors de l'utilisation des Hooks React, il y a quelques règles à respecter :

* **Appelez les hooks uniquement au niveau supérieur d'un composant** : Vous ne devriez pas utiliser de Hooks à l'intérieur de boucles, de conditions ou de fonctions imbriquées. Au lieu de cela, utilisez toujours les Hooks au niveau supérieur de votre fonction React, avant tout mot-clé return.
* **Appelez les hooks uniquement depuis des fonctions React** : N'appelez jamais de Hooks depuis des fonctions JavaScript régulières. Vous pouvez :  
✅ Appeler des Hooks depuis des composants fonctionnels React.  
✅ Appeler des Hooks depuis des Hooks personnalisés.

## Les Hooks React les plus courants

À ce jour, React possède 10 hooks intégrés. Jetons un coup d'œil aux quatre plus courants :

* `useState`
* `useEffect`
* `useContext`
* `useReducer`

### Hook useState

Le Hook useState vous permet de créer, mettre à jour et manipuler l'état à l'intérieur des composants fonctionnels.

React a ce concept d'état (state), qui sont des variables contenant des données dont dépendent nos composants et qui peuvent changer au fil du temps. Chaque fois que ces variables changent, React met à jour l'UI en effectuant un re-rendu du composant dans le DOM avec les valeurs actuelles des variables d'état.

Le hook prend un seul argument optionnel : une valeur initiale pour l'état. Ensuite, il renvoie un tableau de deux valeurs :

* La variable d'état
* Une fonction pour mettre à jour l'état

Jetons un coup d'œil à un composant compteur à titre d'exemple :

Pour utiliser un Hook, la première étape consiste à importer le Hook en haut du fichier :

```javascript
import { useState } from "react";
```

Ensuite, initialisez le Hook avec une valeur. Étant donné qu'il renvoie un tableau, vous pouvez utiliser la décomposition de tableau (array destructuring) pour accéder aux éléments individuels du tableau, comme ceci :

```javascript 
const [count, setCount] = useState(0);
```

Avec cela, le code du composant sera :

```javascript
import { useState } from "react";

function Counter() {
    // Déclare une nouvelle variable d'état, que nous appellerons "count"
    const [count, setCount] = useState(0);
    return (
        <div>
        Nombre d'articles dans le panier : {count}
            <div>
            <button onClick={() => setCount(count - 1)}>Ajouter au panier</button>
            <button onClick={() => setCount(count + 1)}>Retirer du panier</button>
            </div>
        </div>
    );
}
```

Voici à quoi ressemblera le composant lors du rendu.

![Image](https://www.freecodecamp.org/news/content/images/2022/02/Counter.gif)

En cliquant sur le bouton **Ajouter au panier** ou **Retirer du panier**, la valeur de la variable d'état count changera et le composant sera re-rendu avec la valeur mise à jour de l'état.

### Hook useEffect

Si vous êtes familier avec les méthodes de cycle de vie des classes React, vous pouvez considérer le Hook `useEffect` comme les méthodes de cycle de vie `componentDidMount`, `componentDidUpdate` et `componentWillUnmount` toutes combinées en une seule fonction. Il vous permet de reproduire les méthodes de cycle de vie de React dans les composants fonctionnels.

Le Hook `useEffect` vous permet d'effectuer des effets de bord (side effects) dans les composants fonctionnels. Les effets de bord sont des actions qui peuvent s'exécuter parallèlement aux opérations principales d'un composant, telles que les interactions avec des API externes, la modification de variables d'état et la récupération de données.

Le hook `useEffect` accepte 2 arguments :

* Une fonction avec le code à exécuter
* Un tableau qui contient une liste de valeurs de la portée du composant (props, contexte et variables d'état), connu sous le nom de tableau de dépendances, qui indique au Hook de s'exécuter chaque fois que sa valeur est mise à jour. S'il n'est pas fourni, le Hook s'exécutera après chaque rendu.

Voici un exemple d'utilisation du Hook :

```javascript
import { useState, useEffect } from "react";
function Counter() {
    // Déclare les variables d'état
    const [count, setCount] = useState(0);
    const [product, setProduct] = useState("Eggs");
    useEffect(() => {
    	console.log(`${product} va conquérir le monde !`);
    });
    return (
        <div>
        Compteur de {product} : {count}
            <div>
                <button onClick={() => setCount(count + 1)}>Ajouter au panier</button>
                <button onClick={() => setCount(count - 1)}>Retirer du panier</button>
                Changer de produit :{" "}
                <input type="text" onChange={(e) => setProduct(e.target.value)} />
            </div>
        </div>
    );
}
```

Dans l'exemple, l'effet s'exécutera après chaque mise à jour de l'état.

![Image](https://www.freecodecamp.org/news/content/images/2022/02/Effect-default.gif)

![Image](data:image/gif;base64,R0lGODlhGAAYAHAAACH5BAEAAAEALAAAAAAYABgAgZmZmf///wAAAAAAAAIdjI8JkO231psw0iuv3rz7D4biSJbmiabqyrbuVgAAOw==)

#### Comment déclencher un effet de manière conditionnelle

Pour n'exécuter le Hook que lorsque certaines valeurs ont changé, passez les variables en tant que dépendance dans le tableau :

```javascript 
useEffect(() => {
	console.log(`${product} va conquérir le monde !`);
}, [product]); // Ne réexécute l'effet que si la valeur de product change
```

Avec ce changement, le Hook ne s'exécutera qu'au premier rendu et lorsque la valeur de product est modifiée.

![Image](https://www.freecodecamp.org/news/content/images/2022/02/Effect-dependency-array.gif)

#### Comment l'exécuter une seule fois au premier rendu

Si vous voulez qu'un effet ne s'exécute qu'une seule fois au premier rendu, comme pour effectuer des appels API lors du premier affichage du composant, vous pouvez passer un tableau vide comme dépendance comme ceci :

```javascript 
useEffect(() => {
	console.log("Ceci s'exécute une fois au premier rendu");
}, []);
```

En fournissant un tableau vide, cela indique au Hook de n'écouter aucun changement d'état, il ne s'exécutera donc qu'une seule fois.

### Hook useContext

Le Hook `useContext` fonctionne avec l'API React Context. Il vous permet de rendre certaines données accessibles à tous les composants de l'application, quel que soit leur niveau d'imbrication.

React a un flux de données unidirectionnel, où les données ne peuvent être transmises que du parent vers l'enfant. Pour transmettre des données (comme l'état) d'un parent à un composant enfant, vous devrez les passer manuellement en tant que prop à travers différents niveaux selon la profondeur d'imbrication du composant enfant.

Pour des données telles que la langue préférée de l'utilisateur, le thème ou les propriétés de l'utilisateur authentifié, il est fastidieux de devoir les passer manuellement dans l'arbre des composants.

L'API Context de React et le Hook `useContext` facilitent le passage de données à travers tous les composants de l'application.

Il accepte un objet de contexte créé à l'aide de `React.createContext` et renvoie le contexte actuel comme ceci :

```javascript 
const value = useContext(SomeContext);
```

Voyons un exemple de fonctionnement du Hook :

Tout d'abord, créez un contexte pour utiliser le Hook. Par exemple, voici un UserContext pour obtenir la valeur des utilisateurs actuels :

```javascript
import React from "react";
// quelques valeurs de contexte fictives
const users = [
{
    name: "Harry Potter",
    occupation: "Wizard",
},
{
    name: "Kent Clark",
    occupation: "Super hero",
},
];

export const UserContext = React.createContext(users);
```

Chaque contexte possède un wrapper Provider, qui permet à ses composants enfants de s'abonner aux changements du contexte et transmet la valeur du contexte via une prop `value`.

Si la prop `value` du fournisseur est mise à jour, ses composants enfants consommateurs seront re-rendus avec la nouvelle valeur du contexte.

```javascript 
function Users() {
return (
    <UserContext.Provider value={users}>
    <UserProfile />
    </UserContext.Provider>
);
}
```

Dans l'exemple, `UserProfile` devient le composant consommateur du contexte.

```javascript 
import React, { useContext } from "react";
import { UserContext } from "./App";

export function UserProfile() {
    const users = useContext(UserContext);
    return (
        <div>
            {users.map((user) => (
            <li>
            Je suis {user.name} et je suis un {user.occupation} !
            </li>
            ))}
        </div>
    );
}
```

Cela affichera les propriétés des utilisateurs actuels :


![Image](data:image/png;base64,)


### Hook useReducer

Le Hook `useReducer` est une alternative au Hook `useState`. La différence est qu'il permet une logique plus complexe et des mises à jour d'état impliquant plusieurs sous-valeurs.

Semblable à `useState`, `useReducer` vous permet de créer des variables de type état qui provoquent la mise à jour de l'UI chaque fois qu'elles changent.

Ce Hook accepte 2 arguments : une fonction reducer et un état initial.

`useReducer(reducer, initialState);`

Il renvoie un tableau de deux valeurs qui peuvent être décomposées en la valeur actuelle de l'état et une fonction de dispatch.

```javascript 
const [state, dispatch] = useReducer(reducer, initialState);
```

Apprenons-en plus sur ses arguments et les valeurs retournées :

* **state** : Il s'agit de la valeur actuelle de l'initialState passé au Hook.
* **reducer** : Le reducer est une fonction qui accepte l'état et une action. Sur la base de ces arguments, elle détermine comment la valeur de l'état va changer.
* **dispatch** : La fonction dispatch est la façon dont nous passons une action à la fonction reducer. Elle envoie l'action à utiliser pour mettre à jour l'état.

Généralement, nous itérons sur le type d'actions effectuées dans notre application via une instruction `switch` pour déterminer comment la valeur de l'état va changer. C'est ainsi que le Hook met à jour les valeurs de son état.

```javascript 
function reducer(state, action) {
    switch (action.type) {
        case "CASE_1":
        return {
        	updatedState,
        };
        case "CASE_2":
        return {
        	updatedState,
        };
        default:
        	return state;
    }
}
```

La fonction dispatch envoie généralement un objet au format :

```javascript 
dispatch({ type: "ACTION_TYPE", payload: optionalArguments });
```

Où `type` est la description de l'action et `payload` sont les arguments que vous souhaitez passer au reducer.

## Comment créer des Hooks personnalisés

Un Hook personnalisé consiste à extraire la logique de composant couramment utilisée de l'UI vers des fonctions JavaScript en utilisant les Hooks React déjà disponibles. Cela vous aide à éviter la duplication de code et vous permet de rendre cette logique réutilisable au sein de plusieurs composants.

Voyons un exemple de hook personnalisé qui renverra une réponse de n'importe quelle URL d'API valide que nous lui passons.

```javascript 
//useFetch.js
import { useState, useEffect } from "react";

export function useFetch(url) {
	//valeurs
    const [data, setData] = useState(null);
    const [error, setError] = useState("");
    useEffect(() => {
        fetch(url)
        .then(res => {
            if (!res.ok) {
            throw Error("quelque chose ne va pas, impossible de se connecter à la ressource");
        }
        setData(res.json());
        })
        .then(() => {
        	setError("");
        })
        .catch( error => {
            console.warn(`désolé, une erreur s'est produite à cause de ${error.message} `);
            setData(null);
            setError(error.message);
        });
    }, [url]);
    return [data, error];
}
```

Vous pouvez maintenant utiliser cette logique n'importe où dans votre application simplement en important la fonction et en passant un chemin d'API comme argument, plutôt que de tout réécrire de zéro.

## Conclusion

J'espère que vous avez pu voir à quel point les Hooks React sont utiles. Ils vous permettent de créer des composants efficaces à la volée sans vous soucier des tracas liés aux composants de classe.

Qu'il s'agisse de vous permettre de vous concentrer sur l'écriture de votre code principal ou de vous permettre de créer vos propres Hooks personnalisés... les Hooks React sont vraiment géniaux ! J'ai hâte que vous les essayiez par vous-même.

Si vous avez trouvé cet article utile, n'hésitez pas à le partager avec vos amis et votre réseau. N'hésitez pas non plus à me contacter sur [Twitter](https://twitter.com/Victor_codejs) et sur mon [blog](https://vickyikechukwu.hashnode.dev/) où je partage une large gamme d'articles et de ressources pédagogiques gratuits.

Merci de m'avoir lu, et bon code !