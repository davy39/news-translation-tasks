---
title: Comment créer une application de feedback de prototype en temps réel en utilisant
  Vue.js et Pusher
subtitle: ''
author: freeCodeCamp
co_authors: []
series: null
date: '2018-03-15T11:06:37.000Z'
originalURL: https://freecodecamp.org/news/how-to-create-a-realtime-prototype-feedback-app-using-vuejs-and-pusher-daeddae0e055
coverImage: https://cdn-media-1.freecodecamp.org/images/1*Cbn9fgwjzMGSgXOilm6-pQ.png
tags:
- name: JavaScript
  slug: javascript
...
seo_title: Comment créer une application de feedback de prototype en temps réel en
  utilisant Vue.js et Pusher
seo_desc: 'By Neo Ighodaro

  Here is a screen recording of what our application will be able to do once we are
  done building it:


  Requirements we will need to build our application

  Before we get started, we need to have a few things in place first. Some of the
  re...'
---

Par Neo Ighodaro

Voici un enregistrement d'écran de ce que notre application sera capable de faire une fois que nous aurons terminé de la construire :

![Image](https://cdn-media-1.freecodecamp.org/images/YVd8jgeUFHzOyx54R4jQCvqssr1z1l9MqzoJ)

### Exigences dont nous aurons besoin pour construire notre application

Avant de commencer, nous devons avoir quelques choses en place. Certaines des exigences sont les suivantes :
...
### Installation de notre application de feedback de prototype
...
Lorsque l'installation est terminée, `cd` dans le répertoire de l'application. Ouvrez le fichier `.env` et apportons quelques modifications dans le fichier.
...
Maintenant, créons quelques migrations qui créeront les tables requises dans la base de données. Dans votre terminal, exécutez la commande suivante pour créer les migrations dont nous aurons besoin :

```
$ php artisan make:model Photo --migration --controller$ php artisan make:model PhotoComment --migration
```

La commande ci-dessus créera un modèle et les indicateurs `--migration` et `--controller` lui indiqueront de créer une migration et un contrôleur en même temps que le modèle.

Pour l'instant, nous nous intéressons au Modèle et à la migration. Ouvrez les deux fichiers de migration créés dans le répertoire `./database/migrations`. Tout d'abord, nous allons modifier la classe `CreatePhotosTable`. Remplacez le contenu de la méthode `up` par ce qui suit :
...
Dans ce qui précède, nous avons ajouté la propriété `fillable` qui nous empêche d'avoir des exceptions d'assignation de masse lorsque nous essayons de mettre à jour ces colonnes en utilisant `Photo::create`. Nous avons également défini la propriété `with` qui charge simplement les relations `comments` de manière anticipée.

Nous avons défini une relation Eloquent `comments` qui dit simplement que la `Photo` a plusieurs `PhotoComments`.
...
Tout comme le modèle `Photo`, nous avons défini la propriété `fillable`. Nous utilisons également les [accesseurs Eloquent](https://laravel.com/docs/5.5/eloquent-mutators#accessors-and-mutators) pour configurer une nouvelle propriété appelée `position` qui est ensuite ajoutée car nous l'avons spécifié dans la propriété `appends`.
...
Cela installera [Laravel Echo](https://laravel.com/docs/5.5/broadcasting#installing-laravel-echo), le [SDK JS Pusher](https://github.com/pusher/pusher-js), et [vue-dropzone](https://github.com/rowanwins/vue-dropzone). Nous aurons besoin de ces packages pour gérer les événements en temps réel plus tard.
...
Dans le code ci-dessus, nous avons défini quelques routes. La première gérera les feedbacks `POST`. La deuxième route affichera l'image qui doit recevoir des feedbacks. La troisième route gérera les téléchargements, et la route finale affichera la page d'accueil.

Maintenant, ouvrez le fichier `./resources/views/welcome.blade.php` et remplacez le contenu par le code HTML suivant :

```
<!doctype html><html lang="{{ app()->getLocale() }}"><head>    <meta charset="utf-8">    <meta http-equiv="X-UA-Compatible" content="IE=edge">    <meta name="viewport" content="width=device-width, initial-scale=1">    <meta name="csrf-token" content="{{csrf_token()}}">    <title>Télécharger pour obtenir des feedbacks</title>    <link href="https://fonts.googleapis.com/css?family=Roboto:400,600" rel="stylesheet" type="text/css">    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">    <link rel="stylesheet" href="{{ asset('css/app.css') }}"></head><body>    <div id="app">        <div class="flex-center position-ref full-height">            <div class="content">                <uploadarea></uploadarea>            </div>        </div>    </div>    <script src="js/app.js"></script></body></html>
```

Il s'agit d'un simple document HTML, et si vous regardez de près, vous verrez une référence à une balise `uploadarea` qui n'existe pas en HTML mais qui est en fait un composant Vue.
...
### Utilisation de Vue pour créer les fonctionnalités de notre application de feedback de prototype

Ouvrez le fichier `./resources/assets/js/app.js` et créons notre composant Vue. Dans ce fichier, trouvez la ligne ci-dessous :
...
Dans la section `template`, nous utilisons simplement le package Vue dropzone pour définir une zone à travers laquelle les fichiers peuvent être téléchargés. Vous pouvez consulter la documentation [ici](https://github.com/rowanwins/vue-dropzone).

Dans la section `script`, nous obtenons le jeton CSRF Laravel à partir de l'en-tête de la page et importons le composant `Dropzone` dans notre composant Vue actuel.

Dans la propriété `methods`, nous définissons une méthode `showImagePage` qui redirige simplement l'utilisateur vers la page de l'image après que l'image a été téléchargée avec succès. Dans la méthode `mounted`, nous limitons le fichier dropzone pour permettre un téléchargement de fichier à la fois.

Créons notre prochain composant Vue. Dans le répertoire `./resources/assets/js/components`, créez un nouveau fichier appelé `FeedbackCanvas.vue` et collez ce qui suit :
...
Sous `methods`, nous définissons la méthode `addCommentPoint` qui calcule les coordonnées du clic, puis lance une nouvelle modale Bootstrap qui va être créée dans le composant Vue `add-comment-modal`.
...
Créez un fichier `AddCommentModal.vue` dans le même répertoire que nos autres composants Vue. Dans ce fichier, collez le code ci-dessous :

```
<template>    <div id="add-modal" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">        <div class="modal-dialog">            <div class="modal-content">                <form method="post" :action="'/feedback/'+photo.url+'post'" @submit.prevent="submitFeedback()">                    <div class="modal-header">                        <h4 class="modal-title">Ajouter un feedback</h4>                    </div>                    <div class="modal-body">                        <textarea name="feedback" id="feedback-provided" cols="10" rows="5" class="form-control" v-model="feedback" placeholder="Entrez un feedback..." required minlength="2" maxlength="2000"></textarea>                    </div>                    <div class="modal-footer">                        <button type="submit" class="btn btn-primary pull-right">Soumettre</button>                        <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Annuler</button>                    </div>                </form>            </div>    </div>    </div></template><script>export default {    props: ['image'],    data() {        return { photo: this.image, feedback: null }    },    methods: {        submitFeedback: function () {            let vm = this            let modal = $('#add-modal')            let position = modal.data("comment-position")            // Créer l'URL et la charge utile            let url = `/feedback/${this.photo.url}/comment`;            let payload = {comment: this.feedback, left: position.left, top: position.top}            axios.post(url, payload).then(response => {                this.feedback = null                modal.modal('hide')                vm.photo.comments[modal.data('comment-index')] = response.data                $(document).ready(() => $('[data-toggle="popover"]').popover())            })        }    }}</script>
```

Dans la section `template`, nous avons défini une modale Bootstrap typique. Dans le formulaire de la modale, nous avons attaché un appel à `submitFeedback()` qui est déclenché lorsque le formulaire est soumis.

Dans la section `script`, nous avons défini la méthode `submitFeedback()` dans la propriété `methods` du composant Vue. Cette fonction envoie simplement un commentaire au backend pour stockage. Si une réponse favorable est reçue de l'API, la modale Bootstrap est masquée et le commentaire est ajouté au tableau `image.comments`. Le popover Bootstrap est ensuite rechargé pour qu'il prenne en compte les modifications.
...
### Création des endpoints pour notre application de feedback de prototype
...
Cela créera une classe d'événement appelée `FeedbackAdded`. Nous utiliserons ce fichier pour déclencher des événements vers Pusher lorsque nous ajouterons des feedbacks. Cela permettra aux feedbacks d'apparaître en temps réel pour toute personne regardant l'image.
...
Maintenant que nous avons créé la vue, nous devons ajouter le répertoire pour les téléchargements défini dans la méthode `upload`. Dans votre terminal, exécutez la commande ci-dessous :
...
Maintenant que nous avons créé le backend pour soutenir notre application web, nous devons ajouter Pusher au backend afin que les commentaires faits soient diffusés et puissent être récupérés par d'autres personnes navigant sur l'image.

### Ajout de fonctionnalités en temps réel à l'application de feedback de prototype en utilisant Pusher

Ouvrez votre terminal et entrez la commande ci-dessous pour installer le [SDK PHP Pusher](https://github.com/pusher/pusher-http-php) :
...
> _? N'oubliez pas de remplacer les valeurs PUSHER_ID, PUSHER_KEY, PUSHER_SECRET et PUSHER_CLUSTER par les valeurs de votre application Pusher._
...
Dans la classe ci-dessus, nous définissons l'objet `comment` et le `photo_id` qui seront utilisés pour composer le nom du canal dans la méthode `broadcastOn`. Nous définissons également la méthode `broadcastAs`, qui nous permettra de personnaliser le nom de l'événement envoyé à Pusher.
...
![Image](https://cdn-media-1.freecodecamp.org/images/gwSPC6wbQ1dB1FRtn3r4p3Aqu7MPhlyP56yT)

Cela devrait démarrer un nouveau serveur PHP et vous pourrez alors l'utiliser pour tester votre application. Allez à l'URL donnée et vous devriez voir votre application.

### Conclusion

Dans cet article, nous avons réussi à créer une fonctionnalité de feedback pour une application prototype qui permettra aux designers de partager leurs designs avec d'autres et de recevoir des feedbacks dessus.

Si vous avez des questions ou des feedbacks, veuillez les laisser en commentaire ci-dessous. Le code source de l'application est disponible sur [GitHub](https://github.com/neoighodaro/Design-Prototype-Feedback-Application).

_Cet article est d'abord apparu sur le [blog de Pusher](https://blog.pusher.com/create-realtime-prototype-feedback-app-using-vuejs-pusher/)._